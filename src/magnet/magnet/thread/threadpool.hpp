...
private:
  ...
  /*! \brief This mutex is to control access to write that an exception has
   * occurred.
   */
...
public:
   ...
   */
  inline ThreadPool()
      : _exception_flag(false), _idlingThreads(0), _stop_flag(false) {}
  ...
  inline void setThreadCount(size_t x) {
    ...
    for (size_t i = _threads.size(); i < x; ++i)
      _threads.create_thread(
          std::function<void()>(std::bind(&ThreadPool::beginThread, this)));
  }
  ...
  inline void wait() {
    if (_threads.size()) {
      ...
      while (!_waitingFunctors.empty() || (_idlingThreads != _threads.size()))
    ...
    if (_exception_flag)
      M_throw()
          << "Thread Exception found while waiting for tasks/threads to finish"
          << _exception_data.str();
  }
...
private:
  ...
  inline void beginThread() {
    try {
      ...
      std::cout << "\nTHREAD :Catastrophic Failure of thread!!! System will "
                   "Hang, Aborting!"
                << p.what();
      throw;
    }
