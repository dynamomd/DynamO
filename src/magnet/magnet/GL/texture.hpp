...
protected:
  ...
  inline TextureBasic(GLenum texType) : _valid(false), _texType(texType) {}
...
public:
  ...
  */
  ...
  inline GLuint getGLHandle() {
    if (!_valid)
      M_throw() << "Texture is not initialised yet";
    return _handle;
  }
  ...
  inline GLint getInternalFormat() const {
    if (!_valid)
      M_throw()
          << "Cannot query the internal format of an uninitialised texture";
...
public:
  ...
  GLint getPixelCount(GLint lvl = 0) const { return getWidth(lvl); }
  ...
  inline void subImage(const std::vector<uint8_t> &data, GLenum pixelformat,
    ...
    if (width < 0)
      width = _width;
    if (xoffset < 0)
      M_throw() << "x offset is negative";
    if (xoffset + width > _width)
      M_throw() << "Texture write x overrun";

    bind(0);
    glTexSubImage1D(_texType, level, xoffset, width, pixelformat,
  ...
  inline void subImage(const std::vector<GLfloat> &data, GLenum pixelformat,
    ...
    bind(0);
    glTexSubImage1D(_texType, level, xoffset, width, pixelformat, GL_FLOAT,
                    &data[0]);
    detail::errorCheck();
  }

  inline const GLint getWidth(GLint lvl = 0) const {
    return _width / (1 << lvl);
  }
...
public:
  ...
  /*! \brief Initializes a 2D texture.
    ...
    \param internalformat The underlying format of the texture.
   */
  inline virtual void init(size_t width, size_t height,
  ...
  /*! \brief Fills a section of the texture with the passed data
    ...
    \param level The texture mipmap level to write to.
   */
  inline virtual void subImage(const std::vector<uint8_t> &data,
                               GLenum pixelformat, GLint xoffset = 0,
                               GLint yoffset = 0, GLint width = -1,
    ...
    if (xoffset < 0)
      M_throw() << "x offset is negative";
    if (yoffset < 0)
      M_throw() << "y offset is negative";
    if (xoffset + width > _width)
      M_throw() << "Texture write x overrun";
    if (yoffset + height > _height)
      M_throw() << "Texture write y overrun";
  ...
  /*! \brief Fills a section of the texture with the passed data
    ...
    \param level The texture mipmap level to write to.
   */
  inline virtual void subImage(const std::vector<GLfloat> &data,
                               GLenum pixelformat, GLint xoffset = 0,
                               GLint yoffset = 0, GLint width = -1,
                               GLint height = -1, GLint level = 0) {
    // If there are negative values, assume they mean to use the
  ...
  inline virtual void subImage(const uint8_t *data, GLenum pixelformat,
                               GLint width, GLint height, GLint xoffset = 0,
                               GLint yoffset = 0, GLint level = 0) {
    // If there are negative values, assume they mean to use the
    // full space
    if (xoffset < 0)
      M_throw() << "x offset is negative";
  ...
  inline const GLint getHeight(GLint lvl = 0) const {
    return _height / (1 << lvl);
  }
...
public:
  Texture2DMultisampled(GLint samples = 1, bool fixedSampleLocations = false)
      : Texture2D(GL_TEXTURE_2D_MULTISAMPLE),
        _fixedSampleLocations(fixedSampleLocations), _samples(samples) {}
  ...
  inline virtual void init(size_t width, size_t height,
    ...
    glTexImage2DMultisample(_texType, _samples, _internalFormat, _width,
                            _height, _fixedSampleLocations);
    detail::errorCheck();
  }
  ...
  inline void subImage(const uint8_t *data, GLenum pixelformat, GLint width,
                       GLint height, GLint xoffset = 0, GLint yoffset = 0,
                       GLint level = 0) {
    M_throw() << "Cannot perform subimage on a multisampled texture";
  }
...
public:
  ...
  GLint getMaxDimension() const {
    return std::max(std::max(_width, _height), _depth);
  }
  ...
  inline void init(size_t width, size_t height, size_t depth,
    ...
    glTexImage3D(_texType, 0, _internalFormat, _width, _height, _depth,
                 0, // Border is off
                 // The following values are not used (except by GL debug tools)
                 safeFormat(), safeType(), NULL);
    detail::errorCheck();
  ...
  inline void subImage(const std::vector<GLubyte> &data, GLenum pixelformat,
    ...
    if (yoffset < 0)
      M_throw() << "y offset is negative";
    if (zoffset < 0)
      M_throw() << "z offset is negative";

    if (xoffset + width > _width)
      M_throw() << "Texture write x overrun";
    if (yoffset + height > _height)
      M_throw() << "Texture write y overrun";
    if (zoffset + depth > _depth)
      M_throw() << "Texture write z overrun";

    bind(0);
    glTexSubImage3D(_texType, level, xoffset, yoffset, zoffset, width, height,
                    depth, pixelformat, safeType(), &data[0]);
    detail::errorCheck();
  }
  ...
  inline const GLint getDepth(GLint lvl = 0) const {
    return _depth / (1 << lvl);
  }
