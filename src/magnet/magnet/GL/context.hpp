...
public:
   ...
   */
  inline void setAttribute(GLuint idx, GLfloat x = 0, GLfloat y = 0,
                           GLfloat z = 0, GLfloat w = 0) {
    ...
    if (oldval != _vertexAttributeState[idx].current_value)
      M_throw()
          << "Vertex attribute state changed without using the GL context!";
#endif
  ...
  /*! \brief Convenience function to set the vertex attribute
    ...
    the color attribute.
   */
  inline void color(GLfloat r = 0, GLfloat g = 0, GLfloat b = 0,
                    GLfloat a = 1) {
  ...
  inline void rotation(GLfloat angle, math::Vector axis) {
    ...
    setAttribute(instanceOrientationAttrIndex, axis[0] * s, axis[1] * s,
                 axis[2] * s, c);
  }
  ...
  inline void setViewport(const std::array<GLint, 4> &val) {
    ...
    _viewPortState = val;
    glViewport(_viewPortState[0], _viewPortState[1], _viewPortState[2],
               _viewPortState[3]);
    detail::errorCheck();
  }
   ...
   */
  inline size_t getFrameCount() const { return _frameCounter; }
  ...
  inline void setBlend(bool newstate) {
    testAndSetState(newstate, _blend, GL_BLEND);
  }
  inline void setAlphaTest(bool newstate) {
    testAndSetState(newstate, _alphaTest, GL_ALPHA_TEST);
  }
  inline void setDepthTest(bool newstate) {
    testAndSetState(newstate, _depthTest, GL_DEPTH_TEST);
  }
  inline void setCullFace(bool newstate) {
    testAndSetState(newstate, _cullFace, GL_CULL_FACE);
  }
...
protected:
  inline void testAndSetState(const bool newstate, bool &oldstate,
                              const GLenum cap) {
#ifdef MAGNET_DEBUG
  ...
  */
  ...
  inline void initOpenCLContext() {
    std::cout << "GL-Context " << _context << ": Creating an OpenCL GPU context"
              << std::endl;
    if (initOpenCLContextType(CL_DEVICE_TYPE_GPU))
      return;
    std::cout << "GL-Context " << _context
              << ": Failed to create OpenCL GPU context, trying all devices "
                 "for OpenCL context"
              << std::endl;
  ...
  inline void init() {
    ...
    std::cout << "GL-Context " << _context << ": Created a new OpenGL context"
              << std::endl;
    //////////Capability testing /////////////////////////////
    ...
    std::cout << "GL-Context " << _context << ": OpenGL version "
  ...
  Context()
      : _blend(false), _alphaTest(false), _depthTest(false), _cullFace(false),
  ...
  struct VertexAttrState {
    VertexAttrState() : active(false), divisor(0) {
