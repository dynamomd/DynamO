...
#define STRINGIFY(A) #A
    virtual std::string initVertexShaderSource() {
      return STRINGIFY(layout(location = 1) in vec4 vColor;
    ...
    virtual std::string initGeometryShaderSource() {
      return STRINGIFY(
          ...
          smooth out vec2 texCoord; flat out vec4 frag_color;
          ...
          void main() {
            ...
            gl_Position =
                ProjectionMatrix * (ViewMatrix * vec4(-1.0, -1.0, 0.0, 1.0));
            EmitVertex();
    ...
    virtual std::string initFragmentShaderSource() {
      return STRINGIFY(uniform sampler2D cairoTexture; smooth in vec2 texCoord;
                       ...
                       void main() {
                         if (ALPHA_TESTING > 0) {
                           if (texture(cairoTexture, texCoord).r <= 0.5)
                             discard;
...
public:
  ...
  virtual void init(size_t width, size_t height, size_t alpha_testing = 0) {
    ...
    _cairoSurface = Cairo::ImageSurface::create(
        _alpha_testing ? Cairo::FORMAT_A8 : Cairo::FORMAT_ARGB32, _width,
        _height);
  ...
  void syncCairoGL() {
    ...
    if (_alpha_testing) {
      ...
      image::SignedDistanceTransform(_cairoSurface->get_data(), _width,
                                     _height);
      ...
      for (size_t y(0); y < texYSize; ++y)
        for (size_t x(0); x < texXSize; ++x)
          downsampled[y * texXSize + x] =
              _cairoSurface->get_data()[y * _alpha_testing * _width +
                                        x * _alpha_testing];
  ...
  void drawCursor(double x, double y, double size, double spins_per_sec = 0.3) {
    ...
    _cairoContext->rotate(2 * M_PI * spins_per_sec * time /
  ...
  void drawTextBox(double x, double y, std::string text, double padding = 5,
                   float bgR = 1.0, float bgG = 1.0, float bgB = 1.0,
                   float bgA = 0.7, float fgR = 0.0, float fgG = 0.0,
                   float fgB = 0.0, float fgA = 1.0) {
    _cairoContext->save();
    ...
    bottomright[0] =
        topleft[0] + double(pango_width) / Pango::SCALE + 2 * padding;
    bottomright[1] =
        topleft[1] + double(pango_height) / Pango::SCALE + 2 * padding;
