...
public:
  virtual std::string initVertexShaderSource() {
    return STRINGIFY(
        ...
        flat out vec4 color; smooth out vec3 normal; smooth out vec3 position;
        ...
        void main() {
          ...
          normal =
              (ViewMatrix * vec4(qrot(iOrientation, vNormal.xyz), 0.0)).xyz;

          vec3 scale =
              iScale.xyz + vec3(equal(iScale.xyz, vec3(0.0))) * iScale.x;
          vec4 vVertex =
              ViewMatrix *
              vec4(qrot(iOrientation, vPosition.xyz * scale) + iOrigin.xyz,
                   1.0);
  ...
  virtual std::string initFragmentShaderSource() {
    return STRINGIFY(flat in vec4 color; smooth in vec3 normal;
...
public:
  virtual std::string initFragmentShaderSource() {
    return STRINGIFY(uniform mat4 ProjectionMatrix; smooth in vec3 position;
