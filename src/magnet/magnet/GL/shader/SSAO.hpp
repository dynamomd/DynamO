...
public:
  virtual std::string initFragmentShaderSource() {
    return STRINGIFY(
        uniform sampler2DMS NormalsTex; uniform sampler2DMS EyePosTex;
        uniform sampler2D rnm; uniform float radius; uniform float totStrength;
        uniform float depthDropoff; uniform float offset;
        ...
        void main(void) {
          ...
          vec3 pSphere[10] =
              vec3[10](vec3(-0.010735935, 0.01647018, 0.0062425877),
          ...
          vec3 fres = normalize(2.0 * texture(rnm, screenCoord * offset).xyz -
                                vec3(1.0));

          ivec2 pixelcoord = ivec2(textureSize(EyePosTex) * screenCoord);
          vec3 currentPixelPos = texelFetch(EyePosTex, pixelcoord, 0).xyz;
          vec3 currentPixelNorm =
              normalize(2.0 * texelFetch(NormalsTex, pixelcoord, 0).xyz - 1.0);
          ...
          for (int i = 0; i < 10; ++i) {
            ...
            vec3 sampleEyeSpaceLocation =
                currentPixelPos + sign(dot(ray, currentPixelNorm)) * ray;

            // Convert it to clip space
            vec4 sampleLocation =
                ProjectionMatrix *
                (ViewMatrix * vec4(sampleEyeSpaceLocation, 1.0));
            ...
            ivec2 samplecoord =
                ivec2(textureSize(EyePosTex) * sampleLocation.xy);
            vec3 occluderLocation = texelFetch(EyePosTex, samplecoord, 0).xyz;
            vec3 occluderNormal = normalize(
                texelFetch(NormalsTex, samplecoord, 0).xyz * 2.0 - vec3(1.0));
            ...
            // occlusion += max(0.0, 0.9 - occluderDot) * step(0.0, zdiff) *
            // step(0.0, depthDropoff - zdiff);
          }

          float val =
              clamp(1.0 - totStrength * occlusion * invSamples, 0.0, 1.0);
