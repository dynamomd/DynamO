...
public:
  virtual std::string initFragmentShaderSource() {
    return STRINGIFY(
        ...
        uniform sampler2DMS colorTex; uniform sampler2DMS normalTex;
        uniform sampler2DMS positionTex; uniform vec3 lightPosition;
        uniform vec3 lightColor; uniform float lightSpecularExponent;
        uniform float lightSpecularFactor; uniform int samples;
        ...
        vec3 calcLighting(vec3 position, vec3 normal, vec3 diffuseColor) {
          ...
          return decay_factor * lightColor *
        ...
        void main() {
          ...
          for (int sample_id = 0; sample_id < samples; sample_id++) {
            vec4 color =
                texelFetch(colorTex, ivec2(gl_FragCoord.xy), sample_id).rgba;
            ...
            if (color.a != 0) {
              ...
              vec3 normal =
                  texelFetch(normalTex, ivec2(gl_FragCoord.xy), sample_id).rgb;
              vec3 position =
                  texelFetch(positionTex, ivec2(gl_FragCoord.xy), sample_id)
                      .xyz;
...
public:
  virtual std::string initFragmentShaderSource() {
    return STRINGIFY(
        ...
        // Standard G-buffer data
        uniform sampler2DMS colorTex; uniform sampler2DMS normalTex;
        uniform sampler2DMS positionTex; uniform sampler2D shadowTex;
        uniform mat4 shadowMatrix; uniform vec3 lightPosition;
        uniform vec3 lightColor; uniform float lightSpecularExponent;
        uniform float lightSpecularFactor; uniform float maxVariance;
        ...
        void main() {
          ...
          shadow *= chebyshevUpperBound(
              texture(shadowTex, ShadowCoord.xy / ShadowCoord.w).rg,
              lightdistance);
          // Now calculate the color from the samples
          vec4 color_sum = vec4(0.0);
          for (int sample_id = 0; sample_id < samples; sample_id++) {
            ...
            if (color.a != 0) {
              ...
              vec3 position =
                  texelFetch(positionTex, ivec2(gl_FragCoord.xy), sample_id)
                      .xyz;
              color_sum.rgb +=
                  shadow * calcLighting(position, normal, color.rgb);
              color_sum.a += 1.0;
            }
          }
