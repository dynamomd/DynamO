...
public:
   ...
   */
  virtual std::string initVertexShaderSource() { return "void main() {}"; }
  ...
  virtual std::string initGeometryShaderSource() {
    return STRINGIFY(layout(points) in; layout(triangle_strip) out;
                     ...
                     void main() {
                       /*Here we draw a fullscreen triangle and allow the GPU to
                         scissor to the screen. This prevents the difficult
                         interpolation of the vertex property (screenCoord) on
                         the diagonal of a fullscreen quad. This is a ridiculous
                         optimisation I know. */
