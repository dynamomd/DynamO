...
public:
  ...
  template <class T> bool operator==(const T &val) const {
    return ((!_data.empty()) && (typeid(T) == _data.type()) &&
  ...
  template <class T> const T as() {
    if (_data.empty())
      M_throw()
          << "Uniform hasn't been assigned yet! Cannot retrieve its value";
  ...
  inline void operator=(const math::Vector &vec) {
    if (test_assign(vec)) {
      std::array<GLfloat, 3> val = {
          {GLfloat(vec[0]), GLfloat(vec[1]), GLfloat(vec[2])}};
      uniform(3, 1, &(val[0]));
    }
  }
...
private:
   ...
   */
  template <class T> bool test_assign(const T &val) {
...
public:
  ...
  /*! \brief Used to set and retrieve values of \ref Shader
    ...
    A["vec3ShaderVariable"].as<std::array<GLfloat, 3> >();
    \endcode
  ...
  inline ShaderUniformValue &operator[](const std::string &uniformName) {
    if (!_built)
      M_throw()
          << "Cannot set the uniforms of a shader which has not been built()";

    if (_context->_shaderStack.back() != this)
      M_throw() << "You must attach() a shader before you can change/read its "
                   "uniform's values";
    ...
    GLint uniformHandle =
        glGetUniformLocationARB(_programHandle, uniformName.c_str());
    GL::detail::errorCheck();
#ifdef MAGNET_DEBUG
    if (uniformHandle == -1)
  ...
  */
  inline void build() {
    ...
    if (_fragmentShaderCode.empty())
      _fragmentShaderCode =
          magnet::string::format_code(initFragmentShaderSource());
    if (_geometryShaderCode.empty())
      _geometryShaderCode =
          magnet::string::format_code(initGeometryShaderSource());
    ...
    if (!_vertexShaderCode.empty()) {
      ...
      GL::detail::errorCheck();
      glGetObjectParameterivARB(_vertexShaderHandle,
                                GL_OBJECT_COMPILE_STATUS_ARB, &result);
      GL::detail::errorCheck();
      if (!result)
        M_throw() << "Vertex shader compilation failed, build log follows\n"
                  << getShaderBuildlog(_vertexShaderHandle)
                  << "\n Source code:\n"
                  << magnet::string::add_line_numbers(defines +
                                                      _vertexShaderCode)
                  << "\n";
    ...
    if (!_fragmentShaderCode.empty()) {
      ...
      GL::detail::errorCheck();
      glGetObjectParameterivARB(_fragmentShaderHandle,
                                GL_OBJECT_COMPILE_STATUS_ARB, &result);
      GL::detail::errorCheck();
      if (!result)
    ...
    if (!(_geometryShaderCode.empty())) {
      ...
      GL::detail::errorCheck();
      glGetObjectParameterivARB(_geometryShaderHandle,
                                GL_OBJECT_COMPILE_STATUS_ARB, &result);
      GL::detail::errorCheck();
      if (!result)
    ...
    if (!_tfVaryings.empty()) {
      ...
      glTransformFeedbackVaryings(_programHandle, _varyingcstrings.size(),
                                  &_varyingcstrings[0], GL_INTERLEAVED_ATTRIBS);
    }
  ...
  const std::string getVertexShaderSource() const { return _vertexShaderCode; }
...
protected:
  ...
  inline std::string getShaderBuildlog(GLhandleARB shaderHandle) {
    ...
    glGetObjectParameterivARB(shaderHandle, GL_OBJECT_INFO_LOG_LENGTH_ARB,
                              &errorLoglength);
    GL::detail::errorCheck();
    char *buffer = new char[errorLoglength];

    GLsizei actualErrorLogLength;
    glGetInfoLogARB(shaderHandle, errorLoglength, &actualErrorLogLength,
                    buffer);
    GL::detail::errorCheck();
