...
public:
   ...
   */
  inline void init(const std::vector<T> &data, size_t components,
                   buffer_usage::Enum usage = buffer_usage::DYNAMIC_DRAW) {
   ...
   */
  inline void init(size_t size, size_t components, const T *ptr = NULL,
                   buffer_usage::Enum usage = buffer_usage::DYNAMIC_DRAW) {
    if (size == 0)
    ...
    if (size % components)
      M_throw()
          << "Can't initialise buffer without a whole number of elements.";
  ...
  inline void deinit() {
    ...
    if (_cl_buffer_acquired)
      M_throw()
          << "Deinitialising a buffer which is acquired by the OpenCL system!";
#endif
  ...
  inline void drawInstancedElements(element_type::Enum type, size_t instances) {
    ...
    if (GLEW_EXT_draw_instanced)
      glDrawElementsInstancedEXT(
          type, size(), detail::c_type_to_gl_enum<T>::val, 0, instances);
    else if (GLEW_ARB_draw_instanced)
      glDrawElementsInstancedEXT(
          type, size(), detail::c_type_to_gl_enum<T>::val, 0, instances);
    else
      M_throw() << "Cannot perform instanced drawing, "
                   "GL_ARB_draw_instanced/GL_EXT_draw_instanced extensions are "
                   "missing.";
  }
  ...
  */
  ...
  /*! \brief Attaches the buffer to a vertex attribute pointer
    state.
   */
  inline void attachToAttribute(size_t attrnum, size_t divisor = 0,
                                bool normalise = false) {
    initTest();
    bind(buffer_targets::ARRAY);
    glVertexAttribPointer(
        attrnum, _components, detail::c_type_to_gl_enum<T>::val,
  ...
  inline const ::cl::BufferGL &acquireCLObject() {
    ...
    if (!_cl_handle_init) {
      ...
      _cl_handle = ::cl::BufferGL(_context->getCLContext(), CL_MEM_READ_WRITE,
                                  getGLObject());
    }
