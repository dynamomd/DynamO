...
public:
  ...
  /*! \brief Initializes the FBO.
    ...
    must attach at least one texture using \ref attachColorTexture() or \ref
    \attachDepthTexture() before attempting to \ref attach() this FBO.
  */
  ...
  inline void blitToScreen(GLsizei screenwidth, GLsizei screenheight,
    ...
    glBlitFramebuffer(0, 0, getWidth(), getHeight(), screenx, screeny,
                      screenwidth + screenx, screenheight + screeny, mask,
                      filter);
    detail::errorCheck();
   ...
   */
  inline virtual void copyto(FBO &other,
                             GLbitfield opts = GL_COLOR_BUFFER_BIT |
                                               GL_DEPTH_BUFFER_BIT) {
    validate();
    other.validate();
    ...
    glBlitFramebuffer(0, 0, getWidth(), getHeight(), 0, 0, other.getWidth(),
                      other.getHeight(), opts, GL_NEAREST);
    detail::errorCheck();
  ...
  inline std::shared_ptr<Texture2D> &getColorTexture(const size_t ID = 0) {
    ...
    if (!_colorTextures[ID])
      M_throw() << "Cannot fetch the color texture " << ID
                << " as the FBO has none bound";
  ...
  inline GLsizei getWidth() {
    ...
    if (_depthTexture)
      return _depthTexture->getWidth();

    for (auto tex_ptr : _colorTextures)
      if (tex_ptr)
        return tex_ptr->getWidth();
  ...
  inline GLsizei getHeight() {
    ...
    // Find the first bound texture and return its dimensions
    if (_depthTexture)
      return _depthTexture->getHeight();

    for (auto tex_ptr : _colorTextures)
      if (tex_ptr)
        return tex_ptr->getHeight();
  ...
  void checkStatus() {
    ...
    case GL_FRAMEBUFFER_UNDEFINED:
      M_throw()
          << "Failed to create FrameBufferObject: GL_FRAMEBUFFER_UNDEFINED";
    case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT:
      M_throw() << "Failed to create FrameBufferObject: "
    ...
    case GL_FRAMEBUFFER_UNSUPPORTED:
      M_throw()
          << "Failed to create FrameBufferObject: GL_FRAMEBUFFER_UNSUPPORTED";
    ...
    default:
      M_throw() << "Failed to create FrameBufferObject: Unknown error code = "
                << FBOstatus << " ";
...
protected:
  /*! \brief Validate the current state of the FBO and raise an exception if
   * there is an error.
   */
  virtual void validate() {
    ...
    if (!_validated) {
      ...
      // Bind the textures, or unbind the unbound textures ready for the
      // completeness test
      if (_depthTexture) {
        ...
        if ((_depthTexture->getInternalFormat() == GL_DEPTH24_STENCIL8) ||
          ...
          glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT,
                                 _depthTexture->getGLType(),
                                 _depthTexture->getGLHandle(), 0);
          detail::errorCheck();
        } else {
      ...
      for (size_t attachment(0); attachment < _colorTextures.size();
           ++attachment)
        if (_colorTextures[attachment]) {
          if (!width) {
