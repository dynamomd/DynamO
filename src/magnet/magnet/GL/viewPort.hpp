...
namespace magnet {
  namespace GL {
	...
	_viewMatrix
	  = Matrix::rotate(_tiltrotation, Vector(1,0,0))
	  * Matrix::rotate(_panrotation, Vector(0,1,0))
	  * Matrix::translate(-cameraLocation);
	...
	glGetFloatv(GL_MODELVIEW_MATRIX, &_viewMatrix[0]);
      }
	...
	glLoadMatrixf(&_viewMatrix[0]);
       ...
       */
      inline const Matrix getViewMatrix() const { return _viewMatrix; }
