...
class Camera {
public:
  inline Camera(GLfloat zNearDist = 0.3f, GLfloat zFarDist = 300.0f)
      : _height(1), _width(1), _zNearDist(zNearDist), _zFarDist(zFarDist) {
  ...
  virtual void setUp(math::Vector newup,
                     math::Vector axis = math::Vector{0, 0, 0}) = 0;
  ...
  virtual FBO &getResolveBuffer() { return _renderTarget; }
  ...
  virtual void resize(size_t width, size_t height, size_t samples) {
...

    {
      std::shared_ptr<magnet::GL::Texture2D> colorTexture(
          new magnet::GL::Texture2DMultisampled(samples));
      colorTexture->init(width, height, GL_RGBA16F_ARB);

      std::shared_ptr<magnet::GL::Texture2D> normalTexture(
          new magnet::GL::Texture2DMultisampled(samples));
      normalTexture->init(width, height, GL_RGBA16F_ARB);

      std::shared_ptr<magnet::GL::Texture2D> posTexture(
          new magnet::GL::Texture2DMultisampled(samples));
  ...
  GLMatrix getViewRotationMatrix() const {
    ...
    return magnet::GL::promoteToGLMatrix(
        magnet::GL::demoteToMatrix(getViewMatrix()));
  }
  ...
  inline GLfloat getAspectRatio() const { return ((GLfloat)_width) / _height; }
  ...
  magnet::math::NVector<GLfloat, 4> project(math::Vector invec) const {
    magnet::math::NVector<GLfloat, 4> vec = {
        GLfloat(invec[0]), GLfloat(invec[1]), GLfloat(invec[2]), 1.0f};
   ...
   */
  math::Vector unprojectToPosition(int windowx, int windowy,
                                   GLfloat depth) const {
    // We need to calculate the ray from the camera
    magnet::math::NVector<GLfloat, 4> n = {
        (2.0f * windowx) / getWidth() - 1.0f,
        1.0f - (2.0f * windowy) / getHeight(), depth, 1.0f};
  ...
  math::Vector unprojectToDirection(int windowx, int windowy) const {
    ...
    magnet::math::NVector<GLfloat, 4> n = {
        (2.0f * windowx) / getWidth() - 1.0f,
        1.0f - (2.0f * windowy) / getHeight(), 0.0f, 1.0f};
    // Unproject from NDC to camera coords
    magnet::math::NVector<GLfloat, 4> v = inverse(getProjectionMatrix()) * n;
...
public:
  ...
  enum Camera_Mode { ROTATE_CAMERA, ROTATE_POINT };

  /*! \brief The constructor.

    \param height The height of the viewport, in pixels.
    \param width The width of the viewport, in pixels.
    \param position The position of the screen (effectively the camera), in
   ...
   */
  // We need a default constructor as viewPorts may be created without GL being
  // initialized
  inline CameraHeadTracking(math::Vector position = math::Vector{0, 0, 5},
      ...
      : Camera(zNearDist, zFarDist), _up(up.normal()),
        _nearPlanePosition({0, 0, 0}), _rotatePoint({0, 0, 0}),
        _rotation(math::Quaternion::identity()), _simLength(simLength),
  ...
  inline void lookAt(math::Vector lookAtPoint) {
    ...
    // move.
    _rotation = math::Quaternion::fromToVector(
                    right, _rotation * math::Vector{1, 0, 0}) *
                _rotation;
  ...
  virtual inline GLMatrix getViewMatrix() const {
    ...
    // camera
    math::Vector cameraLocation =
        (_rotation.inverse().toMatrix() * _eyeLocation) / _simLength +
        _nearPlanePosition;
  ...
  /*! \brief Converts some inputted motion (e.g., by the mouse or keyboard) into
    ...
    (\ref _camMode).
   */
  inline void movement(float rotationX, float rotationY, float forwards,
                       float sideways, float upwards) {
    forwards /= _simLength;
    sideways /= _simLength;
    upwards /= _simLength;
    ...
    case ROTATE_CAMERA: {
      // Move the camera
      math::Vector newpos =
          getPosition() + up * upwards + right * sideways + at * forwards;
      // Rotate the view
      math::Vector direction =
          math::Quaternion::fromAngleAxis(rotationY / 180.0, right) *
          math::Quaternion::fromAngleAxis(rotationX / 180.0, up) * at;
    ...
    case ROTATE_POINT: {
      ...
      offset =
          math::Quaternion::fromAngleAxis(-M_PI * rotationX / 180.0f, _up) *
          offset;

      math::Vector rotationAxis = up ^ offset;
      rotationAxis.normalise();
      offset = math::Quaternion::fromAngleAxis(-M_PI * rotationY / 180.0f,
                                               rotationAxis) *
               offset;
      offset.normalise();
  ...
  inline void setPosition(math::Vector newposition) {
    _nearPlanePosition =
        newposition - (getNormalMatrix() * _eyeLocation / _simLength);
  }
   ...
   */
  inline void setEyeLocation(math::Vector eye) { _eyeLocation = eye; }
  ...
  virtual inline GLMatrix getProjectionMatrix() const {
    ...
    //_zNearDist)!
    //
    return frustrum((-0.5f * getScreenPlaneWidth() - _eyeLocation[0]) *
                        _zNearDist / _eyeLocation[2], // left
                    (+0.5f * getScreenPlaneWidth() - _eyeLocation[0]) *
                        _zNearDist / _eyeLocation[2], // right
                    (-0.5f * getScreenPlaneHeight() - _eyeLocation[1]) *
                        _zNearDist / _eyeLocation[2], // bottom
                    (+0.5f * getScreenPlaneHeight() - _eyeLocation[1]) *
                        _zNearDist / _eyeLocation[2], // top
   ...
   */
  virtual void setUp(math::Vector newup,
                     math::Vector axis = math::Vector{0, 0, 0}) {
    newup.normalise();
    if (axis.nrm2() != 0) {
      axis.normalise();
      math::Vector to = newup - axis * (axis | newup);
      math::Vector from = _up - axis * (axis | _up);
      setPosition(math::Quaternion::fromToVector(to.normal(), from.normal()) *
                  getPosition());
    }
    _up = newup;
    movement(0, 0, 0, 0, 0);
