...
public:
  template <RetType (*Fun_addr)(Args...)> static inline Delegate create() {
    Delegate d;
    d._this_ptr = 0;
  ...
  static inline Delegate create(T *obj) {
    Delegate d;
    d._this_ptr = obj;
    d._shunt_ptr = [](void *obj, Args... args) {
      return (static_cast<T *>(obj)->*Mem_fun_addr)(args...);
    };
    return d;
  }
  ...
  static inline Delegate create(const T *obj) {
    ...
    d._shunt_ptr = [](void *obj, Args... args) {
      return (static_cast<const T *>(obj)->*Mem_fun_addr)(args...);
    };
    return d;
  }

  RetType operator()(Args... arguments) const {
    return (*_shunt_ptr)(_this_ptr, arguments...);
  }

  bool operator==(const Delegate &od) const {
    return (_this_ptr == od._this_ptr) && (_shunt_ptr == od._shunt_ptr);
  }
  bool operator!=(const Delegate &od) const { return !operator==(od); }

  operator DelegateKey() const {
    static_assert(sizeof(std::size_t) == sizeof(void *),
                  "Error, object pointers are not the same size as size_t!");
    static_assert(
        sizeof(std::size_t) == sizeof(RetType(*)(void *, Args...)),
        "Error, global function pointers are not the same size as size_t!");
...
struct Tracked {
  std::unordered_map<DelegateKey, Tracked *, detail::Delegate_hash>
...
class Signal<Ret_t(Args...)> : Tracked {
...
public:
  template <Ret_t (*Fun_addr)(Args...)> void connect() {
  ...
  template <Ret_t (*Fun_addr)(Args...)> void disconnect() {
    Delegate_t delegate = Delegate_t::template create<Fun_addr>();
