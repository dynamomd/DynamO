...
namespace magnet {
namespace CL {
template <class T> class radixSortNVIDIA : public detail::Program {
  cl::Kernel _radixSortKernel, _findRadixOffsetsKernel, _reorderKeysKernel,
      _radixSortDataKernel, _reorderKeysDataKernel;
...
public:
  radixSortNVIDIA() : _lastSize(0), _lastDataSize(0) {}
  ...
  void operator()(cl::Buffer keyInput, cl::Buffer keyOutput,
    ...
    if (size % 1024)
      M_throw() << "Radix sort works on whole multiples of 1024 elements only, "
                   "please pad your data";

    cl::KernelFunctor _blockSortFunc = _radixSortKernel.bind(
        _queue, cl::NDRange(size / 4), cl::NDRange(groupSize));

    cl::KernelFunctor _findRadixOffsetsFunc = _findRadixOffsetsKernel.bind(
        _queue, cl::NDRange(size / 4), cl::NDRange(groupSize));

    cl::KernelFunctor _ReorderFunc = _reorderKeysKernel.bind(
        _queue, cl::NDRange(size / 4), cl::NDRange(groupSize));

    // Create the buffer holding the bit block offsets
    if (_lastSize != size) {
      _buckets = cl::Buffer(_context, CL_MEM_READ_WRITE,
                            sizeof(cl_uint) * nWorkGroups * maxRadixDigit);

      _offsets = cl::Buffer(_context, CL_MEM_READ_WRITE,
                            sizeof(cl_uint) * nWorkGroups * maxRadixDigit);
    ...
    for (cl_uint startBit = 0; startBit < bits_to_sort;
      ...
      _findRadixOffsetsFunc(_doubleBuffer, _buckets, _offsets, size, startBit,
                            bitsPerPass,
      ...
      _ReorderFunc(_doubleBuffer, keyOutput, _buckets, _offsets, size, startBit,
                   bitsPerPass, cl::__local(sizeof(cl_uint) * maxRadixDigit),
                   cl::__local(sizeof(cl_uint) * maxRadixDigit));
    }
  ...
  void operator()(cl::Buffer keyInput, cl::Buffer dataInput,
    ...
    cl::KernelFunctor _blockSortDataFunc = _radixSortDataKernel.bind(
        _queue, cl::NDRange(size / 4), cl::NDRange(groupSize));

    cl::KernelFunctor _findRadixOffsetsFunc = _findRadixOffsetsKernel.bind(
        _queue, cl::NDRange(size / 4), cl::NDRange(groupSize));

    cl::KernelFunctor _reorderDataFunc = _reorderKeysDataKernel.bind(
        _queue, cl::NDRange(size / 4), cl::NDRange(groupSize));
    ...
    for (cl_uint startBit = 0; startBit < bits_to_sort;
         startBit += bitsPerPass) {
      _blockSortDataFunc(keyInput, dataInput, _doubleBuffer, _dataDoubleBuffer,
      ...
      _reorderDataFunc(_doubleBuffer, _dataDoubleBuffer, keyOutput, dataOutput,
                       _buckets, _offsets, size, startBit, bitsPerPass,
                       cl::__local(sizeof(cl_uint) * maxRadixDigit),
                       cl::__local(sizeof(cl_uint) * maxRadixDigit));
    }
  ...
  virtual std::string initKernelSrc() {
    return "#pragma OPENCL EXTENSION cl_khr_byte_addressable_store : enable\n" +
           scan<cl_uint>().initKernelSrc() + "\n#define keyType4 " +
           detail::traits<typename detail::traits<typename detail::traits<
               T>::bitshiftable_type>::vec4_type>::kernel_type() +
           "\n#define keyType " +
           detail::traits<
           ...
           "\n" STRINGIFY(
               ...
               ////////////          Radix Sort Kernels /////////////
               //////////////////////////////////////////////////////////////////////
               // This function will perform a single bit radix sort on a block
               // of data in local memory
               void radixBlockSort(keyType4 * localkey, uint shift,
                                   uint startBit, __local keyType * share,
                 ...
                 // We need to know the total number of true statements to
                 // calculate offsets for them If something is true, it should
                 ...
                 sum4.x = (test4.x) ? totalFalse + sum4.x - 1
                                    : 4 * get_local_id(0) + 0 - sum4.x;

                 sum4.y = (test4.y) ? totalFalse + sum4.y - 1
                                    : 4 * get_local_id(0) + 1 - sum4.y;

                 sum4.z = (test4.z) ? totalFalse + sum4.z - 1
                                    : 4 * get_local_id(0) + 2 - sum4.z;

                 sum4.w = (test4.w) ? totalFalse + sum4.w - 1
                                    : 4 * get_local_id(0) + 3 - sum4.w;

                 // Avoid bank conflicts on the read by interleaving the uint4
                 // data
               ...
               // Just a wrapper kernel function around radixBlockSort,
               // allocates local memory and loads and stores to global memory
               __kernel __attribute__((reqd_work_group_size(256, 1, 1))) void
                   radixBlockSortKernel(__global keyType4 *const keyData,
                                        __global keyType4 *outkeyData,
                                        uint array_size, uint startBit,
                                        uint nBits) {
                     ...
                     // Loop over the bits we will sort the block over, and
                     // store the totals in the localBuckets array
                     for (uint shift = startBit; shift < (startBit + nBits);
                          ++shift)
               ...
               void radixBlockSortData(
                   keyType4 * localkey, uint4 * localdata, uint shift,
                   uint startBit, __local keyType * share,
                   __local uint * datashare, __local uint * totalTrue) {
                 // First, perform a scan over our little block of 4 items
                 uint4 test4;
               ...
               // allocates local memory and loads and stores to global memory
               __kernel __attribute__((reqd_work_group_size(256, 1, 1))) void
                   radixBlockSortDataKernel(__global keyType4 *const keyData,
                     ...
                     for (uint shift = startBit; shift < (startBit + nBits);
                          ++shift)
                       radixBlockSortData(&key, &data, shift, startBit, share,
                                          datashare, &totalTrue);
               ...
               /////////////////////////////////////////////////////////////////////////////////////////////////////
               /////////////////////////////////////////////////////////////////////////////////////////////////////
               // This kernel takes sorted blocks, offsets, and sizes generated
               // by radixBlockSortKernel. It reads and scatters the blocks of
               // data to the appropriate locations in ram
               __kernel __attribute__((reqd_work_group_size(256, 1, 1))) void
                   reorderKeys(__global const keyType4 *keyData,
                               __global keyType *outkey, __global uint *buckets,
                               __global uint *offsets, uint array_size,
                               uint startBit, uint nBits,
                               __local uint *localOffsets,
                               __local uint *globalOffsets) {
                     ...
                     if (get_local_id(0) < maxRadixDigit) {
                       globalOffsets[get_local_id(0)] =
                           buckets[get_local_id(0) * get_num_groups(0) +
                                   get_group_id(0)];
                       localOffsets[get_local_id(0)] =
                           offsets[get_group_id(0) * maxRadixDigit +
                                   get_local_id(0)];
                     }
                     ...
                     for (uint i = 0; i < 4; ++i) {
                       uint radix =
                           (keys[get_local_id(0) + i * 256] >> startBit) & mask;
                       uint globalOffset = globalOffsets[radix] +
               ...
               __kernel __attribute__((reqd_work_group_size(256, 1, 1))) void
                   reorderKeysData(
                       __global const keyType4 *keyData,
                       __global const uint4 *dataData, __global keyType *outkey,
                       __global uint *outdata, __global uint *buckets,
                       __global uint *offsets, uint array_size, uint startBit,
                       uint nBits, __local uint *localOffsets,
                       __local uint *globalOffsets) {
                     __local keyType4 keyBlocks[256];
                     __local keyType *keys = (__local keyType *)keyBlocks;
               ...
               void findRadixOffsets(
                 ...
                 // We check the first element against the previous key in the
                 // shared array
                 if ((get_local_id(0) > 0) &&
                     (block[get_local_id(0)] != block[get_local_id(0) - 1]))
                   radixOffsets[block[get_local_id(0)]] = get_local_id(0);

                 for (size_t i = 1; i < 4; ++i)
                   if (block[get_local_id(0) + i * 256] !=
                       block[get_local_id(0) + i * 256 - 1])
                     radixOffsets[block[get_local_id(0) + i * 256]] =
                         get_local_id(0) + i * 256;
                 ...
                 if (get_local_id(0) < maxRadixDigit)
                   buckets[get_local_id(0) * get_num_groups(0) +
                           get_group_id(0)] = radixOffsets[get_local_id(0)];
               }

               __kernel __attribute__((reqd_work_group_size(256, 1, 1))) void
                   findRadixOffsetsKernel(
                       __global keyType4 * keyData, __global uint * buckets,
                       __global uint * offsets, uint array_size, uint startBit,
                       uint nBits, __local uint * radixOffsets) {
                     ...
                     keyradix[get_local_id(0)].x =
                         (localKey.x >> startBit) & mask;
                     keyradix[get_local_id(0)].y =
                         (localKey.y >> startBit) & mask;
                     keyradix[get_local_id(0)].z =
                         (localKey.z >> startBit) & mask;
                     keyradix[get_local_id(0)].w =
                         (localKey.w >> startBit) & mask;
                     ...
                     // With all the sorted keys in local memory we can do the
                     // offset and bucket count/scan
                     findRadixOffsets(buckets, offsets,
                                      (__local const keyType *)keyradix,
