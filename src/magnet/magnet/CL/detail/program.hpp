...
public:
   ...
   */
  inline void build(::cl::CommandQueue queue, cl::Context context,
    ...
    sources.push_back(std::make_pair(extensions.c_str(), extensions.size()));

    // Now load the kernel source
    if (_kernelSrc.empty())
      _kernelSrc = magnet::string::format_code(initKernelSrc());

    sources.push_back(std::make_pair(_kernelSrc.c_str(), _kernelSrc.size()));
    ...
    try {
      ...
      for (size_t dev = 0; dev < devices.size(); ++dev) {
        std::string msg =
            _program.getBuildInfo<CL_PROGRAM_BUILD_LOG>(devices[dev]);
