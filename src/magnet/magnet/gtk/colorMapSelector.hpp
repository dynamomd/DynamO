...
public:
  ...
  ColorMapSelector() : _min(0), _invRange(1) {
    ...
    _comboBox.signal_changed().connect(
        sigc::mem_fun(*this, &ColorMapSelector::on_combobox_changed));
    ...
    _minValue.show();
    _minValue.signal_changed().connect(
        sigc::mem_fun(*this, &ColorMapSelector::on_limits_changed));

    _maxValue.set_text("1.0");
    _maxValue.set_width_chars(5);
    _maxValue.show();
    _maxValue.signal_changed().connect(
        sigc::mem_fun(*this, &ColorMapSelector::on_limits_changed));

    Gtk::Label *label = Gtk::manage(new Gtk::Label("Range"));
    label->show();
    ...
    label = Gtk::manage(new Gtk::Label("Scale"));
    label->show();
...
protected:
  ...
  void buildEntry(Mode_t newMode, std::string name) {
    ...
    row[m_Columns.m_col_icon] = Glib::RefPtr<Gdk::Pixbuf>(
        Gdk::Pixbuf::create(Gdk::COLORSPACE_RGB, true, 8, width, height));

    guint8 *pixels =
        (static_cast<Glib::RefPtr<Gdk::Pixbuf>>(row[m_Columns.m_col_icon]))
            ->get_pixels();

    for (size_t column(0); column < width; ++column) {
      map(reinterpret_cast<uint8_t *>(pixels + 4 * column),
          (float)(column) / width);
      for (size_t row(1); row < height; ++row)
        (int32_t &)(pixels[4 * (column + width * row)]) =
            (int32_t &)(pixels[4 * column]);
    }
  }
  ...
  void on_limits_changed() {
    ...
    try {
      _min = boost::lexical_cast<GLfloat>(_minValue.get_text());
    } catch (...) {
    }
