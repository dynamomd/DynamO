...
struct PolynomialFunction : public std::array<double, Order + 1> {
  ...
  // Allow demotion to a lower order polynomial if the highest coefficient is
  // zero
  PolynomialFunction<Order - 1> lowerOrder() const {
#ifdef MAGNET_DEBUG
    if ((*this)[Order] != 0)
      M_throw() << "Trying to reduce the order of a polynomial with non-zero "
  ...
  double nextEvent() const { return nextEvent(*this); }
...
inline double nextEvent(const PolynomialFunction<2> &f) {
  if (f[2] == 0)
    return nextEvent(f.lowerOrder());
  const double arg = f[1] * f[1] - 2 * f[2] * f[0];
  if (f[2] < 0) {
    if (arg <= 0)
      return std::max(0.0, -f[1] / f[2]);
    if (f[1] > 0)
...
inline double nextEvent(const PolynomialFunction<3> &f) {
  ...
  const size_t nroots =
      magnet::math::cubicSolve(6 * f[2] / (2 * f[3]), 6 * f[1] / f[3],
                               6 * f[0] / f[3], roots[0], roots[1], roots[2]);
  ...
  const size_t nderivroots =
      magnet::math::quadSolve(f[3], f[2], f[1], derivroots[0], derivroots[1]);
  ...
  if (f[3] > 0) {
    ...
    if (derivroots[1] < 0)
      return HUGE_VAL;
    if ((nroots == 1) && (roots[0] < derivroots[0]))
      return HUGE_VAL;
...
inline double nextEvent(const PolynomialFunction<4> &f, double f0char,
  ...
  const size_t rootCount =
      magnet::math::cubicSolve(3 * f[3] / f[4], 6 * f[2] / f[4],
                               6 * f[1] / f[4], roots[0], roots[1], roots[2]);
  ...
  if (f[4] > 0) {
    ...
    if (f.eval(tlast).front() <= 0)
      return std::max(0.0, tlast);

    if ((tlast < 0) && (f.eval(0).front() <= 0))
      return 0;
