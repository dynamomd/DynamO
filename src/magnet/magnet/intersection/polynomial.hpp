...
namespace magnet {
  namespace intersection {
	...
	return std::max(0, - f0 / f1);
      }

      inline double secondOrder(const double f0, const double f1, const double f2) {
	if (f2 == 0) return firstOrder(f0, f1);
	
	double arg = f1 * f1 - f2 * f0;
	if (f2 < 0) {
	  if (arg <= 0) return std::max(0, -f1 / f2);
	  if (f1 > 0)
	    return std::max(0, (-f1 - std::sqrt(arg)) / f2);
	  else
	    return std::max(0, f0 / (-f1 + std::sqrt(arg)));
	} else {//f2 > 0
      ...
      {
	if (f.eval<0>(0) > 0) return f.nextRoot<0, true>(0);
        if (f.eval<1>(0) < 0) return 0;
	const double tderiv = f.nextRoot<1, true>(0);
	if (tderiv == HUGE_VAL) return HUGE_VAL;
	if (f.eval<0>(tderiv) > 0) return f.nextRoot<0, true>(tderiv);
      }
    }
  }
}
