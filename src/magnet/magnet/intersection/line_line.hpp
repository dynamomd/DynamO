...
public:
  LinesOverlapFunc(const math::Vector &nr12, const math::Vector &nv12,
                   ...
                   const double &length)
      : w1(nw1), w2(nw2), q1(nq1), q2(nq2), w12(nw1 - nw2), r12(nr12),
        v12(nv12), _length(length) {
  ...
  std::pair<double, double> getCollisionPoints() const {
    ...
    return std::make_pair(
        -(rijdotui - (rijdotuj * uidotuj)) / (1.0 - uidotuj * uidotuj),
  ...
  template <size_t deriv> double eval() const {
    ...
    case 0:
      return ((u1 ^ u2) | r12);
    case 1:
      return ((u1 | r12) * (w12 | u2)) + ((u2 | r12) * (w12 | u1)) -
             ((w12 | r12) * (u1 | u2)) + (((u1 ^ u2) | v12));
    case 2:
      return 2.0 * (((u1 | v12) * (w12 | u2)) + ((u2 | v12) * (w12 | u1)) -
                    ((u1 | u2) * (w12 | v12))) -
             ((w12 | r12) * (w12 | (u1 ^ u2))) +
             ((u1 | r12) * (u2 | (w1 ^ w2))) + ((u2 | r12) * (u1 | (w1 ^ w2))) +
             ((w12 | u1) * (r12 | (w2 ^ u2))) +
             ((w12 | u2) * (r12 | (w1 ^ u1)));
    default:
  ...
  template <size_t deriv> double max() const {
    ...
    case 2:
      return w12.nrm() * ((2 * v12.nrm()) + (_length * (w1.nrm() + w2.nrm())));
    default:
      M_throw() << "Invalid access";
    }
  }
  ...
  std::pair<double, double> discIntersectionWindow() const {
    ...
    if (retVal.second < retVal.first)
      std::swap(retVal.first, retVal.second);
...
line_line(const math::Vector &rij, const math::Vector &vij,
          ...
          const math::Quaternion &orientationi,
          const math::Quaternion &orientationj, const double &length,
          bool skip_zero, double t_max) {
  detail::LinesOverlapFunc fL(rij, vij, angvi, angvj, orientationi,
                              orientationj, length);
