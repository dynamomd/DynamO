...
namespace magnet {
namespace math {
//! \brief Implementation details  for the \ref dilate and \ref undilate
//! functions.
 ...
 */
template <size_t d> struct dilation_rounds {
 ...
 */
template <size_t d> struct undilation_rounds {
  static const size_t result = ctime_ceil_log<s<d>::result, d>::result;
};

//! \brief The calculator for the \f$x_{p,q}\f$ constant.
template <size_t p, size_t q> struct x {
  template <size_t l, size_t dummy> struct xworker {
    static const size_t result = xworker<l - 1, dummy>::result +
                                 ctime_safe_lshift<size_t, 1, l * q>::result;
  };

  template <size_t dummy> struct xworker<0, dummy> {
...
template <size_t i_plus_1, size_t d> struct c {
  static const size_t result =
      x<d, (d - 1) * ctime_pow<d, i_plus_1 - 1>::result>::result;
};

//! \brief The calculator for the \f$b_{d,i}\f$ constant.
template <size_t i, size_t d> struct b {
  static const size_t result =
      x<d,
        ctime_pow<d - 1, dilation_rounds<d>::result - i + 1>::result>::result;
};
 ...
 */
template <size_t n> struct getnbits {
  static const size_t result = (ctime_safe_lshift<size_t, 1, n>::result - 1);
};

//! \brief Returns the maximum value that can be dilated.
template <size_t d> struct maxDilatableValue {
  static const size_t result = getnbits<s<d>::result>::result;
};

//! \brief Calculator for the \f$z_{d,i}\f$ constant.
template <size_t i, size_t d> struct z {
  static const size_t _di = ctime_pow<d, i>::result;
  static const size_t _bitcount = (_di < s<d>::result) ? _di : s<d>::result;

  template <size_t counter, size_t dummy> struct zworker {
    static const size_t result =
        zworker<counter - 1, dummy>::result |
        ctime_safe_rshift<size_t, zworker<0, dummy>::result,
                          ((_di * d) * counter)>::result;
  };

  template <size_t dummy> struct zworker<0, dummy> {
    static const size_t result = ctime_safe_lshift < size_t,
                        getnbits<_bitcount>::result &
                            getnbits<s<d>::result>::result,
                        d *(s<d>::result - 1) + 1 - _bitcount > ::result;
  };

  static const size_t result =
      zworker<s<d>::result / ctime_pow<d, i>::result, 0>::result;
};
 ...
 */
template <size_t i, size_t d> struct y {
  static const size_t _bitcount =
      ctime_pow<d - 1, dilation_rounds<d>::result - i>::result;
  static const size_t _bitseperation =
      ctime_pow<d - 1, dilation_rounds<d>::result - i + 1>::result + _bitcount;

  template <size_t counter, size_t dummy> struct yworker {
    static const size_t result =
        yworker<counter - 1, dummy>::result |
        ctime_safe_lshift<size_t, getnbits<_bitcount>::result,
                          _bitseperation * counter>::result;
  };

  template <size_t dummy> struct yworker<0, dummy> {
 ...
 */
template <size_t i> struct y<i, 2> {
 ...
 * dilation algorithm.
 */
template <size_t d> struct maxDilatedValue {
  static const size_t result = y<dilation_rounds<d>::result, d>::result;
};

//! \brief The undilation function.
template <size_t d> struct undilate {
  template <size_t i, size_t dummy> struct undilateWorker {
    static inline size_t eval(size_t val) {
      return (undilateWorker<i - 1, dummy>::eval(val) * c<i, d>::result) &
             z<i, d>::result;
    }
  };

  template <size_t dummy> struct undilateWorker<1, dummy> {
    static inline size_t eval(size_t val) {
  ...
  static inline size_t eval(size_t val) {
    static const size_t undilate_shift =
        (d * (s<d>::result - 1) + 1 - s<d>::result);
    return undilateWorker<undilation_rounds<d>::result, 0>::eval(val) >>
           undilate_shift;
  };
};

//! \brief The actual dilation function.
template <size_t d> struct dilate {
  template <size_t i, size_t dummy> struct dilateWorker {
    static inline size_t eval(size_t val) {
      return (dilateWorker<i - 1, d>::eval(val) * b<i, d>::result) &
             y<i, d>::result;
    }
  };

  template <size_t dummy> struct dilateWorker<1, dummy> {
    static inline size_t eval(size_t val) {
...
template <> struct dilate<2> {
  template <size_t i> struct shiftval {
    static const size_t result =
        ctime_safe_lshift<size_t, 1, dilation_rounds<2>::result - i>::result;
  };
 ...
 */
template <size_t d> inline size_t dilate(size_t val) {
#ifdef MAGNET_DEBUG
 ...
 */
template <size_t d> inline size_t undilate(size_t val) {
#ifdef MAGNET_DEBUG
 ...
 */
template <size_t d> class DilatedInteger {
public:
   ...
   */
  inline DilatedInteger(const size_t val) : _value(dilate<d>(val)) {}

  /*! \brief Copy constructor.
   */
  inline DilatedInteger(const DilatedInteger<d> &o) : _value(o._value) {}
  ...
  inline DilatedInteger operator+(const DilatedInteger &o) const {
    return DilatedInteger((_value + (~dilatedMask) + o._value) & dilatedMask,
                          0);
  }

  //! \brief Increment operator.
  inline DilatedInteger &operator++() {
    _value = (_value - dilatedMask) & dilatedMask;
    return *this;
  }

  //! \brief Decrement operator.
  inline DilatedInteger &operator--() {
    _value = (_value - 1) & dilatedMask;
    return *this;
  }
...
private:
   ...
   */
  inline DilatedInteger(const size_t val, void *) : _value(val) {}
 ...
 */
template <size_t d> struct numeric_limits<magnet::math::DilatedInteger<d>> {
public:
  ...
  inline static magnet::math::DilatedInteger<d> max() throw() {
    return magnet::math::DilatedInteger<d>(
        magnet::math::dilatedinteger::maxDilatedValue<d>::result, 0);
  }
