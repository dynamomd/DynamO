...
namespace magnet
{
  ...
  contributions (and sum up the impulsive contributions)
    ...
    \sa Correlator
       */
      TimeCorrelator(double sample_time, size_t length, bool removeAverage) : cc(length, removeAverage), ci(length, removeAverage), ic(length, removeAverage), ii(length, removeAverage),
