...
*/
template <class T>
std::pair<bool, double> quadRootHunter(const T &fL, double &t_low,
                                       double &t_high,
  ...
  while (t_low < t_high) {
    ...
    if (++w > 100) {
      ...
      if (fabs(t_high - t_low) < timescale) {
        ...
        std::cerr << "\nThe gap is too large and is converging too slowly."
                  << "\n This rootfinding attempt will be aborted and a fake "
                     "collision returned.";
#endif
    ...
    double deltaT;
    {
      double f0 = tempfL.template eval<0>(), f1 = tempfL.template eval<1>(),
      ...
      try {
        ...
        M_throw()
            << "When trying to improve the bounds using worst-case estimates, "
               "it was "
               "found that they could not be improved. This implies there is "
               "implementation error (zero max 2nd deriv?) in the passed "
               "function.";
      }

      // Sort the roots
      if (worst_case_roots.first > worst_case_roots.second)
        std::swap(worst_case_roots.first, worst_case_roots.second);

#ifdef MAGNET_DEBUG
      if ((worst_case_roots.first > 0) || (worst_case_roots.second < 0))
        M_throw() << "The worst case estimates for the root of the function "
                     "are not either "
                     "side of the current location. This implies there is an "
                     "implementation "
      ...
      // Sort the roots
      if (estimate_roots.first > estimate_roots.second)
        std::swap(estimate_roots.first, estimate_roots.second);

      if (fwdWorking) {
        // Check if there are no positive roots (restart from other boundary if
        // so)
        ...
        // Check if there are no negative roots (restart from other boundary if
        // so)
        if (estimate_roots.first > 0)
          continue;
    ...
    for (size_t i(100); i != 0; --i) {
      ...
      try {
        std::pair<double, double> estimate_roots = quadraticEquation(
            0.5 * tempfL.template eval<2>(), tempfL.template eval<1>(),
            tempfL.template eval<0>());
...
/*! \brief A root finder that is guarranteed to find the earliest
 ...
 If a root is found: start a new search in the window just between this root and
 the lower bound.
  ...
  - Check root validity
    ...
    - If root is invalid, set new concrete t_low just above this found root and
 go from the top

 \param toleranceLengthScale Should be 10^-10 the typical length scale of the
 system
...
*/
template <class T>
std::pair<bool, double> frenkelRootSearch(const T &fL, double t_low,
                                          double t_high,
  ...
  while (t_high > t_low) {
    ...
    do {
      ...
      temp_high = root.second -
                  (fabs(2.0 * tempfL.template eval<1>()) / Fdoubleprimemax);

      // Now check if the upper bound is below the lower bound.
      // If so, the current root is the earliest.
      if ((temp_high < t_low) || (Fdoubleprimemax == 0))
        break;

      // Search for a root in the new interval
      std::pair<bool, double> temp_root =
          quadRootHunter<T>(fL, t_low, temp_high, toleranceLengthScale);

      // If there is no root found in the interval, then the current root is
      // fine
      if ((!temp_root.first) && (temp_root.second == HUGE_VAL))
        break;
    ...
    t_low = root.second + ((2.0 * fabs(tempfL.template eval<1>())) /
                           tempfL.template max<2>());
