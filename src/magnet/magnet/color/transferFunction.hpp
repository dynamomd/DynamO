...
struct Knot {
  Knot(double x, double h, double s, double v, double a)
      : _x(clamp(x, 0.0, 1.0)), _h(clamp(h, 0.0, 1.0)), _s(clamp(s, 0.0, 1.0)),
        _v(clamp(v, 0.0, 1.0)), _a(clamp(a, 0.0, 1.0)) {}
...
public:
  ...
  const std::vector<float> getMap(size_t samples, float transmittanceScale) {
    ...
    for (size_t i(0); i < samples; ++i) {
      double transmittance =
          magnet::clamp(spline[3](double(i) / (samples - 1)), 0.0, 1.0);
  ...
  std::array<float, 4> getValue(float x) {
    std::array<float, 4> retval = {{float(spline[0](x)), float(spline[1](x)),
                                    float(spline[2](x)), float(spline[3](x))}};
    return retval;
  }
