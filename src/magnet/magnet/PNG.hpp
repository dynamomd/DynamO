...
*/
		...
		static void writeFile(const std::string& filename,
			const std::vector<uint32_t>& image, size_t& width,
				      size_t& height, bool gldata = false) {
			...
			if (!gldata)
			  ...
			  for(size_t y = 0; y < height; ++y)
			    for(size_t x = 0; x < width; ++x) 
			      {
				PNGPixel value;
				value.convert(image[(height-1 - y) * width + x]);
				pngData[rowSize * y + x * channels] = value.alpha();
				pngData[rowSize * y + x * channels + 1] = value.blue();
				pngData[rowSize * y + x * channels + 2] = value.green();
			      }
