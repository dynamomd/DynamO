 ...
 */
template <size_t NDim, typename Derived> class OrderingBase {
public:
  ...
  public:
    ...
    const_iterator(const Derived &container, const ArrayType &start,
                   const ArrayType &distance, const ArrayType &pos)
  ...
  OrderingBase(Args... args)
      : _dimensions(std::forward<Args>(args)...), _end() {
  ...
  const_iterator begin() const {
    return const_iterator(static_cast<const Derived &>(*this), {{0, 0, 0}},
                          _dimensions);
  }
  ...
  IndexRange getIndices(const ArrayType &start,
                        const ArrayType &distance) const {
    ArrayType end = ArrayType();
    end[NDim - 1] = distance[NDim - 1];
    return IndexRange(
        const_iterator(static_cast<const Derived &>(*this), start, distance),
        const_iterator(static_cast<const Derived &>(*this), start, distance,
                       end));
  }

  IndexRange getSurroundingIndices(const ArrayType &center,
                                   const ArrayType &distance) const {
    ArrayType zero;
    ArrayType range;
    for (size_t i(0); i < NDim; ++i) {
...
class RowMajorOrdering
    : public detail::OrderingBase<NDim, RowMajorOrdering<NDim>> {
...
public:
  ...
  template <typename... Args>
  RowMajorOrdering(Args... args) : Base(std::forward<Args>(args)...) {}

  size_t toIndex(const ArrayType &loc) const {
    size_t index = 0;
    for (size_t i(0); i < NDim; ++i) {
      index = index * Base::_dimensions[NDim - 1 - i] +
              (loc[NDim - i - 1] % Base::_dimensions[NDim - 1 - i]);
    }
    return index;
  }
...
public:
  ...
  template <typename... Args>
  MortonOrdering(Args... args) : Base(std::forward<Args>(args)...) {}

  size_t toIndex(const ArrayType &loc) const {
    size_t index = 0;
    for (size_t i(0); i < NDim; ++i) {
      index += magnet::math::DilatedInteger<NDim>(loc[i] % Base::_dimensions[i])
                   .getDilatedValue()
               << i;
    }
    return index;
  ...
  size_t length() const {
    size_t length = 0;
    for (size_t i(0); i < NDim; ++i)
      length += magnet::math::DilatedInteger<NDim>(Base::_dimensions[i])
                    .getDilatedValue()
                << i;
    return length;
  }
