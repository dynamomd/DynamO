...
public:
  ...
  inline const Comparable &bottom() const {
    ...
    if (_currentSize <= 1)
      ...
      return _array[1];
    else if (N > 2) // This is just to stop a compiler warning for N=2
                    // containers
  ...
  inline Comparable &unsafe_bottom() {
    ...
    if (_currentSize == 1)
      ...
      M_throw()
          << "Bad _currentSize when accessing heap bottom in N=2 MinMaxHeap";
  }
  ...
  inline size_t findMinDescendent(size_t child, size_t grandchild) {
    ...
    if (child <= _currentSize) {
      ...
      if (grandchild <= _currentSize) {
        ...
        for (size_t i = 1; grandchild < _currentSize && i < 4;
             i++, grandchild++) {
...
inline void swap(magnet::containers::MinMaxHeap<T, N> &lhs,
