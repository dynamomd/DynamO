...
public:
  ...
  void open(std::string filename, const size_t width, const size_t height,
            int fps = 25) {
    if (_outputFile.is_open())
      M_throw()
          << "Trying to open a video file when one is already being outputted!";
    ...
    if (!_codec) {
      _h264 = false;
      std::cerr << "\nWARNING: Cannot open the H264 codec (try installing "
                   "libx264 [ubuntu:libavcodec-extra-53]), falling back to the "
                   "MPEG2 codec.\n";
      _codec = avcodec_find_encoder(AV_CODEC_ID_MPEG2VIDEO);
      if (!_codec) {
        std::cerr << "\nWARNING: Cannot open a MPEG2 codec either! Dropping to "
                     "MPEG1, quality of results will be poor.\n";
    ...
    _outputFile.open(filename.c_str(), std::ios_base::out |
                                           std::ios_base::binary |
                                           std::ios_base::binary);
    if (!_outputFile.is_open())
  ...
  void addFrame(const std::vector<uint8_t> &RGB24Frame, bool flipY = false) {
    ...
    //* convert the RGB image to YUV420p and copy it into the picture buffer
    {
      ...
      for (int j = 0; j < _context->height; j++)
        for (int k = 0; k < _context->width; k++) {
          size_t s =
              (k + (flipY ? (_context->height - j - 1) : j) * _inputWidth) * 3;

          _pictureBuffer[i] =
              (uint8_t)((66 * RGB24Frame[s + 0] + 129 * RGB24Frame[s + 1] +
                         25 * RGB24Frame[s + 2] + 128) >>
                        8) +
              16;

          if (0 == j % 2 && 0 == k % 2) {
            _pictureBuffer[ui++] =
                (uint8_t)((-38 * RGB24Frame[s + 0] - 74 * RGB24Frame[s + 1] +
                           112 * RGB24Frame[s + 2] + 128) >>
                          8) +
                128;
            _pictureBuffer[vi++] =
                (uint8_t)((112 * RGB24Frame[s + 0] - 94 * RGB24Frame[s + 1] -
                           18 * RGB24Frame[s + 2] + 128) >>
                          8) +
                128;
          }
          i++;
    ...
    if (ret < 0)
      M_throw() << "Failed to encode a frame of video.";

    // Now process output
    AVPacket *packet = av_packet_alloc();
    ret = avcodec_receive_packet(_context, packet);
    while (!ret) {
      _outputFile.write(reinterpret_cast<const char *>(packet->data),
                        packet->size);
      ret = avcodec_receive_packet(_context, packet);
    }
