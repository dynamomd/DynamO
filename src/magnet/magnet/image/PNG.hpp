...
inline void read(png_structp png, png_bytep data, png_size_t length) {
  static_cast<std::istream *>(png_get_io_ptr(png))
...
inline void write(png_structp png, png_bytep data, png_size_t length) {
  static_cast<std::ostream *>(png_get_io_ptr(png))
...
inline void readPNGFile(const std::string &filename,
                        std::vector<uint8_t> &image, size_t &width,
  ...
  png_set_read_fn(png, static_cast<void *>(&pngFile), detail::read);
 ...
 * \param compressionLevel The level of compression requested from the PNG
 * library.
 * \param disableFiltering Prevent the PNG library from filtering the output.
 * \param flip Flips the vertical ordering of the image (to allow easy saving of
 * OpenGL renderings).
 */
inline void writePNGFile(const std::string &filename,
  ...
  if (image.size() != width * height * components) {
    ...
    strm << "invalid input to writePNGFile(): "
         ...
         << ", should be " << width << "x" << height << " = " << width * height;
    throw std::runtime_error(strm.str().c_str());
  }
  ...
  png_set_write_fn(png, static_cast<void *>(&pngFile), detail::write,
                   detail::flush);
