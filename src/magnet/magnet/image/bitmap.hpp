 ...
 */
template <class T> inline void write(std::ostream &os, const T &obj) {
...
public:
  inline bitmap_information_header(size_t width, size_t height)
      : _type(19778), _fileSize(54 + ((width * 3 + 3) & 0xFFFC) * height),
        _reserved1(0), _reserved2(0), _offset_bits(54), _headerSize(40),
        _width(width), _height(height), _planes(1), _bitdepth(24),
        _compression(0),
        _compressed_file_size(((width * 3 + 3) & 0xFFFC) * height), _xres(2835),
        _yres(2835), _palleteSize(0), _importantColors(0) {}
...
inline void writeBMPFile(const std::string &filename,
                         const std::vector<uint8_t> &image, size_t width,
                         size_t height, size_t components = 4) {
