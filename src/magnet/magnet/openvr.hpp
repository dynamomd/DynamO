...
public:
  OpenVRTracker(std::function<void(std::string)> log = [](std::string) {})
      : Camera(0.3f, 30.0f), // Observe 30cm to 30m
        _vr(nullptr), _log(log), _eye(vr::Eye_Left) {
    _hmd_pose = magnet::GL::GLMatrix::identity();
  }

  void setEye(vr::EVREye x) { _eye = x; }
  ...
  void init() {
    ...
    if (!vr::VR_IsRuntimeInstalled()) {
      _log("Error: No OpenVR runtime library detected, have you installed "
           "SteamVR?");
      return;
    } else
    ...
    for (vr::TrackedDeviceIndex_t i(0); i < vr::k_unMaxTrackedDeviceCount; ++i)
      if (_vr->IsTrackedDeviceConnected(i)) {
        std::tie(success, val) =
            GetTrackedDeviceString(i, vr::Prop_TrackingSystemName_String);

        if (!success) {
          _log("Error: While fetching Device#" + std::to_string(i) + " name, " +
               val);
          shutdown();
          return;
        } else
        ...
        std::tie(success, val) =
            GetTrackedDeviceString(i, vr::Prop_SerialNumber_String);
        if (!success) {
          _log("Error: While fetching Device#" + std::to_string(i) +
               " serial, " + val);
          shutdown();
          return;
        } else
        ...
        _log("Device#" + std::to_string(i) +
             " class: " + to_string(_vr->GetTrackedDeviceClass(i)));
      }
  ...
  void submit() {
    vr::Texture_t tex = {
        reinterpret_cast<void *>(
            intptr_t(getResolveBuffer().getColorTexture()->getGLHandle())),
  ...
  virtual void resize(size_t width, size_t height, size_t samples) {
    ...
    std::shared_ptr<magnet::GL::Texture2D> r_colorTexture(
    ...
    std::shared_ptr<magnet::GL::Texture2D> r_depthTexture(
        new magnet::GL::Texture2D);
  ...
  void handleEvents() {
    ...
    while (_vr->PollNextEvent(&event, sizeof(event))) {
      ...
      case vr::VREvent_TrackedDeviceActivated:
        ...
        _log("Device " + std::to_string(event.trackedDeviceIndex) +
             " attached.");
        break;
    ...
    // for( vr::TrackedDeviceIndex_t unDevice = 0; unDevice <
    // vr::k_unMaxTrackedDeviceCount; unDevice++ )
    //   {
  ...
  void getPosesAndSync() {
    vr::EVRCompositorError err = vr::VRCompositor()->WaitGetPoses(
        _tracked_devices.data(), vr::k_unMaxTrackedDeviceCount, nullptr, 0);

    if (err != vr::VRCompositorError_None)
      _log("Error: " + to_string(err));

    if (_tracked_devices[vr::k_unTrackedDeviceIndex_Hmd].bPoseIsValid) {
      _hmd_pose = magnet::math::inverse(
          convert(_tracked_devices[vr::k_unTrackedDeviceIndex_Hmd]
                      .mDeviceToAbsoluteTracking));
      _eyePosLeft = magnet::math::inverse(
          convert(_vr->GetEyeToHeadTransform(vr::Eye_Left)));
      _eyePosRight = magnet::math::inverse(
          convert(_vr->GetEyeToHeadTransform(vr::Eye_Right)));

      _projectionLeft = convert(
          _vr->GetProjectionMatrix(vr::Eye_Left, _zNearDist, _zFarDist));
      _projectionRight = convert(
          _vr->GetProjectionMatrix(vr::Eye_Right, _zNearDist, _zFarDist));
    }
  }

  void PostPresentHandoff() { vr::VRCompositor()->PostPresentHandoff(); }
...
protected:
  ...
  static std::string to_string(vr::TrackedPropertyError e) {
    ...
    case vr::TrackedProp_WrongDeviceClass:
      return "The property was requested on a tracked device with the wrong "
             "class.";
    case vr::TrackedProp_BufferTooSmall:
      return "The string property will not fit in the provided buffer. The "
             "buffer size needed is returned.";
    ...
    case vr::TrackedProp_CouldNotContactServer:
      return "OpenVR could not contact vrserver to query the device for this "
             "property.";
    case vr::TrackedProp_ValueNotProvidedByDevice:
      return "The driver for this device returned that it does not provide "
             "this specific property for this device.";
    case vr::TrackedProp_StringExceedsMaximumLength:
      return "The string property value returned by a driver exceeded the "
             "maximum property length of 32k.";
    default:
  ...
  GetTrackedDeviceString(vr::TrackedDeviceIndex_t unDevice,
    ...
    uint32_t unRequiredBufferLen =
        _vr->GetStringTrackedDeviceProperty(unDevice, prop, NULL, 0, &error);
    ...
    unRequiredBufferLen = _vr->GetStringTrackedDeviceProperty(
        unDevice, prop, pchBuffer, unRequiredBufferLen, &error);
  ...
  magnet::GL::GLMatrix _projectionLeft, _projectionRight, _eyePosLeft,
      _eyePosRight, _hmd_pose;
  vr::EVREye _eye;
