...
bool testOutput(const std::vector<T> &input, const std::vector<T> &output) {
  ...
  for (size_t i(0); i < output.size(); ++i)
    if (output[i] != answer[i]) {
      std::cout << "Error i = " << i << " output = " << output[i]
                << " answer = " << answer[i] << "\n";
      result = false;
    }
...
void runTestType(cl::Context context, cl::CommandQueue queue) {
  ...
  std::cout << "##Testing heap sort for " << input.size()
            << " elements and type "
            << magnet::CL::detail::traits<T>::kernel_type() << std::endl;
  ...
  if (!testOutput(input, output))
    M_throw() << "Incorrect output for keysort with size " << input.size()
              << " and type " << magnet::CL::detail::traits<T>::kernel_type();
  ...
  try {
    ...
    for (std::vector<cl::Platform>::const_iterator pltfmIt = platforms.begin();
      ...
      for (std::vector<cl::Device>::const_iterator devIt = allDevices.begin();
        ...
        if (devIt->getInfo<CL_DEVICE_TYPE>() != CL_DEVICE_TYPE_CPU)
          continue;
