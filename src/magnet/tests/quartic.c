...
*/
{
  ...
  worstd = errors(a,b,c,d,rtsd,rterd,nrtsd);
  nrtsf = ferrari(a,b,c,d,rtsf);
  if(iterate==TRUE) quanewton(a,b,c,d,nrtsf,rtsf);
  worstf = errors(a,b,c,d,rtsf,rterf,nrtsf);
  nrtsn = neumark(a,b,c,d,rtsn);
  if(iterate==TRUE) quanewton(a,b,c,d,nrtsn,rtsn);
  worstn = errors(a,b,c,d,rtsn,rtern,nrtsn);
  nrtsy = yacfraid(a,b,c,d,rtsy);
  if(iterate==TRUE) quanewton(a,b,c,d,nrtsy,rtsy);
  worsty = errors(a,b,c,d,rtsy,rtery,nrtsy);
  nrtsc = chris(a,b,c,d,rtsc);
  if(iterate==TRUE) quanewton(a,b,c,d,nrtsc,rtsc);
  worstc = errors(a,b,c,d,rtsc,rterc,nrtsc);
...
*/
{
  ...
  if (debug < 2) printf("quanewtona %d %g\n",
       n4,v4[0]);
  for (j = 0; j < n4; ++j)
  {
     for (k = 0; k < 4; ++k)
     {
        ...
        v4[j] -= corr;
        if (debug < 1) printf("quanewtonb %d %d %g %g %g %g %g\n",
             j,k,root,err,deriv,corr,v4[j]);
     }
  }
} /* quanewton */
/****************************************************/
