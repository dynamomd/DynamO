...
void test()
{
  const size_t maxval = magnet::math::dilatedinteger::maxDilatableValue<d>::result;
  ...
  for (size_t test(0); test < 1000000; ++test)
    {
      size_t i = dist(RNG);
      BOOST_CHECK_MESSAGE(i == magnet::math::undilate<d>(magnet::math::dilate<d>(i)), "Failed to dilate/undilate " << i << "\n");
    }

  BOOST_CHECK_MESSAGE(maxval == magnet::math::undilate<d>(magnet::math::dilate<d>(maxval)), "Undilate and dilate are not symmetric for the max value");
}
