...
COPMSDCorrelator::COPMSDCorrelator(const DYNAMO::SimData* tmp, const XMLNode&):
...
COPMSDCorrelator::initialise()
{
  ...
  speciesData.resize(Sim->Dynamics.getSpecies().size(), std::vector<Iflt>(length, 0.0));

  structData.resize(Sim->Dynamics.getTopology().size(), std::vector<Iflt>(length, 0.0));
}
...
COPMSDCorrelator::accPass()
{
  ...
  BOOST_FOREACH(const CSpecies& sp, Sim->Dynamics.getSpecies())
    ...
    for (size_t step(1); step < length; ++step)
      speciesData[sp.getID()][step] += (posHistory[ID][step] - posHistory[ID][0]).square();
  ...
  BOOST_FOREACH(const smrtPlugPtr<CTopology>& topo, Sim->Dynamics.getTopology())
	...
	{
	  molCOM += posHistory[ID][0] * Sim->Dynamics.getSpecies(Sim->vParticleList[ID]).getMass();
      ...
      for (size_t step(1); step < length; ++step)
	{
	  ...
	  BOOST_FOREACH(const size_t& ID, *range)
	    molCOM2 += posHistory[ID][step] * Sim->Dynamics.getSpecies(Sim->vParticleList[ID]).getMass();
...
COPMSDCorrelator::output(xmlw::XmlStream &XML)
{
  ...
  Iflt dt = dynamic_cast<const CSTicker&>(*Sim->Dynamics.getSystem("SystemTicker")).getPeriod()
  ...
  BOOST_FOREACH(const smrtPlugPtr<CTopology>& topo, Sim->Dynamics.getTopology())
    {
      ...
      for (size_t step(0); step < length; ++step)
	XML << dt * step << " "
	    << structData[topo->getID()][step] / (ticksTaken * topo->getMolecules().size())
	    << "\n";
