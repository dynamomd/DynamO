...
#define FILTER_FACTORY(F)                                                      \
  ...
  F(9, "SSAO: Shadow (After, use a bilateral blur, then multiply)",            \
    SSAOWrapper)                                                               \
  F(10, "Multiply with Original Image", MultiplyFilter)                        \
  F(11, "DOF Filter (Make a fully blurred image first)", DOFFilter)            \
  F(12,                                                                        \
    "Flush Current (Do after a SSAO/DOF filter to allow further processing)",  \
    FlushToOriginal)
...
#define ENUM_GEN_FUNC(enumeration, stringname, type)                           \
  template <> struct filterEnum<type> {                                        \
    static const size_t val = enumeration;                                     \
  };
...
public:
  ...
  virtual void invoke(GLint colorTextureUnit, size_t width, size_t height,
                      const magnet::GL::Camera &vp) = 0;
...
public:
  inline virtual size_t type_id() {
    return detail::filterEnum<FlushToOriginal>::val;
  }
  inline virtual void invoke(GLint, size_t, size_t,
                             const magnet::GL::Camera &) {}
