...
SSAOWrapper::SSAOWrapper() {
  ...
  _radiusSlider.signal_changed().connect(
      sigc::mem_fun(this, &SSAOWrapper::settingsCallback));
  _radiusSlider.show();

  _totStrengthSlider.set_text(boost::lexical_cast<std::string>(_totStrength));
  _totStrengthSlider.signal_changed().connect(
      sigc::mem_fun(this, &SSAOWrapper::settingsCallback));
  _totStrengthSlider.show();

  _dropoffSlider.set_text(boost::lexical_cast<std::string>(_dropoff));
  _dropoffSlider.signal_changed().connect(
      sigc::mem_fun(this, &SSAOWrapper::settingsCallback));
  _dropoffSlider.show();
}

void SSAOWrapper::showControls(Gtk::ScrolledWindow *start) {
  _optlist.unparent();
  start->add(_optlist);
  start->show();
}
...
void SSAOWrapper::settingsCallback() {
  ...
  try {
    _radius = boost::lexical_cast<float>(_radiusSlider.get_text());
  } catch (...) {
  };
  try {
    _totStrength = boost::lexical_cast<float>(_totStrengthSlider.get_text());
  } catch (...) {
  };
  try {
    _dropoff = boost::lexical_cast<float>(_dropoffSlider.get_text());
  } catch (...) {
  };
}

SSAOWrapper::~SSAOWrapper() { glDeleteTextures(1, &_randomTexture); }

void SSAOWrapper::invoke(GLint colorTextureUnit, size_t width, size_t height,
