...
namespace coil {
CLGLWindow::CLGLWindow(std::string title, double updateIntervalValue,
                       bool dynamo)
    : _selectedObjectID(0), _systemQueue(new magnet::thread::TaskQueue),
      _updateIntervalValue(updateIntervalValue), _mouseState(DEFAULT),
      windowTitle(title), _frameCounter(0), _updateCounter(0),
      _mouseSensitivity(0.3), _moveSensitivity(0.01), _specialKeys(0),
      _simrun(false), _simframelock(false), _snapshot(false), _record(false),
      _PNGFileFormat(true), _fpsLimit(false), _fpsLimitValue(25),
      _filterEnable(true), _stereoMode(false), _openVRMode(false),
      _ambientIntensity(0.001), _snapshot_counter(0), _video_counter(0),
      _samples(1), _dynamo(dynamo), _cameraMode(ROTATE_WORLD) {
...
bool CLGLWindow::CallBackIdleFunc() {
  try {
...
namespace {
void setIcon(Glib::RefPtr<Gtk::Builder> &builder, std::string imgname,
...
void CLGLWindow::initGTK() {
  ...
  _timeout_connection = Glib::signal_timeout().connect_seconds(
      sigc::mem_fun(this, &CLGLWindow::GTKTick), 1);
...

  {
    ...
    button->signal_clicked().connect(sigc::bind(
        sigc::mem_fun(_camera, &magnet::GL::CameraHeadTracking::setViewAxis),
        magnet::math::Vector{1, 0, 0}));

    _refXml->get_widget("CamPlusYbtn", button);
    button->signal_clicked().connect(sigc::bind(
        sigc::mem_fun(_camera, &magnet::GL::CameraHeadTracking::setViewAxis),
        magnet::math::Vector{0, 1, 0}));

    _refXml->get_widget("CamPlusZbtn", button);
    button->signal_clicked().connect(sigc::bind(
        sigc::mem_fun(_camera, &magnet::GL::CameraHeadTracking::setViewAxis),
        magnet::math::Vector{0, 0, 1}));
  ...
  for (const auto &b :
       std::vector<std::pair<std::string, void (CLGLWindow::*)()>>{
  ...
  ///////Create a top menu
  {
    ...
    m_refActionGroup->add(Gtk::Action::create("AboutItem", "About"),
                          sigc::mem_fun(this, &CLGLWindow::aboutCallback));
    m_refActionGroup->add(
        Gtk::Action::create("ResizeMenu", "Resize render window"));
    m_refActionGroup->add(Gtk::Action::create("800by600", " 800x600"),
                          sigc::bind(&resizeGlutWindow, 800, 600));
    m_refActionGroup->add(Gtk::Action::create("1024by768", " 1024x768"),
                          sigc::bind(&resizeGlutWindow, 1024, 768));
    m_refActionGroup->add(Gtk::Action::create("1280by720", " 1280x720"),
                          sigc::bind(&resizeGlutWindow, 1280, 720));
    m_refActionGroup->add(Gtk::Action::create("1920by1080", " 1920x1080"),
                          sigc::bind(&resizeGlutWindow, 1920, 1080));
  ...
  { ////////Simulation run control
    ...
    togButton->signal_toggled().connect(
        sigc::mem_fun(this, &CLGLWindow::runCallback));
  }

  { ///////Frame lock control
    Gtk::ToggleButton *framelockButton;
    _refXml->get_widget("SimLockButton", framelockButton);
    framelockButton->signal_toggled().connect(
        sigc::mem_fun(this, &CLGLWindow::simFramelockControlCallback));
  }
  ...
  { ///////Recording button
    ...
    recordButton->signal_toggled().connect(
        sigc::mem_fun(this, &CLGLWindow::recordCallback));
  }
  ...
  { ///////Control the update rate from the simulation
    ...
    updateFreq->signal_changed().connect(
        sigc::mem_fun(this, &CLGLWindow::guiUpdateCallback));
  }
  ...
  { ///////FPS lock
    ...
    fpslockButton->signal_toggled().connect(
        sigc::mem_fun(this, &CLGLWindow::FPSLimitCallback));
  }
  ...
  { ///////FPS lock value
    ...
    fpsButton->signal_value_changed().connect(
        sigc::mem_fun(this, &CLGLWindow::FPSLimitCallback));
  }
  ...
  ///////////////////////Render Pipeline//////////////////////////////////
  {
    ...
    _aasamples->signal_changed().connect(
        sigc::mem_fun(this, &CLGLWindow::AAsamplechangeCallback));

    {
      Gtk::Entry *AmbientLightIntensity;
      _refXml->get_widget("AmbientLightIntensity", AmbientLightIntensity);
      AmbientLightIntensity->set_text(
          boost::lexical_cast<std::string>(_ambientIntensity));
      AmbientLightIntensity->signal_changed().connect(
          sigc::mem_fun(*this, &CLGLWindow::guiUpdateCallback));
    }
    ...
    { ///////////////////////Filters//////////////////////////////////
      ...
      //////Connect the filterView select callback
      {
        ...
        treeSelection->signal_changed().connect(
            sigc::mem_fun(this, &CLGLWindow::filterSelectCallback));
      }
      ...
      { /// Connect the control buttons
        ...
        btn->signal_toggled().connect(
            sigc::mem_fun(this, &CLGLWindow::filterActiveCallback));
      }
    ...
    { /////////////////////3D effects
      {
        ...
        stereoEnable->signal_toggled().connect(
            sigc::mem_fun(this, &CLGLWindow::guiUpdateCallback));
      }

      {
        ...
        view->signal_size_allocate().connect([=](Gtk::Allocation &) {
          win->get_vadjustment()->set_value(
              win->get_vadjustment()->get_upper());
        });

        auto vrlog = Glib::RefPtr<Gtk::TextBuffer>::cast_dynamic(
            _refXml->get_object("OpenVRTextBuffer"));
        Gtk::Button *clear;
...
#ifdef COIL_OpenVR
      {
        ...
        vrEnable->signal_toggled().connect(
            sigc::mem_fun(this, &CLGLWindow::guiUpdateCallback));
...

      {
        ...
        simunits->signal_changed().connect(
            sigc::bind(&magnet::gtk::forceNumericEntry, simunits));
        simunits->signal_activate().connect(
            sigc::mem_fun(*this, &CLGLWindow::guiUpdateCallback));
      }

      {
        ...
        pixelPitch->signal_changed().connect(
            sigc::bind(&magnet::gtk::forceNumericEntry, pixelPitch));
        pixelPitch->signal_activate().connect(
            sigc::mem_fun(*this, &CLGLWindow::guiUpdateCallback));
      }
      ...
      { // Here all the wii stuff should go in
        ...
        btn->signal_clicked().connect(
            sigc::mem_fun(this, &CLGLWindow::wiiMoteConnect));
...

      {
        ...
        ir->signal_draw().connect(
            sigc::mem_fun(this, &CLGLWindow::wiiMoteIRExposeEvent));
      }
      ...
      { // Here all the wii stuff should go in
        ...
        btn->signal_clicked().connect(
            sigc::mem_fun(&(magnet::TrackWiimote::getInstance()),
  ...
  { ///////////////////////Render Objects//////////////////////////////////
    ...
    //////Connect the view to the select callback
    {
      ...
      treeSelection->signal_changed().connect(
          sigc::mem_fun(this, &CLGLWindow::selectRObjCallback));
    }
  }
...
void CLGLWindow::init() {
  ...
  {
    std::shared_ptr<RLight> light(new RLight("Light", Vector{1, 1, 1}, look_at,
                                             0.1f, 300.0f, up,
                                             _camera.getRenderScale(), 0.2));
  ...
  CoilRegister::getCoilInstance().CallGlutCreateWindow(windowTitle.c_str(),
                                                       this);
  ...
  {
    // Build depth buffer
    std::shared_ptr<magnet::GL::Texture2D> depthTexture(
        new magnet::GL::Texture2D());
    // We don't force GL_DEPTH_COMPONENT24 as it is likely you get
    // the best precision anyway
    depthTexture->init(
        1024, 1024, GL_DEPTH_COMPONENT); // SIZE MUST BE THE SAME FOR THE LIGHTS
    ...
    // Build color texture
    std::shared_ptr<magnet::GL::Texture2D> colorTexture(
        new magnet::GL::Texture2D());
    colorTexture->init(1024, 1024,
                       GL_RG32F); // SIZE MUST BE THE SAME FOR THE LIGHTS
...
void CLGLWindow::CallBackDisplayFunc() {
  if (!CoilRegister::getCoilInstance().isRunning() || !_readyFlag)
    return;
  // Setup the timings
  ...
  if ((magnet::TrackWiimote::getInstance()).connected()) {
    ...
    if (wiiHeadTrack->get_active())
      _camera.setEyeLocation(
          magnet::TrackWiimote::getInstance().getHeadPosition());
  }
#endif
  ...
  if (!_stereoMode) {
    ...
    case 2:
      _camera.setEyeLocation(headPosition + eyeDisplacement);
      drawScene(_camera, enable_2D);
      _camera._renderTarget.blitToScreen(
          _camera.getWidth(), _camera.getHeight() / 2, 0, 0, GL_LINEAR);

      _camera.setEyeLocation(headPosition - eyeDisplacement);
      drawScene(_camera, enable_2D);
      _camera._renderTarget.blitToScreen(_camera.getWidth(),
                                         _camera.getHeight() / 2, 0,
                                         _camera.getHeight() / 2, GL_LINEAR);
      break;
    default:
  ...
  if (_snapshot || ((_record) && (!_simframelock || _newData))) {
    ...
    filename << std::setw(6) << std::setfill('0') << std::right << std::dec
             << _snapshot_counter++;

    if (_snapshot)
      magnet::image::writePNGFile(path + "/" + filename.str() + ".png", pixels,
                                  _camera.getWidth(), _camera.getHeight(), 3, 1,
                                  true, true);

#ifdef MAGNET_FFMPEG_SUPPORT
    if (_record)
      _encoder->addFrame(pixels, true);
#endif
    _snapshot = false;
    _newData = false;
  }
...
void CLGLWindow::drawScene(magnet::GL::Camera &camera, bool draw_2D_overlay) {
  ...
  for (auto &obj : _renderObjsTree._renderObjects)
    if (obj->visible())
      obj->glRender(camera, RenderObj::DEFAULT);
  ...
  for (auto &light_obj : _renderObjsTree._renderObjects) {
    std::shared_ptr<RLight> light =
        std::dynamic_pointer_cast<RLight>(light_obj);
    if (!light || !(light->shadowCasting()))
      continue;
  ...
  for (auto &obj : _renderObjsTree._renderObjects)
    if (obj->visible())
      obj->forwardRender(camera._hdrBuffer, camera, lights, _ambientIntensity,
                         RenderObj::DEFAULT);
  ...
  //
  //    avg /= count;
  //    std::cout << "Luminance Avg="<< avg << ", max=" << max << ", min=" <<
  //    min << ", count=" << count * 10000.0 << "\n";
  ...
  // average, minimum and maximum luminances.
  {
    GLsizei currentWidth =
        camera._luminanceBuffer1.getColorTexture()->getWidth();
    GLsizei currentHeight =
        camera._luminanceBuffer1.getColorTexture()->getHeight();
    GLint numLevels =
        camera._luminanceBuffer1.getColorTexture()->calcMipmapLevels();
  ...
  if (_bloomEnable) {
    ...
    std::array<GLfloat, 2> invDim = {
        {1.0f / (tex.getWidth() / 4), 1.0f / (tex.getHeight() / 4)}};
  ...
  if (_filterEnable) {
    ...
    for (auto &child : _filterStore->children()) {
      ...
      if (!(child[_filterModelColumns->m_active]))
        continue; // Only run active filters, skip to the next filter
      if (filter.type_id() ==
          detail::filterEnum<FlushToOriginal>::val) { // Check if we're trying
                                                      // to flush the drawing
        lastFBO->attach();
        ...
        if (FBOalternate) {
          camera._filterTarget1.detach();
          lastFBO = &camera._filterTarget1;
        } else {
          camera._filterTarget2.detach();
          lastFBO = &camera._filterTarget2;
        }
  ...
  if (draw_2D_overlay) {
    ...
    if (_selectedObject) {
      ...
      _cairo_screen.drawTextBox(
          vec[0] + 5, vec[1] + 5,
          _selectedObject->getCursorText(_selectedObjectID), 5);
    }
...
void CLGLWindow::setWindowtitle(const std::string &newtitle) {
  windowTitle = newtitle;
  glutSetWindowTitle(windowTitle.c_str());
}

void CLGLWindow::CallBackMouseFunc(int button, int state, int x, int y) {
  switch (button) {
...
void CLGLWindow::CallBackMouseWheelFunc(int button, int dir, int x, int y) {
  if (dir > 0)
    _moveSensitivity *= 1.1;
  else
    _moveSensitivity /= 1.1;
...
void CLGLWindow::CallBackMotionFunc(int x, int y) {
...
void CLGLWindow::CallBackKeyboardFunc(unsigned char key, int x, int y) {
...
void CLGLWindow::CallBackKeyboardUpFunc(unsigned char key, int x, int y) {
  keyStates[std::tolower(key)] = false;
}

bool CLGLWindow::simupdateTick(double t) {
  if (!isReady())
    return false;
...
void CLGLWindow::simFramelockControlCallback() {
  Gtk::ToggleButton *framelockButton;
  _refXml->get_widget("SimLockButton", framelockButton);
...
void CLGLWindow::snapshotCallback() { _snapshot = true; }

void CLGLWindow::recordCallback() {
  Gtk::ToggleButton *recordButton;
  _refXml->get_widget("SimRecordButton", recordButton);
  ...
  if (_record !=
      recordButton->get_active()) { // The record button has been toggled
    if (!_record) {
      std::string path;
      {
        Gtk::FileChooserButton *fileChooser;
      ...
      counterstr << std::setw(6) << std::setfill('0') << std::right << std::dec
                 << _video_counter++;
...
void CLGLWindow::filterUpCallback() {
...
void CLGLWindow::filterDownCallback() {
  Glib::RefPtr<Gtk::TreeSelection> refTreeSelection =
      _filterView->get_selection();
...
void CLGLWindow::filterDeleteCallback() {
  Glib::RefPtr<Gtk::TreeSelection> refTreeSelection =
      _filterView->get_selection();
...
void CLGLWindow::filterAddCallback() {
  // Grab the filter select box
  Gtk::ComboBox *filterSelectBox;
  _refXml->get_widget("filterSelectBox", filterSelectBox);

  // Check the filterSelectBox is on a valid row
  if (filterSelectBox->get_active_row_number() < 0)
    return;
  ...
  size_t type_id =
      (*filterSelectBox
            ->get_active())[Filter::getSelectColumnsInstance().m_col_id];

  (*iter)[_filterModelColumns->m_filter_ptr] = Filter::createFromID(type_id);

  (*iter)[_filterModelColumns->m_name] = Filter::getName(type_id);

  (*iter)[_filterModelColumns->m_active] = true;

  filterSelectCallback();
}

void CLGLWindow::filterSelectCallback() {
  Glib::RefPtr<Gtk::TreeSelection> refTreeSelection =
      _filterView->get_selection();
...
void CLGLWindow::filterActiveCallback() {
  Glib::RefPtr<Gtk::TreeSelection> refTreeSelection =
      _filterView->get_selection();
  Gtk::TreeModel::iterator iter = refTreeSelection->get_selected();
...
void CLGLWindow::SaveDataCallback() {
  Gtk::FileChooserDialog dialog(*controlwindow, "Save as",
  ...
  if (dialog.run() == Gtk::RESPONSE_OK) {
    ...
    if ((file_path.size() < 5) ||
        (file_path.substr(file_path.size() - 5, 5) != ".coil"))
...
void CLGLWindow::LoadDataCallback() {
  Gtk::FileChooserDialog dialog(*controlwindow, "Load data");

  std::vector<std::pair<std::string, std::string>> types{{"Coil file", "coil"},
                                                         {"Volume raw", "raw"}};
  ...
  if (dialog.run() == Gtk::RESPONSE_OK) {
    ...
    std::string filename_only =
        boost::filesystem::path(dialog.get_filename()).filename().string();

    if ((filename_only.size() >= 4) &&
        (filename_only.substr(filename_only.size() - 4, 4) == ".raw")) {
      ...
      if (volumedialog->run() == Gtk::RESPONSE_OK) {
        std::shared_ptr<coil::RVolume> voldata(
            new coil::RVolume(filename_only));
        ...
        getGLContext()->queueTask(std::bind(
            &coil::RVolume::loadRawFile, voldata.get(), dialog.get_filename(),
            data_dims, data_size, Vector{0, 0, 0}));
      }
      volumedialog->hide();
    } else if ((filename_only.size() >= 5) &&
               (filename_only.substr(filename_only.size() - 5, 5) == ".coil")) {

    } else {
      M_throw() << "Unhandled file type "
                << filename_only.substr(filename_only.size() - 3, 3);
    }
  }
}

void CLGLWindow::filterClearCallback() {
  if (_filterStore->children().empty())
    return;
  ...
  Gtk::MessageDialog confirmation(
      *window, "Are you sure you wish to erase all filters?", false,
...
void CLGLWindow::FPSLimitCallback() {
  Gtk::ToggleButton *fpslockButton;
  _refXml->get_widget("FPSLimit", fpslockButton);
  ...
  // if (!_fpsLimit)
  //  _renderTimeout = Glib::signal_idle().connect(sigc::mem_fun(this,
  //  &CLGLWindow::CallBackIdleFunc));
  // else if (_fpsLimitValue != 0)
  //  _renderTimeout = Glib::signal_timeout().connect(sigc::mem_fun(this,
  //  &CLGLWindow::CallBackIdleFunc), 1000 / _fpsLimitValue,
  //  Glib::PRIORITY_DEFAULT_IDLE);
}

void CLGLWindow::aboutCallback() {
...
namespace {
struct IterFinder {
  IterFinder(std::shared_ptr<RenderObj> selected,
             Gtk::TreeModelColumn<RenderObj *> col)
      : _selected(selected), _col(col) {}
...
void CLGLWindow::performPicking(int x, int y) {
  _camera._renderTarget.attach();
  glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

  // Perform unique coloring of screen objects, note that the value 0 is no
  // object picked
  uint32_t offset = 1;
  ...
  size_t _selectedObjectGlobalID =
      pixel[0] + 256 * (pixel[1] + 256 * (pixel[2] + 256 * pixel[3]));

  offset = 1;
  for (auto &obj : _renderObjsTree._renderObjects) {
    const uint32_t n_objects = obj->pickableObjectCount();

    if ((_selectedObjectGlobalID >= offset) &&
        (_selectedObjectGlobalID - offset) < n_objects) {
  ...
  if (_selectedObject) {
    ...
    _renderObjsTree._store->foreach_iter(
        sigc::mem_fun(&finder, &IterFinder::check));

    if (finder._iter)
      _renderObjsTree._view->get_selection()->select(finder._iter);
  } else if (_cameraMode == ROTATE_POINT)
...
void CLGLWindow::setUpdateRateUnitToSteps(size_t defaultsteps) {
...
void CLGLWindow::cameraModeCallback() {
  switch (_cameraMode) {
  case ROTATE_CAMERA:
...
void CLGLWindow::addLightCallback() {
  std::shared_ptr<RLight> light(new RLight(
      "Light", Vector{0, 1, 0} * 50 / _camera.getRenderScale(), Vector{0, 0, 0},
      0.1f, 300.0f, Vector{0, 1, 0}, _camera.getRenderScale()));
  _renderObjsTree._renderObjects.push_back(light);
  _renderObjsTree._renderObjects.back()->init(_systemQueue);
  _renderObjsTree.buildRenderView();
}

void CLGLWindow::addFunctionCallback() {
...
void CLGLWindow::guiUpdateCallback() {
  ...
  { // Sim Update Frequency Control
    ...
    try {
      _updateIntervalValue = boost::lexical_cast<float>(updateFreq->get_text());
    } catch (...) {
    }
  }
  ...
  { // OpenVR
    ...
    if (newMode != _openVRMode) {
      if (newMode) {
        // Init OpenVR
        _openVR.setLog(
            [=](std::string line) { vrlog->insert_at_cursor(line + "\n"); });
        _openVR.init();
...
#ifdef COIL_wiimote
  {
    ...
    if ((magnet::TrackWiimote::getInstance()).connected()) {
      ...
      batteryBar->set_fraction(
          (magnet::TrackWiimote::getInstance()).getBatteryLevel());
  ...
  std::ostringstream os;
  os << "Coil visualizer (" << _camera.getWidth() << "," << _camera.getHeight()
     << "), Camera Pos (" << pos[0] << "," << pos[1] << "," << pos[2] << ")";
  setWindowtitle(os.str());
}

void CLGLWindow::setSimStatus1(std::string status) {
  Gtk::Label *label;
  _refXml->get_widget("SimDataLabel1", label);

  CoilRegister::getCoilInstance().getTaskQueue().queueTask(
      std::bind(&CLGLWindow::setLabelText, this, label, status));
}

void CLGLWindow::setSimStatus2(std::string status) {
  Gtk::Label *label;
...
void CLGLWindow::setLabelText(Gtk::Label *label, std::string text) {
  label->set_text(text);
}

void CLGLWindow::wiiMoteConnect() {
#ifdef COIL_wiimote
  if ((magnet::TrackWiimote::getInstance()).connected()) {
    guiUpdateCallback();
    return;
  ...
  Gtk::MessageDialog confirmation(
      *window,
      "Place the WiiMote in discovery mode (hit the <b>1</b> &amp; <b>2</b> "
      "buttons together)\nThen hit Ok.",
...
bool CLGLWindow::wiiMoteIRExposeEvent(const Cairo::RefPtr<Cairo::Context> &cr) {
#ifdef COIL_wiimote
  Gtk::DrawingArea *ir;
  _refXml->get_widget("wiiIRImage", ir);
  ...
  // Draw the tracked sources with a red dot, but only if there are just two
  // sources!

  size_t trackeddrawn = 2;
  for (const auto &irdata :
    ...
    float x = ir->get_allocation().get_width() *
              (1 - float(irdata.x) / CWIID_IR_X_MAX);
    float y = ir->get_allocation().get_height() *
              (1 - float(irdata.y) / CWIID_IR_Y_MAX);
...
void CLGLWindow::AAsamplechangeCallback() {
  if (_aasamples.get() != 0)
...
void CLGLWindow::rescaleCameraCallback() {
  ...
  for (auto &obj : _renderObjsTree._renderObjects) {
    std::shared_ptr<RLight> ptr = std::dynamic_pointer_cast<RLight>(obj);
    if (ptr) {
      ptr->setSize(ptr->getSize() * oldScale / newScale);
      ptr->setIntensity(ptr->getIntensity() * (oldScale * oldScale) /
                        (newScale * newScale));
