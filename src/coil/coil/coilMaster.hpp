...
public:
  ...
  template <class T> void addWindow(std::shared_ptr<T> &window) {
    std::shared_ptr<CoilWindow> win =
        std::static_pointer_cast<CoilWindow>(window);
    if (!isRunning())
      M_throw() << "Coil is not running, cannot add a window";
    ...
    if (parallel)
      ...
      while (parallel && (!window->isReady())) {
        ...
        if (!isRunning())
          M_throw()
              << "Coil failed to add the window as the main render thread died";
      }
    else
...
private:
  ...
  //! to access coil!
  inline static CoilMaster &getCoilInstance() { return *_instance; }
