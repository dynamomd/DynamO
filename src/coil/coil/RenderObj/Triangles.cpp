...
void RTriangles::glRender(const magnet::GL::Camera &cam, RenderMode mode,
  ...
  if (mode & RenderObj::PICKING) {
    ...
    _pickingColorBuff.init(vertexColors, 4,
                           magnet::GL::buffer_usage::STREAM_DRAW);
    _pickingColorBuff.attachToColor();
  } else
    _colBuff.attachToColor();

  using namespace magnet::GL::shader::detail;
  Shader &shader = (mode == RenderObj::SHADOW)
                       ? static_cast<Shader &>(_renderVSMShader)
                       : static_cast<Shader &>(_renderShader);
  shader.attach();
...
void RTriangles::setGLColors(const std::vector<GLubyte> &VertexColor) {
  if (!VertexColor.size())
...
void RTriangles::setGLPositions(const std::vector<float> &VertexPos) {
  if (!VertexPos.size())
  ...
  _posBuff.init(VertexPos, _triangleComponents,
                magnet::GL::buffer_usage::STREAM_DRAW);
}

void RTriangles::setGLNormals(const std::vector<float> &VertexNormals) {
...
void RTriangles::setGLElements(const std::vector<GLuint> &Elements) {
  if (!Elements.size())
...
void RTriangles::initGTK() {
  ...
  _gtkTriangleRender->signal_toggled().connect(
      sigc::mem_fun(*this, &RTriangles::guiUpdate));
  _gtkLineRender->signal_toggled().connect(
      sigc::mem_fun(*this, &RTriangles::guiUpdate));
  _gtkPointRender->signal_toggled().connect(
      sigc::mem_fun(*this, &RTriangles::guiUpdate));

  guiUpdate();
}

void RTriangles::showControls(Gtk::ScrolledWindow *win) {
  win->remove();
...
void RTriangles::guiUpdate() {
  RenderModeType rmode = TRIANGLES;
  if (_gtkLineRender->get_active())
    rmode = LINES;
  if (_gtkPointRender->get_active())
    rmode = POINTS;

  setRenderMode(rmode);
}

void RTriangles::setRenderMode(RenderModeType rm) {
  if (rm != _RenderMode) {
    _specialElementBuff.deinit();
