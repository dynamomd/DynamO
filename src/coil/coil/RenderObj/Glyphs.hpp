...
public:
  ...
  virtual std::string initVertexShaderSource() {
    return STRINGIFY(
        ...
        out vec4 f_color; out vec3 f_director; out vec4 f_orientation;
        out vec4 f_scale;
  ...
  virtual std::string initGeometryShaderSource() {
    return STRINGIFY(
        ...
        in vec4 f_color[]; in vec3 f_director[]; in vec4 f_orientation[];
        in vec4 f_scale[];

        // vec4 gl_position is also collected
        flat out vec4 g_color; flat out vec4 g_orientation;
        ...
        void main() {
          ...
          g_scale = f_scale[0].x;
          gl_Position = vec4(
              gl_in[0].gl_Position.xyz + f_scale[0].z * f_director[0], 0.0);
          EmitVertex();
          EndPrimitive();
          ...
          g_scale = f_scale[0].y;
          gl_Position = vec4(
              gl_in[0].gl_Position.xyz - f_scale[0].w * f_director[0], 0.0);
          EmitVertex();
          EndPrimitive();
        });
  }
