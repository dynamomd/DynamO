...
namespace coil {
Glib::RefPtr<Gdk::Pixbuf> DataSet::getIcon() {
  return coil::images::DataSet_Icon();
}

void DataSetChild::request_delete() { _ds.deleteChild(this); }

void DataSet::deleteChildWorker(DataSetChild *childtodelete) {
...
std::string DataSetChild::getCursorText(uint32_t objID) {
...
void DataSet::showControls(Gtk::ScrolledWindow *win) {
  win->remove();
...
void DataSet::initGtk() {
  _gtkOptList.reset(new Gtk::VBox);

  { // The heading of the data set window
    Gtk::Frame *frame = Gtk::manage(new Gtk::Frame("Data Set Information"));
    frame->show();
    ...
    _infolabel.reset(
        new Gtk::Label("Points: " + boost::lexical_cast<std::string>(_N)));
    _infolabel->show();
  ...
  // Glyph adding mechanism
  {
    ...
    _comboPointSet.reset(new Gtk::ComboBoxText);
    _comboPointSet->show();
    ...
    _comboLinkSet.reset(new Gtk::ComboBoxText);
    _comboLinkSet->show();
    ...
    btn->signal_clicked().connect(
        sigc::mem_fun(*this, &DataSet::addLinkGlyphs));
    btn->show();
    box->pack_start(*btn, false, false, 5);
  }

  {
    ...
    _attrtreestore->set_sort_column(_attrcolumns->components,
                                    Gtk::SORT_DESCENDING);
    ...
    Gtk::Frame *frame = Gtk::manage(new Gtk::Frame("Available Attributes"));
    frame->show();
    frame->add(*win);
...
void DataSet::addGlyphs() {
  if (!_context)
    M_throw() << "Cannot add glyphs before the Dataset is initialised";

  std::shared_ptr<Glyphs> glyph(
      new Glyphs(_comboPointSet->get_active_text(), *this));
  _children.push_back(glyph);
  _children.back()->init(_systemQueue);

  if (_iter) {
    Gtk::TreeModel::iterator child_iter =
        _view->_store->append(_iter->children());
    _children.back()->addViewRows(*_view, child_iter);
    _view->_view->expand_to_path(_view->_store->get_path(child_iter));
  }
}

void DataSet::addLinkGlyphs() {}

void DataSet::addAttribute(std::string name, int type, size_t components) {
...
void DataSet::rebuildGui() {
  _attrtreestore->clear();

  _infolabel->set_text(
      "Points: " + boost::lexical_cast<std::string>(_N) +
...
void DataSet::addPointSet(std::string name, const std::vector<GLuint> &data,
                          int datatype) {
  _context->queueTask(
      std::bind(&DataSet::addPointSetWorker, this, name, data, datatype));
}

void DataSet::addPointSetWorker(std::string name, std::vector<GLuint> data,
                                int datatype) {
...
void DataSet::deinit() {
  ...
  for (auto &attribute : _attributes)
    attribute.second->deinit();
  _context.reset();
  RenderObj::deinit();
}

magnet::GL::Buffer<GLfloat> &DataSet::getPositionBuffer() {
  return _positionSel->getBuffer();
}

std::string DataSet::getCursorText(uint32_t objID) {
  std::ostringstream os;
  for (const auto &attribute : _attributes) {
...
magnet::math::NVector<GLfloat, 4> DataSet::getCursorPosition(uint32_t objID) {
  std::vector<GLfloat> pos = _positionSel->getValue(objID);
  pos.resize(3, 0);
  return magnet::math::NVector<GLfloat, 4>{pos[0], pos[1], pos[2], 1.0};
}

magnet::math::Vector DataSet::getMinCoord() const {
...
magnet::math::Vector DataSet::getMaxCoord() const {
