...
void local_move_to(magnet::GL::objects::CairoSurface &cairo,
                   magnet::GL::GLMatrix projViewMatrix, float x, float y,
                   float z) {
...
void Console::interfaceRender(const magnet::GL::Camera &camera,
  ...
  if (_showAxis->get_active()) {
    const GLdouble nearPlane = 0.1, axisScale = 0.09;
...
void Console::glRender(const magnet::GL::Camera &camera, RenderMode mode,
  ...
  if (_showGrid->get_active()) {
    ...
    ////Back face
    // context->setAttribute(Context::instanceOriginAttrIndex, 0, 0,
    // -camera.getScreenPlaneWidth(), 0);
...
void Console::showControls(Gtk::ScrolledWindow *win) {
  win->remove();
  _optList->unparent();
  win->add(*_optList);
  win->show();
