...
RSurface::RSurface(std::string name, size_t N, Vector origin, Vector axis1,
                   Vector axis2, Vector axis3)
    : RTriangles(name), _N(N), _origin(origin), _axis1(axis1), _axis2(axis2),
      _axis3(axis3) {
  // Ensure that the axis have the correct ordering (so the normals
  // aren't messed up as we use CW/CCW vertex ordering).
  if (((axis1 ^ axis2) | axis3) > 0)
    std::swap(_axis1, _axis2);
}
...
magnet::math::Vector RSurface::getMaxCoord() const {
...
magnet::math::Vector RSurface::getMinCoord() const {
