...
Glib::RefPtr<Gdk::Pixbuf> Glyphs::getIcon() {
  return coil::images::Glyphs_Icon();
}

magnet::GL::element_type::Enum Glyphs::getElementType() {
...
magnet::math::NVector<GLfloat, 4> Glyphs::getCursorPosition(uint32_t objID) {
  return _ds.getCursorPosition(objID % _N);
}

std::string Glyphs::getCursorText(uint32_t objID) {
  return _ds.getCursorText(objID % _N);
}

void Glyphs::glRender(const magnet::GL::Camera &cam, RenderMode mode,
  ...
  if ((_glyphRaytrace->get_active()) &&
      ((_glyphType->get_active_row_number() == CYLINDER_GLYPH) ||
       ...
       (_glyphType->get_active_row_number() == DUMBBELL_GLYPH)))
    instancing = 0;

  if (mode == RenderObj::PICKING) { // Send unique color id's to colorbuf
    std::vector<GLubyte> colors;
    colors.resize(4 * _N);
    for (uint32_t i(0); i < _N; ++i)
      *reinterpret_cast<uint32_t *>(&(colors[4 * i])) = offset + i;
    colorbuf.init(colors, 4);
    colorbuf.attachToAttribute(magnet::GL::Context::vertexColorAttrIndex,
                               instancing, true);
  } else
    _colorSel->bindAttribute(magnet::GL::Context::vertexColorAttrIndex,
                             instancing);

  _scaleSel->bindAttribute(magnet::GL::Context::instanceScaleAttrIndex,
                           instancing);
  _orientSel->bindAttribute(magnet::GL::Context::instanceOrientationAttrIndex,
                            instancing);
  ...
  if (_glyphRaytrace->get_active()) {
    ...
    switch (_glyphType->get_active_row_number()) {
      // Select the shader needed for the ray traced object, then ray trace it
      // if needed:
    case CYLINDER_GLYPH:
      shader_ptr =
          (mode == RenderObj::SHADOW) ? &_cylinderVSMShader : &_cylinderShader;
      shader_ptr->defines("ROD") = "";
    case ROD_GLYPH:
      if (shader_ptr == nullptr) {
        shader_ptr = (mode == RenderObj::SHADOW) ? &_cylinderVSMShader
                                                 : &_cylinderShader;
    ...
    case SPHERE_GLYPH: {
      if (shader_ptr == nullptr)
        shader_ptr =
            (mode == RenderObj::SHADOW) ? &_sphereVSMShader : &_sphereShader;
      ...
      if (_glyphType->get_active_row_number() == DUMBBELL_GLYPH) {
        ...
        glVertexAttribPointer(magnet::GL::Context::vertexPositionAttrIndex, 4,
                              GL_FLOAT, GL_FALSE, 13 * sizeof(GLfloat),
                              (GLvoid *)(0));
        glVertexAttribPointer(magnet::GL::Context::vertexColorAttrIndex, 4,
                              GL_FLOAT, GL_FALSE, 13 * sizeof(GLfloat),
                              (GLvoid *)(4 * sizeof(GLfloat)));
        glVertexAttribPointer(magnet::GL::Context::instanceOrientationAttrIndex,
                              4, GL_FLOAT, GL_FALSE, 13 * sizeof(GLfloat),
                              (GLvoid *)(8 * sizeof(GLfloat)));
        glVertexAttribPointer(magnet::GL::Context::instanceScaleAttrIndex, 1,
                              GL_FLOAT, GL_FALSE, 13 * sizeof(GLfloat),
      ...
      shader.defines("DRAWBILLBOARD") =
          _drawbillboards->get_active() ? "true" : "";
      shader.attach();
      ...
      for (int x(-_ximages); x <= _ximages; ++x)
        for (int y(-_yimages); y <= _yimages; ++y)
          for (int z(-_zimages); z <= _zimages; ++z) {
            ...
            shader["ViewMatrix"] =
                cam.getViewMatrix() * magnet::GL::translate(displacement);
  ...
  shader.attach();
  shader["ProjectionMatrix"] = cam.getProjectionMatrix();
  _ds.getPositionBuffer().attachToAttribute(
      magnet::GL::Context::instanceOriginAttrIndex, 1);
...
void Glyphs::showControls(Gtk::ScrolledWindow *win) {
  win->remove();
...
void Glyphs::init(
  ...
  {
    Gtk::Label *label = Gtk::manage(new Gtk::Label("Glyph Type"));
    label->show();
  ...
  {
    _glyphLOD.reset(new Gtk::SpinButton(1.0, 0));
    _glyphLOD->show();
    _glyphLOD->set_numeric(true);
    _glyphBox->pack_end(*_glyphLOD, false, false, 5);
    Gtk::Label *label = Gtk::manage(new Gtk::Label("Level of Detail"));
    label->show();
  ...
  _scaleSel.reset(
      new AttributeSelector(magnet::GL::Context::instanceScaleAttrIndex));
  ...
  _colorSel->buildEntries("Color Data Field:", _ds, 1, 4,
                          Attribute::INTENSIVE | Attribute::EXTENSIVE, 4,
                          Attribute::DEFAULT_GLYPH_COLOUR);
  ...
  _glyphRaytrace->signal_toggled().connect(
      sigc::mem_fun(this, &Glyphs::guiUpdate));
  _glyphLOD->signal_value_changed().connect(
      sigc::mem_fun(*this, &Glyphs::guiUpdate));
  _glyphType->signal_changed().connect(
      sigc::mem_fun(*this, &Glyphs::glyph_type_changed));
  _scaleFactor->signal_changed().connect(
      sigc::mem_fun(*this, &Glyphs::guiUpdate));
  ...
  {
    Gtk::Label *label = Gtk::manage(new Gtk::Label("x"));
    label->show();
  ...
  {
    Gtk::Label *label = Gtk::manage(new Gtk::Label("y"));
    label->show();
    periodicbox->pack_start(*label, false, false, 5);
  }
  ...
  {
    Gtk::Label *label = Gtk::manage(new Gtk::Label("z"));
    label->show();
    periodicbox->pack_start(*label, false, false, 5);
  }
  ...
  {
    Gtk::Label *label = Gtk::manage(new Gtk::Label("Developer options"));
    label->show();
...
void Glyphs::glyph_type_changed() {
...
void Glyphs::guiUpdate() {
  ...
  _primitiveVertices.init(getPrimitiveVertices(), 3,
                          magnet::GL::buffer_usage::STATIC_DRAW);
  _primitiveNormals.init(getPrimitiveNormals(), 3,
                         magnet::GL::buffer_usage::STATIC_DRAW);
  _primitiveIndices.init(
      getPrimitiveIndicies(),
      (_glyphType->get_active_row_number() == LINE_GLYPH) ? 2 : 3,
      magnet::GL::buffer_usage::STATIC_DRAW);
}

std::vector<GLfloat> Glyphs::getPrimitiveVertices() {
  ...
  case SPHERE_GLYPH:
  case DUMBBELL_GLYPH: {
    magnet::GL::objects::primitives::Sphere sph(
        magnet::GL::objects::primitives::Sphere::icosahedron, LOD);
    vertices = std::vector<GLfloat>(
        sph.getVertices(), sph.getVertices() + sph.getVertexCount() * 3);
    break;
  }
  case ARROW_GLYPH:
...
std::vector<GLfloat> Glyphs::getPrimitiveNormals() {
  int LOD = _glyphLOD->get_value_as_int();
  int type = _glyphType->get_active_row_number();
  ...
  case DUMBBELL_GLYPH: {
    magnet::GL::objects::primitives::Sphere sph(
        magnet::GL::objects::primitives::Sphere::icosahedron, LOD);
    return std::vector<GLfloat>(sph.getVertices(),
                                sph.getVertices() + sph.getVertexCount() * 3);
  }
  case ARROW_GLYPH:
...
std::vector<GLuint> Glyphs::getPrimitiveIndicies() {
  int LOD = _glyphLOD->get_value_as_int();
  int type = _glyphType->get_active_row_number();
  ...
  case DUMBBELL_GLYPH: {
    magnet::GL::objects::primitives::Sphere sph(
        magnet::GL::objects::primitives::Sphere::icosahedron, LOD);
    return std::vector<GLuint>(sph.getFaces(),
                               sph.getFaces() + sph.getFaceCount() * 3);
  }
  case ARROW_GLYPH:
...
magnet::math::Vector Glyphs::getMaxCoord() const {
...
magnet::math::Vector Glyphs::getMinCoord() const {
  int images[3] = {_xperiodicimages->get_value_as_int(),
                   _yperiodicimages->get_value_as_int(),
                   _zperiodicimages->get_value_as_int()};
