...
public:
  enum AttributeType {
    ...
    COORDINATE = 1 << 2, //!< A special attribute which specifies the location
                         //!< of the attribute.

    DEFAULT_GLYPH_POSITION =
        1 << 3, //!< This flag marks that the attribute should be used as the
                //!< initial position value for a glyph.
    DEFAULT_GLYPH_SCALING =
        1 << 4, //!< This flag marks that the attribute should be used as the
                //!< initial scaling field for a glyph.
    DEFAULT_GLYPH_ORIENTATION =
        1 << 5, //!< This flag marks that the attribute should be used as the
                //!< initial orientation for a glyph.
    DEFAULT_GLYPH_COLOUR =
        1 << 6 //!< This flag marks that the attribute should be used as the
               //!< initial colouring for a glyph.
  };

  inline Attribute(size_t N, int type, size_t components,
                   magnet::GL::Context::ContextPtr context)
      : std::vector<GLfloat>(N * components), _usedInLastRender(false),
        _usedInCurrentRender(false), _context(context), _dataUpdates(0),
        _components(components), _type(type), _references(0) {
    if (_components > 4)
      M_throw() << "We don't support greater than 4 component attributes due "
                   "to the way "
                << "data is sometimes directly passed to the shaders (e.g. "
                   "positional data)";
  }
