...
public:
  RLight(std::string name, magnet::math::Vector position,
         magnet::math::Vector lookAtPoint, GLfloat zNearDist = 8.0f,
         GLfloat zFarDist = 10000.0f,
         magnet::math::Vector up = magnet::math::Vector{0, 1, 0},
         GLfloat simLength = 25.0f, GLfloat size = 0.2)
      : RenderObj(name),
        CameraHeadTracking(position, lookAtPoint, zNearDist, zFarDist, up,
                           simLength, magnet::math::Vector{0, 0, 20}),
        _intensity(1.0), _specularExponent(32), _specularFactor(1),
        _maxvariance(0.1), _bleedreduction(0.2), _size(size) {
  ...
  */
   ...
   */
  magnet::math::Vector
  getEyespacePosition(const magnet::GL::Camera &camera) const {
    magnet::math::Vector vec = getPosition();
    magnet::math::NVector<GLfloat, 4> lightPos = {
        {GLfloat(vec[0]), GLfloat(vec[1]), GLfloat(vec[2]), 1.0f}};
  ...
  virtual uint32_t pickableObjectCount() { return visible(); }

  virtual std::string getCursorText(uint32_t objID) { return _name; }

  virtual magnet::math::NVector<GLfloat, 4> getCursorPosition(uint32_t objID) {
    magnet::math::Vector loc = getPosition();
    return magnet::math::NVector<GLfloat, 4>{GLfloat(loc[0]), GLfloat(loc[1]),
                                             GLfloat(loc[2]), 1.0f};
  }
