...
namespace coil {
Glib::RefPtr<Gdk::Pixbuf> RVolume::getIcon() {
...
void RVolume::loadRawFile(std::string filename, std::array<size_t, 3> data_dim,
                          size_t bytes, Vector dims) {
  std::ifstream file(filename.c_str(), std::ifstream::binary);
  std::vector<uint8_t> filebuffer(data_dim[0] * data_dim[1] * data_dim[2] *
                                  bytes);
  file.read(reinterpret_cast<char *>(&filebuffer[0]), filebuffer.size());
  if (file.fail())
    M_throw() << "Failed to load the texture from the file, possible incorrect "
                 "dimensions ";
    ...
    for (size_t y(0); y < data_dim[1]; ++y)
      for (size_t z(0); z < data_dim[2]; ++z) {
        ...
        for (size_t byte(0); byte < bytes; ++byte)
          val += filebuffer[(x + (y + z * data_dim[1]) * data_dim[0]) * bytes +
                            byte]
                 << (8 * (bytes - byte));
  ...
  std::cout << "min=" << min << ", max=" << max << ", shift=" << shift
            << std::endl;
...
#ifdef COIL_TIFFSUPPORT
void RVolume::loadTIFFFiles(std::vector<std::string> files) {
  ...
  for (size_t z(0); z < size; ++z)
    for (size_t y(0); y < size; ++y)
      for (size_t x(0); x < size; ++x)
        inbuffer[x + size * (y + size * z)] = std::sqrt(
            std::pow(x - size / 2.0, 2) + std::pow(y - size / 2.0, 2) +
            std::pow(z - size / 2.0, 2));

  loadData(inbuffer, std::array<size_t, 3>{{size, size, size}},
           Vector{1, 1, 1});
}

namespace {
inline int coordCalc(GLint x, GLint y, GLint z, GLint width, GLint height,
                     GLint depth, const std::vector<GLubyte> &buffer) {
...
void RVolume::loadData(const std::vector<GLubyte> &inbuffer,
                       std::array<size_t, 3> dim, Vector d) {
  _dimensions = d;

  if (_dimensions * _dimensions == 0) {
    size_t maxdim = std::max(dim[0], std::max(dim[1], dim[2]));
    _dimensions = Vector{double(dim[0]) / maxdim, double(dim[1]) / maxdim,
                         double(dim[2]) / maxdim};
  }
  ...
  for (int z(0); z < int(depth); ++z)
    for (int y(0); y < int(height); ++y)
      for (int x(0); x < int(width); ++x) {
        Vector sample1{
            double(coordCalc(x - 1, y, z, width, height, depth, inbuffer)),
  ...
  {
    float logMaxVal =
        std::log(*std::max_element(histogram.begin(), histogram.end()));
    float logMinVal = std::log(
        std::max(*std::min_element(histogram.begin(), histogram.end()), 1.0f));
...
void RVolume::forwardRender(magnet::GL::FBO &fbo,
                            const magnet::GL::Camera &camera,
                            std::vector<std::shared_ptr<RLight>> &lights,
                            GLfloat ambient, RenderMode mode) {
  if (!_visible || !_data.isValid())
    return;
  if (lights.empty())
    return;

  // Before we render, we need the current depth buffer so we can test against
  // it
  fbo.detach();
  ...
  for (std::vector<std::shared_ptr<RLight>>::const_iterator iPtr =
           lights.begin();
       iPtr != lights.end(); ++iPtr) {
    ...
    light_factors.push_back(Vector{0.0, (*iPtr)->getSpecularExponent(),
  ...
  _currentDepthFBO.getContext().setAttribute(
      magnet::GL::Context::instanceScaleAttrIndex, _dimensions[0],
      _dimensions[1], _dimensions[2], 1);
...
void RVolume::transferFunctionUpdated() {
  if (_transferFunction.get() != NULL) {
    size_t samples = 256;
    float transmittanceFactor = 1000;

    std::vector<float> data =
        _transferFunction->getMap(samples, transmittanceFactor);
...
void RVolume::initGTK() {
  _optList.reset(new Gtk::VBox); // The Vbox of options

  { // Transfer function widget
    _transferFunction.reset(new magnet::gtk::TransferFunction(
        magnet::Delegate<void()>::create<
            RVolume, &RVolume::transferFunctionUpdated>(this)));
    _transferFunction->set_size_request(-1, 100);

    _optList->add(*_transferFunction);
    _transferFunction->show();
    transferFunctionUpdated(); // Force an update of the transfer function now
                               // we have the widget
  }
  ...
  _optList->show();
  // Callbacks
  _stepSize->signal_changed().connect(
      sigc::bind(&magnet::gtk::forceNumericEntry, _stepSize.get()));
  _stepSize->signal_activate().connect(
      sigc::mem_fun(*this, &RVolume::guiUpdate));

  _filterData->signal_toggled().connect(
      sigc::mem_fun(*this, &RVolume::guiUpdate));

  _preintegrate->signal_toggled().connect(
      sigc::mem_fun(*this, &RVolume::guiUpdate));

  guiUpdate();
}

void RVolume::showControls(Gtk::ScrolledWindow *win) {
  win->remove();
  _optList->unparent();
  win->add(*_optList);
  win->show();
...
void RVolume::guiUpdate() {
  std::string val = _stepSize->get_text();
  if (val.empty()) {
    val = "0.01";
    _stepSize->set_text("0.01");
  }
...
void RVolume::xml(stator::xml::Node xml) {
