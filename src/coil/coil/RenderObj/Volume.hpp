...
#define STRINGIFY(A) #A
    virtual std::string initFragmentShaderSource() {
      return STRINGIFY(uniform sampler2D depthTex; void main() {
...
public:
  RVolume(std::string name)
      : RenderObj(name), _stepSizeVal(0.01), _dimensions({1, 1, 1}),
        _frameCounter(0) {}
  ...
  void loadRawFile(std::string filename, std::array<size_t, 3> dimensions,
                   size_t bytes, Vector dims = Vector{0, 0, 0});
  ...
  virtual magnet::math::Vector getMaxCoord() const {
    return +0.5 * _dimensions;
  }
  virtual magnet::math::Vector getMinCoord() const {
    return -0.5 * _dimensions;
  }

  void loadData(const std::vector<GLubyte> &inbuffer, std::array<size_t, 3> dim,
                Vector scale);
