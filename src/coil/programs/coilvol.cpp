...
int main(int argc, char *argv[]) {
  try {
    ...
    opts.add_options()("help,h", "Produces this message.")(
    ...
    if (vm.count("help")) {
      cout << "Usage : coilvol <OPTIONS>...[DATA FILE]\n"
           ...
           << "of the data set. Alternatively, just specify a list of TIFF "
              "files to be stacked."
           << opts;
      return 1;
    }
    ...
    std::shared_ptr<coil::CLGLWindow> window(
        new coil::CLGLWindow("Coil Volume Renderer : ", 1.0));
    coil.getInstance().addWindow(window);

    if (vm.count("data-file")) {
      std::vector<std::string> files =
          vm["data-file"].as<std::vector<std::string>>();
      std::shared_ptr<coil::RVolume> voldata(new coil::RVolume("Volume data"));
      window->addRenderObj(voldata);

      if (files.size() == 1) {
        size_t datasize[3] = {vm["x-elements"].as<size_t>(),
                              vm["y-elements"].as<size_t>(),
                              vm["z-elements"].as<size_t>()};
        window->getGLContext()->queueTask(std::bind(
            &coil::RVolume::loadRawFile, voldata.get(), files[0],
            std::array<size_t, 3>{{datasize[0], datasize[1], datasize[2]}},
            vm["data-size"].as<size_t>(), magnet::math::Vector{0, 0, 0}));
      } else {
#ifdef COIL_TIFFSUPPORT
        std::cout << "Loading " << vm.count("data-file") << " datafiles"
                  << std::endl;
        window->getGLContext()->queueTask(
            std::bind(&coil::RVolume::loadTIFFFiles, voldata.get(), files));
#else
        M_throw() << "Loading multiple images is only supported if TIFF "
                     "support is built in";
#endif
      }
    }
