...
void init(dynamo::Simulation &Sim, const double density) {
  ...
  std::unique_ptr<dynamo::UCell> packptr(
      new dynamo::CUFCC(std::array<long, 3>{{10, 10, 10}},
                        dynamo::Vector{1, 1, 1}, new dynamo::UParticle()));
  packptr->initialise();
  ...
  Sim.interactions.push_back(
      dynamo::shared_ptr<dynamo::Interaction>(new dynamo::IHardSphere(
          &Sim, particleDiam,
          new dynamo::IDPairRangeSingle(new dynamo::IDRangeRange(0, Na - 1)),
          "AAInt")));

  Sim.interactions.push_back(
      dynamo::shared_ptr<dynamo::Interaction>(new dynamo::IHardSphere(
          &Sim, ((1.0 + sizeRatio) / 2.0) * particleDiam,
          new dynamo::IDPairRangePair(
              new dynamo::IDRangeRange(0, Na - 1),
              new dynamo::IDRangeRange(Na, latticeSites.size() - 1)),
          "ABInt")));

  Sim.interactions.push_back(dynamo::shared_ptr<dynamo::Interaction>(
      new dynamo::IHardSphere(&Sim, sizeRatio * particleDiam,
                              new dynamo::IDPairRangeAll(), "BBInt")));

  Sim.addSpecies(dynamo::shared_ptr<dynamo::Species>(new dynamo::SpPoint(
      &Sim, new dynamo::IDRangeRange(0, Na - 1), 1.0, "A", 0)));

  Sim.addSpecies(dynamo::shared_ptr<dynamo::Species>(new dynamo::SpPoint(
      &Sim, new dynamo::IDRangeRange(Na, latticeSites.size() - 1), massFrac,
      "B", 0)));
  ...
  // 0.000000001); BOOST_CHECK_CLOSE(Sim.getPackingFraction(),
  // Sim.getNumberDensity() * Sim.units.unitVolume() * M_PI / 6.0, 0.000000001);
}
...
BOOST_AUTO_TEST_CASE(Equilibrium_Simulation) {
  ...
  // Sim.units.unitDiffusion(); BOOST_CHECK_CLOSE(D, expectedD, 6);
...
//   Sim.addOutputPlugin("Misc");
//
//   dynamo::shared_ptr<dynamo::IPCompression> compressPlug(new
//   dynamo::IPCompression(&Sim, growthRate)); compressPlug->MakeGrowth();
//
...
//   targetDensity, 0.000000001); BOOST_CHECK_CLOSE(Sim.getPackingFraction(),
//   Sim.getNumberDensity() * Sim.units.unitVolume() * M_PI / 6.0, 0.000000001);
//
//   BOOST_CHECK_MESSAGE(Sim.checkSystem() <= 1, "After compression, there are
//   more than one invalid states in the final configuration");
// }
