...
BOOST_AUTO_TEST_CASE( Initialisation )
{
  ...
  if (std::dynamic_pointer_cast<dynamo::SNeighbourList>(Sim.ptrScheduler))
    {
      if (std::dynamic_pointer_cast<dynamo::BCLeesEdwards>(Sim.BCs))
	Sim.globals.push_back(dynamo::shared_ptr<dynamo::Global>(new dynamo::GCellsShearing(&Sim,"SchedulerNBList")));
      else
