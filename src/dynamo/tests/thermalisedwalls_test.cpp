...
void init(dynamo::Simulation &Sim, const double density) {
  ...
  std::unique_ptr<dynamo::UCell> packptr(
      new dynamo::CUFCC(std::array<long, 3>{{cells, cells, cells}},
  ...
  Sim.interactions.push_back(
      dynamo::shared_ptr<dynamo::Interaction>(new dynamo::IHardSphere(
          &Sim, "D", elasticity, new dynamo::IDPairRangeAll(), "Bulk")));
  ...
  Sim.locals.push_back(dynamo::shared_ptr<dynamo::Local>(
      new dynamo::LWall(&Sim, elasticity, "D", dynamo::Vector{1, 0, 0},
                        dynamo::Vector{-0.5 * Sim.primaryCellSize[0] - 1, 0, 0},
                        "XwallLow", new dynamo::IDRangeAll(&Sim))));
  Sim.locals.push_back(dynamo::shared_ptr<dynamo::Local>(
      new dynamo::LWall(&Sim, elasticity, "D", dynamo::Vector{-1, 0, 0},
                        dynamo::Vector{0.5 * Sim.primaryCellSize[0] + 1, 0, 0},
                        "XwallHigh", new dynamo::IDRangeAll(&Sim))));

  Sim.locals.push_back(dynamo::shared_ptr<dynamo::Local>(
      new dynamo::LWall(&Sim, elasticity, "D", dynamo::Vector{0, 0, 1},
                        dynamo::Vector{0, 0, -0.5 * Sim.primaryCellSize[2] - 1},
                        "ZwallLow", new dynamo::IDRangeAll(&Sim))));
  Sim.locals.push_back(dynamo::shared_ptr<dynamo::Local>(
      new dynamo::LWall(&Sim, elasticity, "D", dynamo::Vector{0, 0, -1},
                        dynamo::Vector{0, 0, 0.5 * Sim.primaryCellSize[2] + 1},
                        "ZwallHigh", new dynamo::IDRangeAll(&Sim))));

  Sim.locals.push_back(dynamo::shared_ptr<dynamo::Local>(
      new dynamo::LWall(&Sim, elasticity, "D", dynamo::Vector{0, 1, 0},
                        dynamo::Vector{0, -0.5 * Sim.primaryCellSize[1] - 1, 0},
                        "GroundPlate", new dynamo::IDRangeAll(&Sim), wallkT)));

  for (size_t i(0); i < latticeSites.size(); ++i) {
    Sim.particles.push_back(dynamo::Particle(
        boxL * latticeSites[i], getRandVelVec(), Sim.particles.size()));
