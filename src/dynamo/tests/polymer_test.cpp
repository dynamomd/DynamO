...
void init(dynamo::Simulation &Sim) {
  ...
  Sim.interactions.push_back(
      dynamo::shared_ptr<dynamo::Interaction>(new dynamo::ISquareBond(
          &Sim, bondinner, bondouter / bondinner, elasticity,
          new dynamo::IDPairRangeChains(0, N - 1, N), "Bonds")));
  Sim.interactions.push_back(dynamo::shared_ptr<dynamo::Interaction>(
      new dynamo::ISquareWell(&Sim, diameter, lambda, welldepth, elasticity,
  ...
  for (size_t i = 0; i < N; ++i)
    Sim.particles.push_back(dynamo::Particle(
        dynamo::Vector{0 + (bondinner + 0.95 * (bondouter - bondinner)) * i, 0,
                       0},
        getRandVelVec() * Sim.units.unitVelocity(), Sim.particles.size()));

  Sim.systems.push_back(dynamo::shared_ptr<dynamo::System>(
      new dynamo::SysAndersen(&Sim, 0.001 / Sim.N(), kT, "Thermostat")));
...
BOOST_AUTO_TEST_CASE(Equilibrium_Simulation) {
  ...
  BOOST_CHECK_MESSAGE(
      Sim.checkSystem() <= 2,
      "There are more than three invalid states in the final configuration");
}
