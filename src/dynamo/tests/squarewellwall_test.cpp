...
void init(dynamo::Simulation &Sim) {
  ...
  Sim.locals.push_back(dynamo::shared_ptr<dynamo::Local>(new dynamo::LWall(
      &Sim, 1.0, 1.0, dynamo::Vector{1, 0, 0}, dynamo::Vector{-3, 0, 0},
      "LowWall", new dynamo::IDRangeAll(&Sim))));

  Sim.locals.push_back(dynamo::shared_ptr<dynamo::Local>(new dynamo::LWall(
      &Sim, 1.0, 1.0, dynamo::Vector{-1, 0, 0}, dynamo::Vector{3, 0, 0},
      "HighWall", new dynamo::IDRangeAll(&Sim))));

  Sim.interactions.push_back(
      dynamo::shared_ptr<dynamo::Interaction>(new dynamo::ISquareWell(
          &Sim, 1, 1.5, 1, 1, new dynamo::IDPairRangeAll(), "Bulk")));

  Sim.particles.push_back(dynamo::Particle(dynamo::Vector{0.75, 0, 0},
                                           dynamo::Vector{2, 0, 0},
                                           Sim.particles.size()));
  Sim.particles.push_back(dynamo::Particle(dynamo::Vector{-0.75, 0, 0},
                                           dynamo::Vector{-2, 0, 0},
...
BOOST_AUTO_TEST_CASE(Null_Compression_Simulation) {
  ...
  dynamo::shared_ptr<dynamo::IPCompression> compressPlug(
      new dynamo::IPCompression(&Sim, 0.0));
