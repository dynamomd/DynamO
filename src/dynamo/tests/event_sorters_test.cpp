...
dynamo::Event
genInteractionEvent(size_t N, const double meanFreeTime = 1.0,
                    const size_t interactions = 2,
                    size_t p1ID = std::numeric_limits<size_t>::max()) {
  const double dt =
      -meanFreeTime * std::log(1.0 - std::uniform_real_distribution<>()(RNG));
  ...
  return dynamo::Event(p1ID, dt, dynamo::INTERACTION, dynamo::CORE,
                       iIDdist(RNG), p2ID);
}

template <class T>
std::vector<dynamo::Event> fillSorter(T &sorter, const size_t totalEvents,
                                      const size_t N) {
...
typedef boost::mpl::list<dynamo::HeapPEL, dynamo::MinMaxPEL<2>,
                         dynamo::MinMaxPEL<3>, dynamo::MinMaxPEL<4>>
    PEL_types;
...
typedef boost::mpl::list<dynamo::ReferenceFEL, dynamo::CBTFEL<dynamo::HeapPEL>,
...
#define validateEvents(e1, e2)                                                 \
  ...
  if (e1._source != dynamo::INTERACTION)                                       \
    BOOST_REQUIRE_EQUAL(e1._additionalData2, e2._additionalData2);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(FEL_standard, T, FEL_types) {
  ...
  while (!reference.empty()) {
    ...
    if ((nextEvent._dt == std::numeric_limits<float>::infinity()) &&
        FEL.empty())
      break;
    BOOST_REQUIRE(!FEL.empty());
  ...
  while (!reference.empty()) {
    ...
    auto test = [=](const dynamo::Event &e) {
      return (e._particle1ID == testEvent._particle1ID) ||
             ((e._source == dynamo::INTERACTION) &&
              (e._particle2ID == testEvent._particle1ID));
    };
    reference.erase(std::remove_if(reference.begin(), reference.end(), test),
                    reference.end());
  ...
  for (size_t i(0); (i < N * eventsPerParticle) && (!reference.empty()); ++i) {
    ...
    auto test = [=](const dynamo::Event &e) {
      return (e._particle1ID == testEvent._particle1ID) ||
             (e._particle1ID == testEvent._particle2ID) ||
             ((e._source == dynamo::INTERACTION) &&
              ((e._particle2ID == testEvent._particle1ID) ||
               (e._particle2ID == testEvent._particle2ID)));
    };
    reference.erase(std::remove_if(reference.begin(), reference.end(), test),
                    reference.end());
    ...
    for (size_t j(0); j < eventsPerParticle; j++) {
      const dynamo::Event newEvent =
          genInteractionEvent(N, 1.0, 1, testEvent._particle1ID);
      FEL.push(newEvent);
    ...
    for (size_t j(0); j < eventsPerParticle; j++) {
      const dynamo::Event newEvent =
          genInteractionEvent(N, 1.0, 1, testEvent._particle2ID);
      FEL.push(newEvent);
      reference.push_back(newEvent);
    }
  }
