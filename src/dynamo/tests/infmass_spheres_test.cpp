...
void init(dynamo::Simulation &Sim, const double density) {
  ...
  std::unique_ptr<dynamo::UCell> packptr(
      new dynamo::CUFCC(std::array<long, 3>{{7, 7, 7}}, dynamo::Vector{L, L, L},
  ...
  Sim.addSpecies(
      dynamo::shared_ptr<dynamo::Species>(new dynamo::SpFixedCollider(
          &Sim, new dynamo::IDRangeRange(0, 1), "HeavySpheres", 0)));

  Sim.addSpecies(dynamo::shared_ptr<dynamo::Species>(new dynamo::SpPoint(
      &Sim, new dynamo::IDRangeRange(2, 1371), 1.0, "Bulk", 1)));

  Sim.interactions.push_back(
      dynamo::shared_ptr<dynamo::Interaction>(new dynamo::IHardSphere(
          &Sim, 1.0, 1,
          new dynamo::IDPairRangeSingle(new dynamo::IDRangeRange(0, 1)),
          "HeavyHeavy")));

  Sim.interactions.push_back(
      dynamo::shared_ptr<dynamo::Interaction>(new dynamo::IHardSphere(
          &Sim, 0.6, 1,
          new dynamo::IDPairRangePair(new dynamo::IDRangeRange(0, 1),
                                      new dynamo::IDRangeRange(2, 1371)),
          "HeavyBulk")));

  Sim.interactions.push_back(
      dynamo::shared_ptr<dynamo::Interaction>(new dynamo::IHardSphere(
          &Sim, 0.2, 1, new dynamo::IDPairRangeAll(), "BulkBulk")));
...
BOOST_AUTO_TEST_CASE(Equilibrium_Simulation) {
  ...
  BOOST_CHECK_CLOSE(MFT, expectedMFT, 5);
  // 6.2715616026913548
  BOOST_CHECK_MESSAGE(
      Sim.checkSystem() <= 1,
...
BOOST_AUTO_TEST_CASE(Compression_Simulation) {
  ...
  // BOOST_CHECK_CLOSE(Sim.getPackingFraction(), Sim.getNumberDensity() *
