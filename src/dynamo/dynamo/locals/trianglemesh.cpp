...
Event LTriangleMesh::getEvent(const Particle &part) const {
  ...
  std::pair<double, size_t> tmin(std::numeric_limits<float>::infinity(),
                                 0); // Default to no collision

  for (size_t id(0); id < _elements.size(); ++id) {
    std::pair<double, size_t> t = Sim->dynamics->getSphereTriangleEvent(
        part, _vertices[std::get<0>(_elements[id])],
        _vertices[std::get<1>(_elements[id])],
        _vertices[std::get<2>(_elements[id])], diam);
    if (t < tmin) {
      tmin = t;
      triangleid = id;
    }
...
ParticleEventData LTriangleMesh::runEvent(Particle &part,
...
void LTriangleMesh::operator<<(const magnet::xml::Node &XML) {
  ...
  { // Load the vertex coordinates
    ...
    while (!is.eof()) {
      is >> tmp[0];
      if (is.eof())
        M_throw() << "The vertex coordinates is not a multiple of 3";
  ...
  { // Load the triangle elements
    ...
    while (!is.eof()) {
      is >> std::get<0>(tmp);
      if (is.eof())
        M_throw() << "The triangle elements are not a multiple of 3";
      ...
      if ((std::get<0>(tmp) >= _vertices.size()) ||
        ...
        M_throw() << "Triangle " << _elements.size()
                  << " has an out of range vertex ID";
...
void LTriangleMesh::outputXML(magnet::xml::XmlStream &XML) const {
  XML << magnet::xml::attr("Type") << "TriangleMesh"
      << magnet::xml::attr("Name") << localName
      << magnet::xml::attr("Elasticity") << _e->getName()
      << magnet::xml::attr("Diameter") << _diameter->getName() << range;
  ...
  for (TriangleElements elements : _elements)
    XML << std::get<0>(elements) << " " << std::get<1>(elements) << " "
        << std::get<2>(elements) << "\n";
...
shared_ptr<coil::RenderObj> LTriangleMesh::getCoilRenderObj() const {
  if (!_renderObj) {
