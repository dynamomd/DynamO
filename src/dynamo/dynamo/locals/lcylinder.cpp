...
Event LCylinder::getEvent(const Particle &part) const {
  ...
  return Event(
      part,
      Sim->dynamics->getCylinderWallCollision(part, vPosition, vAxis, colldist),
      LOCAL, WALL, ID);
}

ParticleEventData LCylinder::runEvent(Particle &part,
                                      const Event &iEvent) const {
  ++Sim->eventCount;
  return Sim->dynamics->runCylinderWallCollision(part, vPosition, vAxis,
                                                 _e->getProperty(part));
}

void LCylinder::operator<<(const magnet::xml::Node &XML) {
  range = shared_ptr<IDRange>(IDRange::getClass(XML.getNode("IDRange"), Sim));
  _diameter = Sim->_properties.getProperty(XML.getAttribute("ParticleDiameter"),
                                           Property::Units::Length());
  _cyl_radius =
      XML.getAttribute("CylinderRadius").as<double>() * Sim->units.unitLength();
  ...
  if (std::abs(_diameter->getMinValue() + _cyl_radius) == 0)
    M_throw()
        << "Cannot have a wall with a diameter and CylinderRadius of zero!";

  magnet::xml::Node xBrowseNode = XML.getNode("Axis");
  vAxis << xBrowseNode;
  if (vAxis.nrm() == 0)
    M_throw() << "The Axis for " << XML.getPath() << " named \"" << getName()
...
void LCylinder::outputXML(magnet::xml::XmlStream &XML) const {
  XML << magnet::xml::attr("Type") << "Cylinder" << magnet::xml::attr("Name")
      << localName << magnet::xml::attr("Elasticity") << _e->getName()
      << magnet::xml::attr("ParticleDiameter") << _diameter->getName()
      << magnet::xml::attr("CylinderRadius")
      << _cyl_radius / Sim->units.unitLength() << range
      << magnet::xml::tag("Axis") << vAxis << magnet::xml::endtag("Axis")
      << magnet::xml::tag("Origin") << vPosition / Sim->units.unitLength()
      << magnet::xml::endtag("Origin");
}

bool LCylinder::validateState(const Particle &part, bool textoutput) const {
  ...
  if ((r > 0) && (_cyl_radius > 0)) {
    if (textoutput)
      derr << "Particle " << part.getID() << " is "
           << r / Sim->units.unitLength() << " far into the cylindrical wall."
           << "\nWall Position = "
           << (vPosition / Sim->units.unitLength()).toString()
           << "\nWall Axis = " << vAxis.toString()
           << ", d = " << diam / Sim->units.unitLength()
           << "\nParticle Position = "
           << (part.getPosition() / Sim->units.unitLength()).toString()
           << "\nSeparation Vector = "
           << (pos / Sim->units.unitLength()).toString()
    ...
    if (textoutput)
      derr << "Particle " << part.getID() << " is "
           << r / Sim->units.unitLength()
           << " far into the inverse cylindrical wall."
           << "\nWall Position = "
           << (vPosition / Sim->units.unitLength()).toString()
           << "\nWall Axis = " << vAxis.toString()
           << ", d = " << diam / Sim->units.unitLength()
...
#ifdef DYNAMO_visualizer
shared_ptr<coil::RenderObj> LCylinder::getCoilRenderObj() const {
  if (!_renderObj) {
    ...
    const double axis_length =
        Sim->primaryCellSize[0] / Sim->units.unitLength();
    const double radius = 2 * std::abs(_cyl_radius) / Sim->units.unitLength();
    for (size_t i(0); i < verts.size() / 3; ++i) {
      Vector result =
          q * Vector{radius * verts[3 * i + 0], radius * verts[3 * i + 1],
                     axis_length * verts[3 * i + 2]} +
          vPosition / Sim->units.unitLength();
    ...
    _renderObj.reset(
        new coil::RTriangleMesh(getName(), verts, Cylinder::getIndices(LOD)));
  }

  return std::static_pointer_cast<coil::RenderObj>(_renderObj);
}

void LCylinder::updateRenderData() const {}
#endif
} // namespace dynamo
