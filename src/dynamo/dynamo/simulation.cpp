...
//! The configuration file version, a version mismatch prevents an XML file
//! load.
...
void Simulation::initialise() {
  ...
  for (const Particle &part : particles) {
    ...
    if (count > 1)
      M_throw() << "Particle ID=" << part.getID()
  ...
  // than there are particles
  {
    ...
    if (tot < N())
      M_throw() << "The particle count according to the species definition is "
                   "too low\n"
                << "discrepancy = " << tot - N() << "\nN = " << N();
  ...
  for (const Particle &particle : particles) {
    try {
      ...
      M_throw()
          << "The particle (ID=" << particle.getID()
          << ") does not have a self Interaction defined. Self Interactions "
             "are not used for the dynamics of the system, but are used to "
             "draw/visualise the particles, as well as calculate the excluded "
             "volume and other properties. Please add a self-Interaction";
    }
  }
  ...
  if (std::dynamic_pointer_cast<BCPeriodic>(BCs)) {
    ...
    for (size_t i(0); i < NDIM; ++i)
      if (primaryCellSize[i] <= (2.0 * max_interaction_dist))
        M_throw()
            << "When using periodic boundary conditions, the size of the "
               "primary image must be at least 2x the maximum interaction "
               "distance in all dimensions, otherwise one particle can "
               "interact "
  ...
  if (std::dynamic_pointer_cast<BCPeriodic>(BCs))
    globals.push_back(
        shared_ptr<Global>(new GPBCSentinel(this, "PBCSentinel")));
...
Event Simulation::getEvent(const Particle &p1, const Particle &p2) const {
...
void Simulation::stream(const double dt) {
  BCs->update(dt);
...
double Simulation::getLongestInteraction() const {
  double maxval = 0.0;
...
Simulation::getInteraction(const Particle &p1, const Particle &p2) const {
  ...
  M_throw() << "Could not find an Interaction between particles " << p1.getID()
            << " and " << p2.getID()
            << ". All particle pairings must have a corresponding Interaction "
               "defined.";
}
...
void Simulation::loadXMLfile(std::string fileName) {
  if (status != START)
    M_throw() << "Loading config at wrong time, status = " << status;

  using namespace magnet::xml;

  dout << "Reading the XML input file, " << fileName << ", into memory"
       << std::endl;
...

  {
    ...
    if (version != configFileVersion)
      M_throw() << "This version of the config file is obsolete"
                ...
                << "\nPlease look at the XMLFILE.VERSION file in the root "
                   "directory of the dynamo source.";
  }
  ...
  if (simNode.hasNode("Topology")) {
    ...
    size_t i(0);
    for (magnet::xml::Node node =
             simNode.getNode("Topology").findNode("Structure");
         node.valid(); ++node, ++i)
...

  {
    ...
    size_t i(0);
    for (magnet::xml::Node node = simNode.getNode("Genus").findNode("Species");
         node.valid(); ++node, ++i)
  ...
  checkNodeNameAttribute(
      simNode.getNode("Interactions").findNode("Interaction"));
  for (magnet::xml::Node node =
           simNode.getNode("Interactions").findNode("Interaction");
       node.valid(); ++node)
  ...
  if (simNode.hasNode("Locals")) {
    ...
    for (magnet::xml::Node node = simNode.getNode("Locals").findNode("Local");
         node.valid(); ++node)
  ...
  if (simNode.hasNode("Globals")) {
    ...
    for (magnet::xml::Node node = simNode.getNode("Globals").findNode("Global");
         node.valid(); ++node)
  ...
  if (simNode.hasNode("SystemEvents")) {
    ...
    for (magnet::xml::Node node =
             simNode.getNode("SystemEvents").findNode("System");
         node.valid(); ++node)
...
void Simulation::writeXMLfile(std::string fileName, bool applyBC, bool round) {
  ...
  XML << std::setprecision(std::numeric_limits<double>::digits10 + 2 -
                           4 * round)
      << xml::prolog() << xml::tag("DynamOconfig") << xml::attr("version")
      << configFileVersion << xml::tag("Simulation");
  ...
  for (const shared_ptr<Species> &ptr : species)
    XML << xml::tag("Species") << ptr << xml::endtag("Species");

  XML << xml::endtag("Genus") << xml::tag("BC") << BCs << xml::endtag("BC")
      << xml::tag("Topology");

  for (const shared_ptr<Topology> &ptr : topology)
    XML << xml::tag("Structure") << ptr << xml::endtag("Structure");

  XML << xml::endtag("Topology") << xml::tag("Interactions");

  for (const shared_ptr<Interaction> &ptr : interactions)
    XML << xml::tag("Interaction") << ptr << xml::endtag("Interaction");

  XML << xml::endtag("Interactions") << xml::tag("Locals");

  for (const shared_ptr<Local> &ptr : locals)
    XML << xml::tag("Local") << ptr << xml::endtag("Local");

  XML << xml::endtag("Locals") << xml::tag("Globals");

  for (const shared_ptr<Global> &ptr : globals)
    XML << ptr;

  XML << xml::endtag("Globals") << xml::tag("SystemEvents");

  for (const shared_ptr<System> &ptr : systems)
    XML << ptr;

  XML << xml::endtag("SystemEvents") << xml::tag("Dynamics") << dynamics
      << xml::endtag("Dynamics") << xml::endtag("Simulation") << _properties;
...
void Simulation::replexerSwap(Simulation &other) {
  ...
  if (outputPlugins.size() != other.outputPlugins.size())
    std::cerr << "Error, could not swap output plugin lists as they are not "
                 "equal in size";
#endif
  ...
  for (size_t i(0); i < outputPlugins.size(); ++i) {
    ...
    if (typeid(*outputPlugins[i]) != typeid(*other.outputPlugins[i]))
      M_throw() << "Output plugin mismatch while replexing! lists not sorted "
                   "the same perhaps?";
#endif
  ...
  // #ifdef DYNAMO_DEBUG
  //     //Here we check that all plugins etc have the correct simulation
  //     pointer. for (auto Sim : {this, &other}) {
  //       for (const auto& plugin: Sim->outputPlugins)
  //	if (plugin->getSimPointer() != Sim)
  //	  M_throw() << "Programming error: Mismatch of Sim pointer in output
  // plugins after replica exchange. Please file a bug report.";
  //
  //       for (const auto& plugin: Sim->interactions)
  //	if (plugin->getSimPointer() != Sim)
  //	  M_throw() << "Programming error: Mismatch of Sim pointer in
  // Interactions after replica exchange. Please file a bug report.";
  //
  //       for (const auto& plugin: Sim->locals)
  //	if (plugin->getSimPointer() != Sim)
  //	  M_throw() << "Programming error: Mismatch of Sim pointer in Local
  // after replica exchange. Please file a bug report.";
  //
  //       for (const auto& plugin: Sim->globals)
  //	if (plugin->getSimPointer() != Sim)
  //	  M_throw() << "Programming error: Mismatch of Sim pointer in Global
  // after replica exchange. Please file a bug report.";
  //
  //       for (const auto& plugin: Sim->systems)
  //	if (plugin->getSimPointer() != Sim)
  //	  M_throw() << "Programming error: Mismatch of Sim pointer in System
  // after replica exchange. Please file a bug report.";
  //
  //       for (const auto& plugin: Sim->topology)
  //	if (plugin->getSimPointer() != Sim)
  //	  M_throw() << "Programming error: Mismatch of Sim pointer in Topology
  // after replica exchange. Please file a bug report.";
  //
  //       if (Sim->ptrScheduler->getSimPointer() != Sim)
  //	M_throw() << "Programming error: Mismatch of Sim pointer in Scheduler
  // after replica exchange. Please file a bug report.";
  //
  //       if (Sim->dynamics->getSimPointer() != Sim)
  //	M_throw() << "Programming error: Mismatch of Sim pointer in Dynamics
  // after replica exchange. Please file a bug report.";
  //
  //       if (Sim->BCs->getSimPointer() != Sim)
  //	M_throw() << "Programming error: Mismatch of Sim pointer in BCs after
  // replica exchange. Please file a bug report.";
  //     }
  // #endif
}

double Simulation::calcInternalEnergy() const {
...
void Simulation::setCOMVelocity(const Vector COMVelocity) {
...
void Simulation::addSystemTicker() {
  for (shared_ptr<System> &ptr : systems)
    if (ptr->getName() == "SystemTicker")
      M_throw() << "System Ticker already exists";

  systems.push_back(
      shared_ptr<System>(new SysTicker(this, lastRunMFT, "SystemTicker")));
}

double Simulation::getSimVolume() const {
  double vol = 1.0;
...
double Simulation::getNumberDensity() const { return N() / getSimVolume(); }

double Simulation::getPackingFraction() const {
  double volume = 0.0;

  for (const Particle &particle : particles)
    volume +=
        getInteraction(particle, particle)->getExcludedVolume(particle.getID());

  return volume / getSimVolume();
}

size_t Simulation::checkSystem() {
  dynamics->updateAllParticles();
  ...
  for (const shared_ptr<Interaction> &interaction_ptr : interactions) {
    dout << "Checking Interaction \"" << interaction_ptr->getName() << "\""
         << std::endl;
...
void Simulation::outputData(std::string filename) {
  if (status < INITIALISED)
...
void Simulation::setTickerPeriod(double nP) {
  shared_ptr<SysTicker> ptr =
      std::dynamic_pointer_cast<SysTicker>(systems["SystemTicker"]);
  if (!ptr)
...
void Simulation::scaleTickerPeriod(double nP) {
  shared_ptr<SysTicker> ptr =
      std::dynamic_pointer_cast<SysTicker>(systems["SystemTicker"]);
  if (!ptr)
    M_throw() << "Could not find system ticker (maybe not required?)";
...
void Simulation::addOutputPlugin(std::string Name) {
  if (status >= INITIALISED)
...
void Simulation::simShutdown() { nextPrintEvent = endEventCount = eventCount; }

bool Simulation::runSimulationStep(bool silentMode) {
  if (status < INITIALISED)
  ...
  try {
    ...
    M_throw() << "Exception caught while executing event " << eventCount << "\n"
              << cep.what();
  }
