...
shared_ptr<Ensemble> Ensemble::loadEnsemble(const dynamo::Simulation &Sim) {
  ...
  try {
    std::shared_ptr<SysAndersen> thermoptr =
        std::dynamic_pointer_cast<SysAndersen>(Sim.systems["Thermostat"]);
...
void EnsembleNVE::initialise() {
  EnsembleVals[0] = Sim->particles.size();
  EnsembleVals[1] = Sim->primaryCellSize[0] * Sim->primaryCellSize[1] *
                    Sim->primaryCellSize[2];
  EnsembleVals[2] =
      Sim->calcInternalEnergy() + Sim->dynamics->getSystemKineticEnergy();
...
std::array<double, 3> EnsembleNVE::getReducedEnsembleVals() const {
...
void EnsembleNVT::initialise() {
  ...
  EnsembleVals[2] =
      std::dynamic_pointer_cast<SysAndersen>(thermostat)->getTemperature();
...
std::array<double, 3> EnsembleNVT::getReducedEnsembleVals() const {
  std::array<double, 3> retval;
  retval[0] = EnsembleVals[0];
  retval[1] = EnsembleVals[1] / Sim->units.unitVolume();
  retval[2] = EnsembleVals[2] / Sim->units.unitEnergy();
...
double EnsembleNVT::exchangeProbability(const Ensemble &oE) const {
#ifdef DYNAMO_DEBUG
  ...
  if (std::dynamic_pointer_cast<DynNewtonianMC>(ensemble2.Sim->dynamics)) {
    factor +=
        static_cast<const DynNewtonianMC &>(*ensemble2.Sim->dynamics).W(E2);
    factor -=
        static_cast<const DynNewtonianMC &>(*ensemble2.Sim->dynamics).W(E1);
  }
