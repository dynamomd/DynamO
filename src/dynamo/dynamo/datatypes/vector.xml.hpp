...
template<class T>
inline
magnet::xml::XmlStream& 
operator<<(magnet::xml::XmlStream& XML, const CVector<T> &vec)
{
  char name[2] = "x";
  ...
  for (int iDim = 0; iDim < NDIM; iDim++)
    {
      ...
      XML << magnet::xml::attr(name) << vec[iDim];
    }
...
inline
magnet::xml::XmlStream& 
operator<<(magnet::xml::XmlStream& XML, const CVector<CVector<T> > &vec)
{
  char name[2] = "x";
  ...
  for (int iDim = 0; iDim < NDIM; iDim++)
    {
      name[0]= 'x'+iDim; //Write the dimension
      XML << magnet::xml::tag(name) << vec[iDim]
