...
namespace dynamo {
Scheduler::Scheduler(dynamo::Simulation *const tmp, const char *aName, FEL *nS)
    : SimBase(tmp, aName), sorter(nS), _interactionRejectionCounter(0),
      _localRejectionCounter(0) {}

Scheduler::~Scheduler() {}

void Scheduler::operator<<(const magnet::xml::Node &XML) {
...
void Scheduler::initialise() {
    ...
    for (const size_t id2 : *ids)
      if (id2 > id1)
        if (Sim->getInteraction(Sim->particles[id1], Sim->particles[id2])
                ->validateState(Sim->particles[id1], Sim->particles[id2],
                                (warnings < 101)))
          ++warnings;
  }
  ...
  if (warnings > 100)
    derr << "Over 100 warnings of invalid states, further output was "
            "suppressed (total of "
         << warnings << " warnings detected)" << std::endl;

  dout << "Building all events on collision " << Sim->eventCount << std::endl;
  rebuildList();
}

void Scheduler::rebuildList() {
  sorter->clear();
  sorter->init(Sim->N() + 1);
...
void Scheduler::addEvents(Particle &part) {
...
shared_ptr<Scheduler> Scheduler::getClass(const magnet::xml::Node &XML,
...
void Scheduler::rebuildSystemEvents() const {
...
void Scheduler::pushEvent(const Event &newevent) { sorter->push(newevent); }

void Scheduler::invalidateEvents(const Particle &part) {
  sorter->invalidate(part.getID());
}

void Scheduler::runNextEvent() {
#ifdef DYNAMO_DEBUG
  if (sorter->empty())
  ...
  */
  ...
  if (next_event._type == NONE)
    M_throw()
        << "A type=NONE event with no source has reached the top of the queue."
  ...
  case INTERACTION: {
...
#ifdef DYNAMO_DEBUG
    if (sorter->empty())
      M_throw() << "The next PEL is empty, cannot perform the comparison to "
                   "see if this event is out of sequence";
#endif
    ...
    stream(Event._dt);
    // Allow everything to stream up to the current time before executing the
    // event
    Sim->stream(Event._dt);

    PairEventData eventdata =
  ...
  case GLOBAL: {
    if (!std::isfinite(next_event._dt))
      M_throw() << "Next event time is not finite!"
                ...
                << ")= " << Sim->globals[next_event._sourceID]->getName();
    ...
    // behavior)
    Sim->globals[next_event._sourceID]->runEvent(
        Sim->particles[next_event._particle1ID], next_event._dt);
    break;
  }
  case LOCAL: {
    ...
    if ((iEvent._type == NONE) ||
        ((iEvent._dt > next_event._dt) &&
         (++_localRejectionCounter < rejectionLimit))) {
  ...
  case SYSTEM: {
    sorter->pop();
    // System events can use the value -std::numeric_limits<float>::infinity()
    // to request immediate processing, therefore, only NaN and
    // +std::numeric_limits<float>::infinity() values are invalid
    if (std::isnan(next_event._dt) ||
        (next_event._dt == std::numeric_limits<float>::infinity()))
      M_throw() << "Next event time is not finite!"
                << "\ndt = " << next_event._dt
                << "\nEvent Type = " << next_event._type
                << "\nParticle ID = " << next_event._particle1ID
                << "\nSystem (ID=" << next_event._sourceID
                << ")= " << Sim->systems[next_event._sourceID]->getName();
    ...
    if (!data.L1partChanges.empty() || !data.L2partChanges.empty()) {
      ...
      for (const auto &d2 : data.L2partChanges)
        this->fullUpdate(Sim->particles[d2.particle1_.getParticleID()],
                         Sim->particles[d2.particle2_.getParticleID()]);
...
void Scheduler::addInteractionEvent(const Particle &part,
...
void Scheduler::addLocalEvent(const Particle &part, const size_t &id) const {
