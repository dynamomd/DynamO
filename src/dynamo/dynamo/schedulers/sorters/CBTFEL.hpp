...
namespace dynamo {
template <class PEL> class CBTFEL : public FEL {
public:
...
protected:
  ...
  flushChanges(const size_t ID = std::numeric_limits<size_t>::max()) {
    if ((_activeID != ID) &&
        (_activeID != std::numeric_limits<size_t>::max())) {
      if (_Min[_activeID + 1].empty() ||
          (_Min[_activeID + 1].top()._dt ==
  ...
  inline void UpdateCBT(const size_t i) {
    ...
    for (; (f > 0) && (_CBT[f] == i); f /= 2) {
      size_t l = _CBT[f * 2], r = _CBT[f * 2 + 1];
    ...
    for (; f > 0; f /= 2) {
      size_t w = _CBT[f], /* old winner */
          l = _CBT[f * 2], r = _CBT[f * 2 + 1];

      _CBT[f] = (_Min[r] > _Min[l]) ? l : r;

      if (_CBT[f] == w)
        return; /* end of the event time comparisons */
    }
  }
