...
namespace dynamo {
namespace detail {
template <class PEL> struct BPQEntry : public PEL {
...
public:
  ...
  void init(const size_t N) {
    ...
    scale = 0;
    nlists = 1;
    linearLists.resize(nlists + 1,
                       NO_LINK); /*+1 for overflow, NO_LINK for marking empty*/
  }
  ...
  inline void stream(const double ndt) { Base::_pecTime += ndt; }
...
private:
  virtual void
  flushChanges(const size_t ID = std::numeric_limits<size_t>::max()) {
    if ((Base::_activeID != ID) &&
        (Base::_activeID != std::numeric_limits<size_t>::max())) {
  ...
  void optimiseSettings() {
    ...
    double minVal(std::numeric_limits<float>::infinity()),
    ...
    if ((counter < 10) || (maxVal < 0) || ((maxVal - minVal) == 0) ||
        (maxVal < minVal)) { // In unusual systems, drop down to a CBT queue
      scale = 0;
      nlists = 1;
    } else {
  ...
  inline void insertInEventQ(const size_t p) {
    ...
    if (p >= Base::_Min.size())
      M_throw() << "p=" << p
                << " is out of range of Min (size()=" << Base::_Min.size()
                << ")";
#endif
    ...
    // Check that the Q is not empty or filled with events which will never
    // happen
    if (Base::_Min[p].empty() ||
        (Base::_Min[p].top()._dt == std::numeric_limits<float>::infinity()))
    ...
    if (i >= linearLists.size())
      M_throw() << "i=" << p << " is out of range of linearLists (size()="
                << linearLists.size() << ") box=" << box
                << " dt=" << Base::_Min[p].top()._dt << " scale=" << scale;
#endif
  ...
  inline void processOverflowList() {
    ...
    linearLists[nlists] =
        NO_LINK; /* mark empty; we will treat all entries and may re-add some */
  ...
  inline void deleteFromEventQ(const size_t e) {
    if (Base::_Min[e].qIndex == currentIndex)
      ...
      const size_t prev = Base::_Min[e].previous, next = Base::_Min[e].next;
      if (prev == NO_LINK)
  ...
  inline void orderNextEvent() {
    while (Base::_NP == 0) {
      ...
      if (++currentIndex == nlists) {
        ...
        for (auto &dat : Base::_Min) {
          no_events =
              no_events &&
              ((dat.empty()) ||
               (dat.top()._dt == std::numeric_limits<float>::infinity()));
      ...
      /* populate pq */
      for (size_t e = linearLists[currentIndex]; e != NO_LINK;
           e = Base::_Min[e].next)
        Base::Insert(e);
