...
namespace dynamo {
void SNeighbourList::initialiseNBlist() {
  ...
  if (NBListID == std::numeric_limits<size_t>::max()) {
    ...
    shared_ptr<DynCompression> compressiondynamics =
        std::dynamic_pointer_cast<DynCompression>(Sim->dynamics);
    if (compressiondynamics) {
      ...
      Sim->systems.push_back(shared_ptr<System>(new SysNBListCompressionFix(
          Sim, compressiondynamics->getGrowthRate(), NBListID)));
    }
  }
...
void SNeighbourList::initialise() {
  ...
  if (nblist->getMaxSupportedInteractionLength() < Sim->getLongestInteraction())
    M_throw() << "Neighbourlist supports too small interaction distances! "
                 "Supported distance is "
              << nblist->getMaxSupportedInteractionLength() /
                     Sim->units.unitLength()
              << " but the longest interaction distance is "
              << Sim->getLongestInteraction() / Sim->units.unitLength();

  nblist->_sigNewNeighbour.connect<Scheduler, &Scheduler::addInteractionEvent>(
      this);
  nblist->_sigReInitialise.connect<SNeighbourList, &SNeighbourList::initialise>(
      this);
  Scheduler::initialise();
}

void SNeighbourList::outputXML(magnet::xml::XmlStream &XML) const {
  XML << magnet::xml::attr("Type") << "NeighbourList"
      << magnet::xml::tag("Sorter") << *sorter << magnet::xml::endtag("Sorter");
}
...
double SNeighbourList::getNeighbourhoodDistance() const {
#ifdef DYNAMO_DEBUG
  if (!std::dynamic_pointer_cast<GNeighbourList>(Sim->globals[NBListID]))
    M_throw() << "Not a GNeighbourList!";
#endif

  return static_cast<const GNeighbourList *>(Sim->globals[NBListID].get())
...
SNeighbourList::getParticleNeighbours(const Particle &part) const {
  ...
  const GNeighbourList &nblist(
      *static_cast<const GNeighbourList *>(Sim->globals[NBListID].get()));
