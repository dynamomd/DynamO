...
double ISquareBond::getCaptureEnergy() const { return 0.0; }

double ISquareBond::maxIntDist() const {
  return _diameter->getMaxValue() * _lambda->getMaxValue();
}

bool ISquareBond::captureTest(const Particle &p1, const Particle &p2) const {
...
bool ISquareBond::validateState(const Particle &p1, const Particle &p2,
  ...
  if (!Sim->dynamics->sphereOverlap(p1, p2, d * l)) {
    if (textoutput)
      derr << "Particle " << p1.getID() << " and Particle " << p2.getID()
           << " are bonded and cannot exceed a distance of "
  ...
  if (Sim->dynamics->sphereOverlap(p1, p2, d)) {
    if (textoutput)
      derr << "Particle " << p1.getID() << " and Particle " << p2.getID()
           << " are bonded with an inner hard core at "
...
size_t ISquareBond::validateState(bool textoutput, size_t max_reports) const {
  size_t retval(0);
...
Event ISquareBond::getEvent(const Particle &p1, const Particle &p2) const {
#ifdef DYNAMO_DEBUG
...
PairEventData ISquareBond::runEvent(Particle &p1, Particle &p2, Event iEvent) {
  ++Sim->eventCount;
  ...
  const double d = _diameter->getProperty(p1, p2);
  return Sim->dynamics->SmoothSpheresColl(iEvent, _e->getProperty(p1, p2),
                                          d * d, iEvent._type);
}

void ISquareBond::outputXML(magnet::xml::XmlStream &XML) const {
  XML << magnet::xml::attr("Type") << "SquareBond"
      << magnet::xml::attr("Diameter") << _diameter->getName()
      << magnet::xml::attr("Lambda") << _lambda->getName()
      << magnet::xml::attr("Name") << intName << magnet::xml::attr("Elasticity")
      << _e->getName() << range;
}
} // namespace dynamo
