...
namespace dynamo {
shared_ptr<Potential> Potential::getClass(const magnet::xml::Node &XML) {
...
void PotentialStepped::outputXML(magnet::xml::XmlStream &XML) const {
  ...
  for (size_t id(0); id < steps(); ++id)
    XML << magnet::xml::tag("Step") << magnet::xml::attr("R") << _r_cache[id]
        << magnet::xml::attr("E") << _u_cache[id]
        << magnet::xml::endtag("Step");
}

void PotentialStepped::operator<<(const magnet::xml::Node &XML) {
  if (XML.getAttribute("Direction").as<std::string>() == "Left")
    _direction = false;
  else if (XML.getAttribute("Direction").as<std::string>() == "Right")
    _direction = true;
  else
    M_throw() << "Could not parse Direction, should be either \"Left\" or "
                 "\"Right\"\nXML path:"
              << XML.getPath();
  ...
  for (magnet::xml::Node node = XML.findNode("Step"); node.valid(); ++node)
    steps.push_back(
        std::pair<double, double>(node.getAttribute("R").as<double>(),
                                  node.getAttribute("E").as<double>()));

  if (steps.empty())
    M_throw()
        << "You cannot load a stepped potential with no steps.\nXML path: "
        << XML.getPath();
