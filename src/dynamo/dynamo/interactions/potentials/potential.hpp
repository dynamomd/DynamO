...
/*! \brief The base class for any stepped potential.
  ...
  potentials have zero energy at large distances, but "bonding" potentials have
  zero energy at zero distance.
  ...
  */
  ...
  std::pair<double, double> getStepBounds(size_t ID) const {
    ...
    if (direction()) {
      ...
      maxR = (ID == steps()) ? std::numeric_limits<float>::infinity()
                             : operator[](ID).first;
    } else {
      minR = (ID == steps()) ? 0 : operator[](ID).first;
      maxR = (ID == 0) ? std::numeric_limits<float>::infinity()
                       : operator[](ID - 1).first;
    }
  ...
  /*! \brief Calculate the potential energy difference changing from
      ...
      transition.
   */
  double getEnergyChange(const size_t orig_step_ID,
                         const size_t new_step_ID) const {
    double orig_energy =
        (orig_step_ID == 0) ? 0 : operator[](orig_step_ID - 1).second;
    double new_energy =
        (new_step_ID == 0) ? 0 : operator[](new_step_ID - 1).second;
...
public:
   ...
   */
  PotentialStepped(std::vector<std::pair<double, double>> step_pos_energy,
                   bool direction);

  virtual std::size_t steps() const { return _r_cache.size(); }
  ...
  virtual double max_distance() const {
    return (_direction ? _r_cache.back() : _r_cache.front());
  }
