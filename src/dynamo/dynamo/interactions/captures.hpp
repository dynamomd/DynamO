...
namespace std {
template <> struct hash<dynamo::detail::PairKey> {
...
public:
  ...
  EntryProxy operator[](const PairKey &key) { return EntryProxy(*this, key); }
...
struct CaptureMapKey : public std::vector<CaptureMap::value_type> {
  ...
  CaptureMapKey(const CaptureMap &map) : Container(map.begin(), map.end()) {}

  std::size_t hash() const {
    std::size_t hash(0);
    for (const Container::value_type &val : *this)
      hash = hash_combine(hash,
                          hash_combine(size_t(val.first), size_t(val.second)));
    return hash;
  }
};
...
struct CaptureMapKeyHash {
  std::size_t operator()(const CaptureMapKey &map) const { return map.hash(); }
};
} // namespace detail
} // namespace dynamo
...
public:
  ICapture(dynamo::Simulation *sim, IDPairRange *range)
      : Interaction(sim, range), _mapUninitialised(true) {}
