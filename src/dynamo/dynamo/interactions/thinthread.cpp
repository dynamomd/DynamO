...
void IThinThread::operator<<(const magnet::xml::Node &XML) {
...
Event IThinThread::getEvent(const Particle &p1, const Particle &p2) const {
#ifdef DYNAMO_DEBUG
...
PairEventData IThinThread::runEvent(Particle &p1, Particle &p2, Event iEvent) {
  ++Sim->eventCount;
  ...
  case CORE: {
    PairEventData retVal =
        Sim->dynamics->SmoothSpheresColl(iEvent, e, d2, CORE);
    if (!isCaptured(p1, p2)) {
...
bool IThinThread::validateState(const Particle &p1, const Particle &p2,
  ...
  } else if (Sim->dynamics->sphereOverlap(p1, p2, d)) {
    if (textoutput)
      derr << "Particle " << p1.getID() << " and Particle " << p2.getID()
...
void IThinThread::outputXML(magnet::xml::XmlStream &XML) const {
