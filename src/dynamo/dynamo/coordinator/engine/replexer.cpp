...
namespace dynamo {
  ...
  EReplicaExchangeSimulation::initialisation()
  {
	...
	if (vm.count("snapshot"))
	  Simulations[i].systems.push_back(shared_ptr<System>(new SysSnapshot(&(Simulations[i]), vm["snapshot"].as<double>(), "SnapshotTimer", "ID%ID.%COUNT", !vm.count("unwrapped"))));
    ...
    for (size_t i(1); i < nSims; ++i) {
      const Dynamics& di = *Simulations[i].dynamics;
      const Dynamics& d0 = *Simulations[0].dynamics;
      if (typeid(di) != typeid(d0))
	M_throw() << vm["config-file"].as<std::vector<std::string> >()[i]
