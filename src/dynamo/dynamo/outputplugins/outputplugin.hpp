...
public:
  OutputPlugin(const dynamo::Simulation *, const char *,
  ...
  static shared_ptr<OutputPlugin> getPlugin(const magnet::xml::Node &,
                                            const dynamo::Simulation *);
  static shared_ptr<OutputPlugin> getPlugin(const std::string,
                                            const dynamo::Simulation *);
  ...
  virtual void replicaExchange(OutputPlugin &) {
    M_throw() << "This output plugin (" << typeid(*this).name()
              << ") is not is not configured for replica exchange";
  }
