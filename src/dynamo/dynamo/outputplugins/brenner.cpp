...
namespace dynamo
{
  OPBrenner::OPBrenner(const dynamo::Simulation *tmp, const magnet::xml::Node &) : OutputPlugin(tmp, "Brenner")
  {
  }
  ...
  void
  OPBrenner::initialise()
  {
    for (size_t i(0); i < NDIM; ++i)
      _sysmomentum_hist[i] = magnet::math::HistogramWeighted<>(Sim->N() * 0.001 * Sim->units.unitMomentum());
  ...
  void
  OPBrenner::eventUpdate(const Event &event, const NEventData &SDat)
  {
    for (size_t i(0); i < NDIM; ++i)
  ...
  void
  OPBrenner::output(magnet::xml::XmlStream &XML)
  {
