...
namespace dynamo
{
  OPVACF::OPVACF(const dynamo::Simulation *tmp,
                 const magnet::xml::Node &XML) : OPTicker(tmp, "VACF"),
                                                 length(50),
                                                 currCorrLength(0),
                                                 ticksTaken(0)
  {
  ...
  OPVACF::ticker()
  {
    ...
    currCorrLength += (currCorrLength != length); //Add to the length while we're not at full length
    ...
    for (const shared_ptr<Species> &sp : Sim->species)
      for (const size_t &ID : *sp->getRange())
        for (size_t step(0); step < currCorrLength; ++step)
          speciesData[sp->getID()][step] += velHistory[ID][step] | velHistory[ID][0];
    ...
    for (const shared_ptr<Topology> &topo : Sim->topology)
      for (const shared_ptr<IDRange> &range : topo->getMolecules())
      {
        ...
        for (size_t step(0); step < currCorrLength; ++step)
        {
          ...
          for (const size_t &ID : *range)
            COMvelocity2 += velHistory[ID][step] * Sim->species[Sim->particles[ID]]->getMass(ID);
  ...
  OPVACF::output(magnet::xml::XmlStream &XML)
  {
    ...
    for (const shared_ptr<Species> &sp : Sim->species)
    {
      ...
      for (size_t step(0); step < currCorrLength; ++step)
        XML << dt * step << " "
            << speciesData[sp->getID()][step] / (static_cast<double>(ticksTaken - step) * static_cast<double>(sp->getCount()) * Sim->units.unitVelocity() * Sim->units.unitVelocity())
            << "\n";
    ...
    for (const shared_ptr<Topology> &topo : Sim->topology)
    {
      ...
      for (size_t step(0); step < currCorrLength; ++step)
        XML << dt * step << " "
            << structData[topo->getID()][step] / (static_cast<double>(ticksTaken - step) * static_cast<double>(topo->getMolecules().size()) * Sim->units.unitVelocity() * Sim->units.unitVelocity())
            << "\n";
