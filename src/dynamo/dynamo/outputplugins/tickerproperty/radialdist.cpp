  ...
  namespace dynamo
  {
    ...
    OPRadialDistribution::initialise()
    {
      gr_accumulator.resize(Sim->species.size(),
                            std::vector<std::vector<long long>>(Sim->species.size(), std::vector<long long>(length, 0)));
          ...
          for (const shared_ptr<Species> &sp2 : Sim->species)
            {
              ...
              for (size_t i(1); i < length; ++i)
                initial_moment[moment_offset + i] += initial_moment[moment_offset + i - 1];
            }
      } else
    ...
    OPRadialDistribution::ticker()
    {
      ...
      std::vector<long long> accumulator; //Eventually becomes the number of particles at this radius or less
      ...
      for (const shared_ptr<Species> &sp1 : Sim->species)
        ...
        {      
          ...
          const size_t initial_moment_offset = (sp1->getID() * Sim->species.size() + sp2->getID()) * length;

          for (size_t m(0); m < N_moments; ++m) {
            //Generate the moment of the accumulator
            for (size_t i(0); i < length; ++i)
              moment[i] *= accumulator[i] - initial_moment[initial_moment_offset + i];
      ...
      for (const shared_ptr<Species> &sp1 : Sim->species)
        ...
        const size_t initial_moment_offset = (sp1->getID() * Sim->species.size() + sp2->getID()) * length;
              XML << magnet::xml::tag("Moment")
                  << magnet::xml::attr("Order") << -1
                  << magnet::xml::chardata();

            for (size_t i = 0; i < length; ++i)
              XML << binWidth * (i+1) / Sim->units.unitLength() << " " << initial_moment[initial_moment_offset + i] << "\n";
