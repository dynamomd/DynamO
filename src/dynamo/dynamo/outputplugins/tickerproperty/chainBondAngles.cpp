...
namespace dynamo {
OPChainBondAngles::Cdata::Cdata(size_t ID, size_t CL, double bw) : chainID(ID) {
...
void OPChainBondAngles::initialise() {
  for (const shared_ptr<Topology> &plugPtr : Sim->topology)
    if (std::dynamic_pointer_cast<TChain>(plugPtr))
      chains.push_back(Cdata(
          plugPtr->getID(), plugPtr->getMolecules().front()->size(), binwidth));
}

void OPChainBondAngles::ticker() {
...
void OPChainBondAngles::output(magnet::xml::XmlStream &XML) {
  ...
  for (Cdata &dat : chains) {
    ...
    size_t Nc = Sim->topology[dat.chainID]->getMolecules().front()->size() - 2;
