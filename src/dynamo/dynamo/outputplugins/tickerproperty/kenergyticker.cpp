...
OPKEnergyTicker::OPKEnergyTicker(const dynamo::Simulation *tmp,
    ...
    : OPTicker(tmp, "KEnergyTicker"), count(0) {
  operator<<(XML);
}

void OPKEnergyTicker::operator<<(const magnet::xml::Node &XML) {}

void OPKEnergyTicker::initialise() {
  for (size_t iDim = 0; iDim < NDIM; ++iDim)
    for (size_t jDim = 0; jDim < NDIM; ++jDim)
      sum[iDim][jDim] = 0.0;
}

void OPKEnergyTicker::ticker() {
  ++count;
  ...
  for (const Particle &part : Sim->particles)
    for (size_t iDim = 0; iDim < NDIM; ++iDim)
      for (size_t jDim = 0; jDim < NDIM; ++jDim)
        localE[iDim][jDim] += part.getVelocity()[iDim] *
                              part.getVelocity()[jDim] *
...
void OPKEnergyTicker::output(magnet::xml::XmlStream &XML) {
  double sumComp(0);

  for (size_t iDim(0); iDim < NDIM; ++iDim)
    sumComp += sum[iDim][iDim];

  XML << magnet::xml::tag("KEnergyTicker") << magnet::xml::attr("T")
  ...
  for (size_t iDim = 0; iDim < NDIM; ++iDim) {
    std::string name =
        std::string("d") + boost::lexical_cast<std::string>(iDim);

    XML << magnet::xml::tag(name.c_str());

    for (size_t jDim = 0; jDim < NDIM; ++jDim) {
      std::string name =
          std::string("d") + boost::lexical_cast<std::string>(jDim);
      XML << magnet::xml::attr(name.c_str())
          << sum[iDim][jDim] /
                 (((double)count) * Sim->N() * Sim->units.unitEnergy());
      ;
    }
...
void OPKEnergyTicker::periodicOutput() {
  double sumComp(0);
