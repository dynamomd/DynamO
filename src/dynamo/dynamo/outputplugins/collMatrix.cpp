...
namespace dynamo
{
  OPCollMatrix::OPCollMatrix(const dynamo::Simulation *tmp, const magnet::xml::Node &) : OutputPlugin(tmp, "CollisionMatrix"),
                                                                                         totalCount(0),
  ...
  OPCollMatrix::output(magnet::xml::XmlStream &XML)
  {
    ...
    for (const locPair &ele : counters)
    {
      XML << magnet::xml::tag("Count")
          ...
          << magnet::xml::attr("Percent") << 100.0 * ((double)ele.second.count) / ((double)totalCount)
          << magnet::xml::attr("mft") << ele.second.totalTime / (Sim->units.unitTime() * ((double)ele.second.count))
      ...
      // Add the rate
      totmap[ele.first.first].second += ((double)ele.second.count) / ele.second.totalTime;
    }
    ...
    for (const auto &mp1 : totmap)
      XML << magnet::xml::tag("TotCount")
          ...
          << 100.0 * (((double)mp1.second.first) + ((double)initialCounter[mp1.first])) / (((double)totalCount) + ((double)initialsum))
          << magnet::xml::attr("Count") << mp1.second.first + initialCounter[mp1.first]
          << magnet::xml::attr("EventMeanFreeTime")
          << Sim->systemTime / ((mp1.second.first + initialCounter[mp1.first]) * Sim->units.unitTime())
    ...
    for (const auto &val : _captureCounters)
    {
      ...
      val.second.rijdotvij.outputHistogram(XML, 1.0 / Sim->units.unitLength() / Sim->units.unitMomentum());
