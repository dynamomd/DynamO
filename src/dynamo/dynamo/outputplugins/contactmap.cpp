...
namespace {
::std::size_t hash_combine(const ::std::size_t hash1,
...
namespace dynamo {
namespace detail {
::std::size_t OPContactMapPairHash::operator()(
    const std::pair<std::size_t, std::size_t> &entry) const {
...
void OPContactMap::initialise() {
  ...
  _interaction =
      std::dynamic_pointer_cast<ICapture>(Sim->interactions[_interaction_name]);

  if (!_interaction)
    M_throw() << "Could not cast \"" << _interaction_name
              << "\" to an ICapture type to build the contact map";

  _current_map =
      _collected_maps
          .insert(CollectedMapType::value_type(
              *_interaction, MapData(Sim->systemTime, Sim->calcInternalEnergy(),
                                     _next_map_id++)))
          .first;
}
...
void OPContactMap::flush() {
  // Cannot create new maps here, as flush may happen when the output plugins
  // are invalid
...
void OPContactMap::operator<<(const magnet::xml::Node &XML) {
  if (!XML.hasAttribute("Interaction"))
    M_throw() << "You must specify an Interaction name for ContactMap tracking "
                 "using the Interaction option";

  _interaction_name = XML.getAttribute("Interaction").as<std::string>();
}

void OPContactMap::eventUpdate(const Event &event, const NEventData &) {
  stream(event._dt);
...
void OPContactMap::mapChanged(bool addLink) {
  flush();
...
void OPContactMap::replicaExchange(OutputPlugin &otherplugin) {
...
void OPContactMap::periodicOutput() {
  I_Pcout() << ", Maps " << _collected_maps.size() << ", links "
            << _map_links.size();
}

void OPContactMap::output(magnet::xml::XmlStream &XML) {
  dout << "Writing out " << _collected_maps.size() << " Contact maps with "
       << _map_links.size() << " links" << std::endl;

  namespace xml = magnet::xml;
  XML << xml::tag("ContactMap") << xml::tag("Maps") << xml::attr("Count")
      << _collected_maps.size();
  ;

  for (const CollectedMapType::value_type &entry : _collected_maps) {
    XML << xml::tag("Map") << xml::attr("ID") << entry.second._id
        << xml::attr("DiscoveryTime")
        << entry.second._discovery_time / Sim->units.unitTime()
    ...
    for (const ICapture::value_type &ids : entry.first)
      XML << xml::tag("Contact") << xml::attr("ID1") << ids.first.first
          << xml::attr("ID2") << ids.first.second << xml::attr("State")
          << ids.second << xml::endtag("Contact");

    XML << xml::endtag("Map");
  }

  XML << xml::endtag("Maps") << xml::tag("Links") << xml::attr("Count")
      << _map_links.size();

  for (const LinksMapType::value_type &entry : _map_links)
    XML << xml::tag("Link") << xml::attr("Source") << entry.first.first
        << xml::attr("Target") << entry.first.second << xml::attr("Occurrences")
        << entry.second << xml::endtag("Link");

  XML << xml::endtag("Links") << xml::endtag("ContactMap");
}
} // namespace dynamo
