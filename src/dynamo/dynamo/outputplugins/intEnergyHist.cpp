...
namespace dynamo {
OPIntEnergyHist::OPIntEnergyHist(const dynamo::Simulation *tmp,
                                 const magnet::xml::Node &XML)
    : OutputPlugin(tmp, "InternalEnergyHistogram", 10), // Before OPEnergy
      intEnergyHist(1.0), binwidth(1.0) {
  operator<<(XML);
}

void OPIntEnergyHist::eventUpdate(const Event &event, const NEventData &) {
  intEnergyHist.addVal(_ptrOPMisc->getConfigurationalU(), event._dt);
}

void OPIntEnergyHist::operator<<(const magnet::xml::Node &XML) {
  if (XML.hasAttribute("BinWidth"))
    binwidth = XML.getAttribute("BinWidth").as<double>();
}

void OPIntEnergyHist::initialise() {
  _ptrOPMisc = Sim->getOutputPlugin<OPMisc>();
  if (!_ptrOPMisc)
    M_throw() << "IntEnergyHist requires Misc plugin!";
  intEnergyHist =
      magnet::math::HistogramWeighted<>(binwidth * Sim->units.unitEnergy());
}

void OPIntEnergyHist::replicaExchange(OutputPlugin &EHist2) {
  std::swap(intEnergyHist,
            static_cast<OPIntEnergyHist &>(EHist2).intEnergyHist);
  std::swap(binwidth, static_cast<OPIntEnergyHist &>(EHist2).binwidth);
}

std::unordered_map<int, double> OPIntEnergyHist::getImprovedW() const {
  if (!std::dynamic_pointer_cast<const DynNewtonianMC>(Sim->dynamics))
    M_throw() << "Cannot improve an non-Multicanonical Dynamics";

  const DynNewtonianMC &dynamics =
      static_cast<const DynNewtonianMC &>(*Sim->dynamics);

  if (dynamics.getEnergyStep() != intEnergyHist.getBinWidth())
    M_throw()
        << "Cannot improve the W potential when there is a mismatch between the"
  ...
  // Now center the energy warps about 0 to not cause funny changes in the
  // tails.
...
void OPIntEnergyHist::output(magnet::xml::XmlStream &XML) {
  XML << magnet::xml::tag("EnergyHist") << magnet::xml::attr("BinWidth")
      << binwidth;

  if (std::dynamic_pointer_cast<const dynamo::EnsembleNVT>(Sim->ensemble))
    XML << magnet::xml::attr("T")
        << static_cast<const dynamo::EnsembleNVT &>(*(Sim->ensemble))
               .getReducedEnsembleVals()[2];

  intEnergyHist.outputClearHistogram(XML, Sim->units.unitEnergy());

  if (std::dynamic_pointer_cast<const DynNewtonianMC>(Sim->dynamics)) {
    dout << "Detected a Multi-canonical Dynamics, outputting w parameters"
         << std::endl;
    const DynNewtonianMC &dynamics(
        static_cast<const DynNewtonianMC &>(*Sim->dynamics));
    ...
    for (const locpair &p1 : dynamics.getMap())
      XML << magnet::xml::tag("W") << magnet::xml::attr("Energy")
