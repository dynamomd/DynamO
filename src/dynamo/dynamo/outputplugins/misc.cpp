...
OPMisc::OPMisc(const dynamo::Simulation *tmp, const magnet::xml::Node &)
    : OutputPlugin(tmp, "Misc", 0), // ContactMap must be after this
      _dualEvents(0), _singleEvents(0), _virtualEvents(0), _reverseEvents(0),
      _lateInitComplete(false) {}

void OPMisc::replicaExchange(OutputPlugin &misc2) {
...
void OPMisc::temperatureRescale(const double &scale) {
  _KE = _KE.current() * scale;
}

double OPMisc::getMeankT() const {
  return 2.0 * _KE.mean() / Sim->dynamics->getParticleDOF();
}

double OPMisc::getMeanSqrkT() const {
  return 4.0 * _KE.meanSqr() / std::pow(Sim->dynamics->getParticleDOF(), 2);
}

double OPMisc::getCurrentkT() const {
  return 2.0 * _KE.current() / Sim->dynamics->getParticleDOF();
}

double OPMisc::getMeanUConfigurational() const { return _internalE.mean(); }

double OPMisc::getMeanSqrUConfigurational() const {
  return _internalE.meanSqr();
}

void outputCorrelator(magnet::xml::XmlStream &XML, const bool i1, const bool i2,
                      const double inv_units, const double time_units,
                      magnet::math::LogarithmicTimeCorrelator<double> &corr) {
  std::string t = "CC";
  if (i1)
    t[0] = 'I';
  if (i2)
  ...
  XML << tag("Component") << attr("type") << t << chardata();
  {
    std::vector<magnet::math::LogarithmicTimeCorrelator<double>::Data> data =
        corr.getAveragedCorrelator(i1, i2);

    XML << "0 0 0\n";
    for (size_t i(0); i < data.size(); ++i) {
      XML << data[i].time / time_units << " " << data[i].sample_count << " "
...
void outputCorrelator(magnet::xml::XmlStream &XML, const bool i1, const bool i2,
                      const double inv_units, const double time_units,
                      magnet::math::LogarithmicTimeCorrelator<Vector> &corr) {
  std::string t = "CC";
  if (i1)
    t[0] = 'I';
  if (i2)
  ...
  XML << tag("Component") << attr("type") << t << chardata();
  {
    std::vector<magnet::math::LogarithmicTimeCorrelator<Vector>::Data> data =
        corr.getAveragedCorrelator(i1, i2);
...
void outputCorrelator(magnet::xml::XmlStream &XML, const bool i1, const bool i2,
                      const double inv_units, const double time_units,
                      magnet::math::LogarithmicTimeCorrelator<Matrix> &corr) {
  std::string t = "CC";
  if (i1)
    t[0] = 'I';
  if (i2)
  ...
  {
    std::vector<magnet::math::LogarithmicTimeCorrelator<Matrix>::Data> data =
        corr.getAveragedCorrelator(i1, i2);
...
template <class T>
void outputCorrelator(magnet::xml::XmlStream &XML, const double inv_units,
                      const double time_units,
                      magnet::math::LogarithmicTimeCorrelator<T> &corr,
                      const double time) {
  using namespace magnet::xml;

  if constexpr (std::is_same<T, double>::value) {
    XML << tag("AvgSqrImpulse") << attr("value")
        << corr.getAvgImpulseSqSum() * inv_units / time
        << endtag("AvgSqrImpulse");
  } else {
    XML << tag("AvgSqrImpulse") << corr.getAvgImpulseSqSum() * inv_units / time
        << endtag("AvgSqrImpulse");
  }
...
void OPMisc::initialise() {
  ...
  dout << "Particle Count " << Sim->N() << "\nSim Unit Length "
       ...
       << getCurrentkT() / Sim->units.unitEnergy();

  if (std::dynamic_pointer_cast<BCLeesEdwards>(Sim->BCs)) {
    dout << " (Assuming linear shear profile)" << std::endl;
    derr << "\nTemperature output disabled, as shearing/LEBC can give "
            "non-linear velocity profiles."
         << std::endl;
  }
  ...
  for (const auto &p1 : Sim->particles) {
    ...
    for (size_t ID2 : *ids)
      if (ID2 != p1.getID())
        _internalEnergy[p1.getID()] +=
            0.5 * Sim->getInteraction(p1, Sim->particles[ID2])
                      ->getInternalEnergy(p1, Sim->particles[ID2]);
  }
  ...
  for (const Particle &part : Sim->particles) {
    ...
    thermalConductivityFS +=
        part.getVelocity() *
        (sp.getParticleKineticEnergy(part) + _internalEnergy[part.getID()]);
  }
  ...
  for (size_t spid1(0); spid1 < Sim->species.size(); ++spid1) {
    ...
    _thermalDiffusion[spid1].setFreeStreamValue(
        thermalConductivityFS,
        _speciesMomenta[spid1] -
            (_speciesMasses[spid1] / _systemMass) * _sysMomentum.current());

    for (size_t spid2(spid1); spid2 < Sim->species.size(); ++spid2) {
      _mutualDiffusion[spid1 * Sim->species.size() + spid2].resize(
          correlator_dt, 10, 2, false);
      _mutualDiffusion[spid1 * Sim->species.size() + spid2].setFreeStreamValue(
          _speciesMomenta[spid1] -
              (_speciesMasses[spid1] / _systemMass) * _sysMomentum.current(),
          _speciesMomenta[spid2] -
              (_speciesMasses[spid2] / _systemMass) * _sysMomentum.current());
    }
  }
...
void OPMisc::eventUpdate(const Event &eevent, const NEventData &NDat) {
  ...
  //   _viscosity.resize(_viscosity.getSampleTime(), 10, 2, avgPk, avgPk, avgPc,
  //   avgPc);
  // }
  ...
  CounterData &counterdata =
      _counters[CounterKey(getEventSourceKey(eevent), type)];
  ...
  for (const ParticleEventData &PDat : NDat.L1partChanges) {
    ...
    const double deltaKE =
        species.getParticleKineticEnergy(part) - PDat.getOldKE();

    _KE += deltaKE;
    _internalE += PDat.getDeltaU();
    // This must be updated before p1E is calculated
    _internalEnergy[PDat.getParticleID()] += PDat.getDeltaU();
    const double p1E = species.getParticleKineticEnergy(part) +
                       _internalEnergy[PDat.getParticleID()];
    ...
    thermalDel +=
        part.getVelocity() * p1E - PDat.getOldVel() * (p1E - p1deltaE);
  }
  ...
  for (const PairEventData &PDat : NDat.L2partChanges) {
    ...
    const Vector delP =
        mass1 * (part1.getVelocity() - PDat.particle1_.getOldVel());

    const double deltaKE = deltaKE1 + deltaKE2;
    _KE += deltaKE;
    counterdata.netKEchange += deltaKE;
    const double deltaU =
        PDat.particle1_.getDeltaU() + PDat.particle2_.getDeltaU();
    _internalE += deltaU;
    counterdata.netUchange += deltaU;

    _internalEnergy[PDat.particle1_.getParticleID()] +=
        PDat.particle1_.getDeltaU();
    _internalEnergy[PDat.particle2_.getParticleID()] +=
    ...
    thermalDel += part1.getVelocity() * p1E + part2.getVelocity() * p2E -
                  PDat.particle1_.getOldVel() * (p1E - p1deltaE) -
                  PDat.particle2_.getOldVel() * (p2E - p2deltaE);
  }
  ...
  for (size_t spid1(0); spid1 < Sim->species.size(); ++spid1) {
    _thermalDiffusion[spid1].setFreeStreamValue(
        ...
        _speciesMomenta[spid1] -
            _sysMomentum.current() * (_speciesMasses[spid1] / _systemMass));
...
double OPMisc::getMFT() const {
  return Sim->systemTime * static_cast<double>(Sim->N()) /
         (Sim->units.unitTime() * ((2.0 * static_cast<double>(_dualEvents)) +
                                   static_cast<double>(_singleEvents)));
}

double OPMisc::getDuration() const {
  return std::chrono::duration<double>(std::chrono::system_clock::now() -
                                       _starttime)
      .count();
}

double OPMisc::getEventsPerSecond() const {
  return Sim->eventCount / getDuration();
}

double OPMisc::getSimTimePerSecond() const {
  return Sim->systemTime / (getDuration() * Sim->units.unitTime());
}

Matrix OPMisc::getPressureTensor() const {
  return ((collisionalP / Sim->systemTime) + _kineticP.mean()) /
         Sim->getSimVolume();
}

void OPMisc::output(magnet::xml::XmlStream &XML) {
  using namespace magnet::xml;

  dout << "\nTotal events executed " << Sim->eventCount
       << "\nSimulation end time  " << Sim->systemTime / Sim->units.unitTime()
       << "\nAvg. events/s " << getEventsPerSecond() << "\nSim time per second "
       << getSimTimePerSecond() << std::endl;
  ...
  XML << tag("Misc")
      ...
      << tag("Timing") << attr("RuntimeSeconds") << getDuration()
      << attr("RuntimeHours") << getDuration() / 3600 << attr("EventsPerSec")
      << getEventsPerSecond() << attr("SimTimePerSec") << getSimTimePerSecond()
      << endtag("Timing")

      << tag("Density") << attr("val")
      << Sim->getNumberDensity() * Sim->units.unitVolume() << endtag("Density")

      << tag("PackingFraction") << attr("val") << Sim->getPackingFraction()
      << endtag("PackingFraction")

      << tag("SpeciesCount") << attr("val") << Sim->species.size()
      << endtag("SpeciesCount")

      << tag("ParticleCount") << attr("val") << Sim->N()
      << endtag("ParticleCount")

      << tag("SystemMomentum") << tag("Current") << attr("x")
      << _sysMomentum.current()[0] / Sim->units.unitMomentum() << attr("y")
      << _sysMomentum.current()[1] / Sim->units.unitMomentum() << attr("z")
      << _sysMomentum.current()[2] / Sim->units.unitMomentum()
      << endtag("Current") << tag("Average") << attr("x")
      << _sysMomentum.mean()[0] / Sim->units.unitMomentum() << attr("y")
      << _sysMomentum.mean()[1] / Sim->units.unitMomentum() << attr("z")
      << _sysMomentum.mean()[2] / Sim->units.unitMomentum() << endtag("Average")
      << endtag("SystemMomentum");

  if (!std::dynamic_pointer_cast<BCLeesEdwards>(Sim->BCs))
    XML << tag("Temperature") << attr("Mean")
        << getMeankT() / Sim->units.unitEnergy() << attr("MeanSqr")
        << getMeanSqrkT() / (Sim->units.unitEnergy() * Sim->units.unitEnergy())
        << attr("Current") << getCurrentkT() / Sim->units.unitEnergy()
        << attr("Min")
        << 2.0 * _KE.min() /
               (Sim->dynamics->getParticleDOF() * Sim->units.unitEnergy())
        << attr("Max")
        << 2.0 * _KE.max() /
               (Sim->dynamics->getParticleDOF() * Sim->units.unitEnergy())
        << endtag("Temperature");

  XML << tag("UConfigurational") << attr("Mean")
      << getMeanUConfigurational() / Sim->units.unitEnergy() << attr("MeanSqr")
      << getMeanSqrUConfigurational() /
             (Sim->units.unitEnergy() * Sim->units.unitEnergy())
  ...
  if (!std::dynamic_pointer_cast<BCLeesEdwards>(Sim->BCs))
    XML << tag("ResidualHeatCapacity") << attr("Value")
        << (getMeanSqrUConfigurational() -
            getMeanUConfigurational() * getMeanUConfigurational()) /
               (getMeankT() * getMeankT())
        << endtag("ResidualHeatCapacity");

  XML << tag("Pressure") << attr("Avg")
      << P.tr() / (3.0 * Sim->units.unitPressure()) << tag("Tensor")
      << chardata();
  ...
  XML << endtag("Tensor") << tag("InteractionContribution") << chardata();
  ...
  XML << endtag("InteractionContribution") << endtag("Pressure")

      << tag("Duration") << attr("Events") << Sim->eventCount
      << attr("OneParticleEvents") << _singleEvents << attr("TwoParticleEvents")
      << _dualEvents << attr("VirtualEvents") << _virtualEvents << attr("Time")
      << Sim->systemTime / Sim->units.unitTime() << endtag("Duration")

      << tag("EventCounters");

  for (const auto &mp1 : _counters)
    XML << tag("Entry") << attr("Type")
        << getEventSourceTypeName(mp1.first.first) << attr("Name")
        << getEventSourceName(mp1.first.first, Sim) << attr("Event")
        << mp1.first.second << attr("Count") << mp1.second.count
        << tag("NetImpulse")
        << mp1.second.netimpulse / Sim->units.unitMomentum()
        << endtag("NetImpulse") << tag("NetKEChange") << attr("Value")
        << mp1.second.netKEchange / Sim->units.unitEnergy()
        << endtag("NetKEChange") << tag("NetUChange") << attr("Value")
        << mp1.second.netUchange / Sim->units.unitEnergy()
        << endtag("NetUChange") << endtag("Entry");
  ...
  XML << endtag("EventCounters")
      ...
      << tag("totMeanFreeTime") << attr("val") << getMFT()
      << endtag("totMeanFreeTime")

      << tag("NegativeTimeEvents") << attr("Count") << _reverseEvents
      << endtag("NegativeTimeEvents")

      << tag("Memusage") << attr("MaxKiloBytes") << magnet::process_mem_usage()
      << endtag("Memusage");
  ...
  if (!std::dynamic_pointer_cast<BCLeesEdwards>(Sim->BCs)) {
    ...
    {
      const double inv_units =
          Sim->units.unitk() /
          (Sim->units.unitTime() * Sim->units.unitThermalCond() * 2.0 *
           getMeankT() * getMeankT() * V);

      outputCorrelator(XML, inv_units, Sim->units.unitTime(),
                       _thermalConductivity, Sim->systemTime);
    }
    ...
    {
      const double inv_units =
          1.0 / (Sim->units.unitTime() * Sim->units.unitViscosity() * 2.0 *
                 getMeankT() * V);
      outputCorrelator(XML, inv_units, Sim->units.unitTime(), _viscosity,
                       Sim->systemTime);
    }

    XML << endtag("Correlator") << endtag("Viscosity") << tag("BulkViscosity")
        << tag("Correlator");
    ...
    for (size_t i(0); i < Sim->species.size(); ++i) {
      XML << tag("Correlator") << attr("Species") << Sim->species[i]->getName();

      const double inv_units =
          1.0 / (Sim->units.unitTime() * Sim->units.unitThermalDiffusion() *
                 2.0 * getMeankT() * V);

      outputCorrelator(XML, inv_units, Sim->units.unitTime(),
                       _thermalDiffusion[i], Sim->systemTime);

      XML << endtag("Correlator");
    }

    XML << endtag("ThermalDiffusion") << tag("MutualDiffusion");

    for (size_t i(0); i < Sim->species.size(); ++i)
      for (size_t j(i); j < Sim->species.size(); ++j) {
        XML << tag("Correlator") << attr("Species1")
            << Sim->species[i]->getName() << attr("Species2")
            << Sim->species[j]->getName();

        const double inv_units =
            1.0 / (Sim->units.unitTime() * Sim->units.unitMutualDiffusion() *
                   2.0 * getMeankT() * V);

        outputCorrelator(XML, inv_units, Sim->units.unitTime(),
                         _mutualDiffusion[i * Sim->species.size() + j],
                         Sim->systemTime);
...
void OPMisc::periodicOutput() {
  // Calculate the ETA of the simulation, and take care with overflows and the
  // like
  ...
  double time_seconds_remaining =
      _earliest_end_time / (getSimTimePerSecond() * Sim->units.unitTime());
  ...
  if (Sim->endEventCount != std::numeric_limits<size_t>::max()) {
    double event_seconds_remaining =
        (Sim->endEventCount - Sim->eventCount) / getEventsPerSecond() + 0.5;

    if (event_seconds_remaining < double(std::numeric_limits<size_t>::max()))
      seconds_remaining =
          std::min(seconds_remaining, size_t(event_seconds_remaining));
  }
  ...
  if (seconds_remaining != std::numeric_limits<size_t>::max()) {
    ...
    if (ETA_days)
      ...
      I_Pcout() << ETA_days << "d "
                << std::lround(ETA_hours + float(ETA_mins) / 60) << "hr, ";
    else if (ETA_hours)
      // Show hours, and round to nearest minute
      I_Pcout() << ETA_hours << "hr "
                << std::lround(ETA_mins + float(ETA_secs) / 60) << "min, ";
    else if (ETA_mins > 5)
  ...
  I_Pcout() << "Events " << (Sim->eventCount + 1) / 1000 << "k, t "
            << Sim->systemTime / Sim->units.unitTime() << ", <MFT> "
            << getMFT();
  ...
  I_Pcout() << ", U "
            << _internalE.current() / (Sim->units.unitEnergy() * Sim->N());
}
} // namespace dynamo
