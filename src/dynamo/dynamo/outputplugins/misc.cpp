...
namespace dynamo
{
  OPMisc::OPMisc(const dynamo::Simulation *tmp, const magnet::xml::Node &) : OutputPlugin(tmp, "Misc", 0), // ContactMap must be after this
                                                                             _dualEvents(0),
                                                                             _singleEvents(0),
                                                                             _virtualEvents(0),
                                                                             _reverseEvents(0),
  ...
  template <class T>
  void outputCorrelator(magnet::xml::XmlStream &XML, const double inv_units, const double time_units, magnet::math::LogarithmicTimeCorrelator<T> &corr, const double time)
  {
    using namespace magnet::xml;

    if constexpr (std::is_same<T, double>::value)
    {
      XML << tag("AvgSqrImpulse")
          << attr("value") << corr.getAvgImpulseSqSum() * inv_units / time
  ...
  OPMisc::initialise()
  {
    ...
    for (size_t spid1(0); spid1 < Sim->species.size(); ++spid1)
    {
      ...
      for (size_t spid2(spid1); spid2 < Sim->species.size(); ++spid2)
      {
        ...
        _mutualDiffusion[spid1 * Sim->species.size() + spid2].setFreeStreamValue(_speciesMomenta[spid1] - (_speciesMasses[spid1] / _systemMass) * _sysMomentum.current(),
  ...
  OPMisc::eventUpdate(const Event &eevent, const NEventData &NDat)
  {
    ...
    for (const PairEventData &PDat : NDat.L2partChanges)
    {
      ...
      _kineticP += mass1 * (Dyadic(part1.getVelocity(), part1.getVelocity()) - Dyadic(PDat.particle1_.getOldVel(), PDat.particle1_.getOldVel())) + mass2 * (Dyadic(part2.getVelocity(), part2.getVelocity()) - Dyadic(PDat.particle2_.getOldVel(), PDat.particle2_.getOldVel()));
      ...
      thermalDel += part1.getVelocity() * p1E + part2.getVelocity() * p2E - PDat.particle1_.getOldVel() * (p1E - p1deltaE) - PDat.particle2_.getOldVel() * (p2E - p2deltaE);
    }

    _thermalConductivity.setFreeStreamValue(_thermalConductivity.getFreeStreamValue() + thermalDel);
  ...
  OPMisc::getMFT() const
  {
    return Sim->systemTime * static_cast<double>(Sim->N()) / (Sim->units.unitTime() * ((2.0 * static_cast<double>(_dualEvents)) + static_cast<double>(_singleEvents)));
  }
  ...
  OPMisc::output(magnet::xml::XmlStream &XML)
  {
    ...
    if (!std::dynamic_pointer_cast<BCLeesEdwards>(Sim->BCs))
      XML << tag("ResidualHeatCapacity")
          << attr("Value")
          << (getMeanSqrUConfigurational() - getMeanUConfigurational() * getMeanUConfigurational()) / (getMeankT() * getMeankT())
    ...
    if (!std::dynamic_pointer_cast<BCLeesEdwards>(Sim->BCs))
    {
      ...
      {
        const double inv_units = Sim->units.unitk() / (Sim->units.unitTime() * Sim->units.unitThermalCond() * 2.0 * getMeankT() * getMeankT() * V);

        outputCorrelator(XML, inv_units, Sim->units.unitTime(), _thermalConductivity, Sim->systemTime);
      }
...

      {
        ...
        outputCorrelator(XML, inv_units, Sim->units.unitTime(), _viscosity, Sim->systemTime);
      }
      ...
      {
        const double inv_units = 1.0 / (Sim->units.unitTime() * Sim->units.unitViscosity() * 2.0 * getMeankT() * V);
        outputCorrelator(XML, inv_units, Sim->units.unitTime(), _bulkVisc, Sim->systemTime);
      }
      ...
      {
        const double inv_units = 1.0 / (Sim->units.unitTime() * Sim->units.unitViscosity() * 2.0 * getMeankT() * V);
        outputCorrelator(XML, inv_units, Sim->units.unitTime(), _crossVisc, Sim->systemTime);
      }
      ...
      for (size_t i(0); i < Sim->species.size(); ++i)
      {
        ...
        const double inv_units = 1.0 / (Sim->units.unitTime() * Sim->units.unitThermalDiffusion() * 2.0 * getMeankT() * V);

        outputCorrelator(XML, inv_units, Sim->units.unitTime(), _thermalDiffusion[i], Sim->systemTime);
      ...
      for (size_t i(0); i < Sim->species.size(); ++i)
        for (size_t j(i); j < Sim->species.size(); ++j)
        {
          ...
          const double inv_units = 1.0 / (Sim->units.unitTime() * Sim->units.unitMutualDiffusion() * 2.0 * getMeankT() * V);

          outputCorrelator(XML, inv_units, Sim->units.unitTime(), _mutualDiffusion[i * Sim->species.size() + j], Sim->systemTime);
