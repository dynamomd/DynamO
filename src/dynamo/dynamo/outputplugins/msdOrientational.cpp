...
void OPMSDOrientational::initialise() {
  initialConfiguration.clear();
  initialConfiguration.resize(Sim->N());

  const std::vector<Dynamics::rotData> &rdat(
      Sim->dynamics->getCompleteRotData());

  for (size_t ID = 0; ID < Sim->N(); ++ID) {
    initialConfiguration[ID] =
        RUpair(Sim->particles[ID].getPosition(),
               rdat[ID].orientation * Quaternion::initialDirector());
  }
}

void OPMSDOrientational::output(magnet::xml::XmlStream &XML) {
  msdCalcReturn MSDOrientational(calculate());

  XML << magnet::xml::tag("MSDOrientational")

      << magnet::xml::tag("Perpendicular") << magnet::xml::attr("val")
      << MSDOrientational.perpendicular << magnet::xml::attr("diffusionCoeff")
      << MSDOrientational.perpendicular * Sim->units.unitTime() /
             Sim->systemTime
      << magnet::xml::endtag("Perpendicular")

      << magnet::xml::tag("Parallel") << magnet::xml::attr("val")
      << MSDOrientational.parallel << magnet::xml::attr("diffusionCoeff")
      << MSDOrientational.parallel * Sim->units.unitTime() / Sim->systemTime
      << magnet::xml::endtag("Parallel")

      << magnet::xml::tag("Rotational") << magnet::xml::attr("method")
      << "LegendrePolynomial1" << magnet::xml::attr("val")
      << MSDOrientational.rotational_legendre1
      << magnet::xml::attr("diffusionCoeff")
      << MSDOrientational.rotational_legendre1 * Sim->units.unitTime() /
             Sim->systemTime
      << magnet::xml::endtag("Rotational")

      << magnet::xml::tag("Rotational") << magnet::xml::attr("method")
      << "LegendrePolynomial2" << magnet::xml::attr("val")
      << MSDOrientational.rotational_legendre2
      << magnet::xml::attr("diffusionCoeff")
      << MSDOrientational.rotational_legendre2 * Sim->units.unitTime() /
             Sim->systemTime
      << magnet::xml::endtag("Rotational")
...
OPMSDOrientational::msdCalcReturn OPMSDOrientational::calculate() const {
  msdCalcReturn MSR;
  ...
  const std::vector<Dynamics::rotData> &latest_rdat(
      Sim->dynamics->getCompleteRotData());

  for (const Particle &part : Sim->particles) {
    displacement_term =
        part.getPosition() - initialConfiguration[part.getID()].first;
    longitudinal_projection =
        (displacement_term | initialConfiguration[part.getID()].second);
    cos_theta = (initialConfiguration[part.getID()].second |
                 (latest_rdat[part.getID()].orientation *
                  Quaternion::initialDirector()));

    acc_perp +=
        (displacement_term -
         (longitudinal_projection * initialConfiguration[part.getID()].second))
            .nrm2();
  ...
  // In the N-dimensional case, the parallel component is 1-dimensonal and the
  // perpendicular (N-1)
  acc_perp /=
      (initialConfiguration.size() * 2.0 * (NDIM - 1) * Sim->units.unitArea());
  ...
  acc_rotational_legendre1 =
      log(acc_rotational_legendre1 / initialConfiguration.size()) / (-2.0);
  acc_rotational_legendre2 =
      log(acc_rotational_legendre2 / initialConfiguration.size()) / (-6.0);
