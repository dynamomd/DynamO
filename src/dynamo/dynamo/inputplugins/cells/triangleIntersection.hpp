...
struct CUTriangleIntersect : public UCell {
  ...
  CUTriangleIntersect(UCell *nextCell, double diameter, std::string fileName)
      : UCell(nextCell), _diameter(diameter), _diametersq(diameter * diameter),
        _fileName(fileName) {}
  ...
  virtual void initialise() {
    ...
    if (!input)
      M_throw() << "Could not open " << _fileName
                << " to load the triangles from";
    ...
    std::cout << "\nCUTriangleIntersect :Loaded " << _triangles.size()
              << " triangles";
    std::cout.flush();

    for (triangle_type &triangle : _triangles) {
      // std::cout << "\nCUTriangleIntersect :Loaded " << triangle[0] << " " <<
  ...
  bool triangleIntersects(const Vector &sphere,
    ...
    // Check if the triangle contains the sphere
    {
      ...
      // std::cerr << "\nCross product should be zero and is actually " << ((C ^
      // triangle[1]) | triangle[0]);
    ...
    if (sphereEdgeCheck(triangle[0], triangle[1], sphere))
      return true;
    if (sphereEdgeCheck(triangle[0], triangle[2], sphere))
      return true;
    if (sphereEdgeCheck(triangle[0] + triangle[1], triangle[2] - triangle[1],
                        sphere))
      return true;
  ...
  bool sphereEdgeCheck(const Vector &linecentre, const Vector &edge,
                       const Vector &sphere) const {
