...
struct CUHCP : public UCell {
  CUHCP(std::array<long, 3> ncells, Vector ndimensions, UCell *nextCell)
    ...
    _cellDim =
        _lattice_size / std::max(_lattice_size[0],
                                 std::max(_lattice_size[1], _lattice_size[2]));
  ...
  virtual std::vector<Vector> placeObjects(const Vector &centre) {
    ...
    while (iterVec[NDIM - 1] != cells[NDIM - 1]) {
      ...
      for (size_t i(0); i < 4; ++i) {
        ...
        for (const Vector &vec : uc->placeObjects(
                 position + elementwiseMultiply(rel_pos, cellWidth)))
