...
SysFrancesco::SysFrancesco(const magnet::xml::Node &XML,
...
SysFrancesco::SysFrancesco(dynamo::Simulation *nSim, double mft, double t,
                           std::string nName)
    : System(nSim), meanFreeTime(mft / nSim->N()), Temp(t), dimensions(NDIM),
      eventCount(0), lastlNColl(0), range(new IDRangeAll(Sim)) {
  sysName = nName;
  type = GAUSSIAN;
}

NEventData SysFrancesco::runEvent() {
  ++Sim->eventCount;
  ++eventCount;
  dt = getGhostt();

  size_t step = std::uniform_int_distribution<size_t>(0, range->size() - 1)(
      Sim->ranGenerator);
...
void SysFrancesco::initialise(size_t nID) {
  ID = nID;
  dt = getGhostt();
  sqrtTemp = sqrt(Temp);
  eventCount = 0;
...
void SysFrancesco::operator<<(const magnet::xml::Node &XML) {
  meanFreeTime =
...
void SysFrancesco::outputXML(magnet::xml::XmlStream &XML) const {
  XML << magnet::xml::tag("System") << magnet::xml::attr("Type") << "Francesco"
      << magnet::xml::attr("Name") << sysName << magnet::xml::attr("MFT")
      << meanFreeTime * Sim->N() / Sim->units.unitTime()
      << magnet::xml::attr("Temperature") << Temp << magnet::xml::attr("R")
      << _R / Sim->units.unitLength() / Sim->units.unitEnergy();
...
double SysFrancesco::getGhostt() const {
  return -meanFreeTime *
         std::log(1.0 - std::uniform_real_distribution<>()(Sim->ranGenerator));
}

double SysFrancesco::getReducedTemperature() const {
  return Temp / Sim->units.unitEnergy();
}

void SysFrancesco::setReducedTemperature(double nT) {
