...
namespace dynamo {
SVisualizer::SVisualizer(dynamo::Simulation *nSim, std::string nName,
                         double tickFreq)
    : System(nSim) {
...
NEventData SVisualizer::runEvent() {
  if (_window->dynamoParticleSync())
    Sim->dynamics->updateAllParticles();

  for (shared_ptr<System> &system : Sim->systems) {
    shared_ptr<SysRotateGravity> rgrav =
        std::dynamic_pointer_cast<SysRotateGravity>(system);
    if (!rgrav)
      continue;

    std::shared_ptr<DynGravity> dynamics =
        std::dynamic_pointer_cast<DynGravity>(Sim->dynamics);
    if (dynamics)
      _window->getGLContext()->queueTask(
          std::bind(&magnet::GL::Camera::setUp, &(_window->getCamera()),
                    -dynamics->getGravityVector(), rgrav->getAxis()));
    break;
  }
...
void SVisualizer::initialise(size_t nID) {
  ID = nID;
  ...
  for (shared_ptr<Local> &local : Sim->locals) {
    ...
    if (obj != NULL) {
      ...
      _window->_updateDataSignal
          .connect<CoilRenderObj, &CoilRenderObj::updateRenderData>(obj);
    }
  }
  ...
  _window->_updateDataSignal
      .connect<SVisualizer, &SVisualizer::updateRenderData>(this);
  updateRenderData();
  ...
  Sim->_sigParticleUpdate.connect<SVisualizer, &SVisualizer::particlesUpdated>(
      this);
}

void SVisualizer::particlesUpdated(const NEventData &) {
...
void SVisualizer::initDataSet() {
  ...
  _particleData->addAttribute(
      "Size",
      coil::Attribute::INTENSIVE | coil::Attribute::DEFAULT_GLYPH_SCALING, 4);
  _particleData->addAttribute("Mass", coil::Attribute::EXTENSIVE, 1);
  // We allow glyphs to be default color white
  _particleData->addAttribute(
      "ID",
      coil::Attribute::INTENSIVE /*| coil::Attribute::DEFAULT_GLYPH_COLOUR*/,
      1);
  ...
  if (Sim->dynamics->hasOrientationData()) {
    _particleData->addAttribute("Orientation",
                                coil::Attribute::EXTENSIVE |
                                    coil::Attribute::DEFAULT_GLYPH_ORIENTATION,
                                4);
    _particleData->addAttribute("Angular Velocity", coil::Attribute::EXTENSIVE,
                                3);
  }
  ...
  for (const Particle &p : Sim->particles) {
    IDs[p.getID()] = p.getID();
    masses[p.getID()] =
        Sim->species[p]->getMass(p.getID()) / Sim->units.unitMass();
  }

  (*_particleData)["Mass"].flagNewData();
  (*_particleData)["ID"].flagNewData();

  // Check through every interaction, collecting the IDs of particles
  ...
  for (const Particle &particle : Sim->particles)
    _interactionIDs[Sim->getInteraction(particle, particle)->getID()].push_back(
        particle.getID());

  // Update the size information once (only Compression dynamics needs to update
  // it again)
  ...
  for (auto &interaction : Sim->interactions)
    if (!_interactionIDs[interaction->getID()].empty()) {
      dout << "Rendering Interaction \"" << interaction->getName() << "\" with "
           << _interactionIDs[interaction->getID()].size() << " particles"
           << std::endl;
  ...
  for (auto &interaction : Sim->interactions)
    if (!_interactionIDs[interaction->getID()].empty())
      _particleData->addPointSet(interaction->getName(),
                                 _interactionIDs[interaction->getID()],
                                 interaction->getDefaultGlyphType());
}

void SVisualizer::updateRenderData() {
  if (!_particleData)
    M_throw() << "Updating before the render object has been fetched";

  shared_ptr<BCLeesEdwards> BC =
      std::dynamic_pointer_cast<BCLeesEdwards>(Sim->BCs);
  if (BC)
    _particleData->setPeriodicVectors(
        Vector{Sim->primaryCellSize[0], 0, 0},
  ...
  if (std::dynamic_pointer_cast<DynCompression>(Sim->dynamics)) {
    ...
    const double rfactor =
        (1 +
         static_cast<const DynCompression &>(*Sim->dynamics).getGrowthRate() *
             Sim->systemTime) /
  ...
  if (Sim->dynamics->hasOrientationData()) {
    ...
    const std::vector<Dynamics::rotData> &data =
