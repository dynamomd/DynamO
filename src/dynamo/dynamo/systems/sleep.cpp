...
SSleep::SSleep(dynamo::Simulation *nSim, std::string nName, IDRange *r1,
               double sleepV)
    : System(nSim), _range(r1), _sleepVelocity(sleepV) {
  sysName = nName;
  type = SLEEP;
}

void SSleep::initialise(size_t nID) {
  ID = nID;
...
void SSleep::operator<<(const magnet::xml::Node &XML) {
  sysName = XML.getAttribute("Name");
  _sleepVelocity =
      XML.getAttribute("SleepV").as<double>() * Sim->units.unitVelocity();
...
void SSleep::outputXML(magnet::xml::XmlStream &XML) const {
  XML << magnet::xml::tag("System") << magnet::xml::attr("Type") << "Sleep"
      << magnet::xml::attr("Name") << sysName << magnet::xml::attr("SleepV")
      << _sleepVelocity / Sim->units.unitVelocity() << _range
      << magnet::xml::endtag("System");
}

void SSleep::recalculateTime() {
  dt = (stateChange.empty()) ? std::numeric_limits<float>::infinity()
                             : -std::numeric_limits<float>::max();
  type = (stateChange.empty()) ? NONE : SLEEP;
}

bool SSleep::sleepCondition(const Particle &part, const Vector &g,
                            const Vector &vel) {
...
void SSleep::particlesUpdated(const NEventData &PDat) {
  for (const PairEventData &pdat : PDat.L2partChanges) {
    ...
    // Other variables
    Vector g(
        static_cast<const DynGravity &>(*Sim->dynamics).getGravityVector());
