...
public:
  IDPairRangeRings(unsigned long r1, unsigned long r2, unsigned long r3)
    ...
    if ((r2 - r1 + 1) % r3)
      M_throw()
          << "Range of IDPairRangeRings does not split evenly into interval";
  }
    ...
    if (p1.getID() > p2.getID()) {
      if (p1.getID() - p2.getID() != 1) {
        if ((p1.getID() - p2.getID() == interval - 1) &&
          ...
          return true;
      } else if ((p2.getID() >= range1) && (p1.getID() <= range2))
        if (((p2.getID() - range1) / interval) ==
            ((p1.getID() - range1) / interval))
          return true;
    } else {
      if (p2.getID() - p1.getID() != 1) {
        if ((p2.getID() - p1.getID() == interval - 1) &&
            ...
            (((p1.getID() - range1) / interval) ==
             ((p2.getID() - range1) / interval)))
          return true;
      } else if ((p1.getID() >= range1) && (p2.getID() <= range2))
        if (((p1.getID() - range1) / interval) ==
            ((p2.getID() - range1) / interval))
          return true;
    }
    return false;
...
protected:
  virtual void outputXML(magnet::xml::XmlStream &XML) const {
    XML << magnet::xml::attr("Type") << "Rings" << magnet::xml::attr("Start")
