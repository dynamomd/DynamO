...
public:
  ...
  // THIS IS A STRANGE ONE, as above is longer, including a G type, but all
  // tables exclude G. Need to check this in Gil's work, for now, to fix out of
  // bounds error, I will define this here.
  ...
  */
  ...
  /*! \brief Unbonded interaction well-diameters.
    ...
    For SC-BB interactions, bead diameters from _PRIME_diameters multipled
    by 1.5.
  ...
  */
  ...
  struct BeadData {
    BeadData(PRIME_residue_type b, size_t r, PRIME_residue_type t = A,
             BeadLocation e = MID)
        : bead_type(b), residue(r), residue_type(t), location(e) {}

    // Define sorting for storing in maps (note location does not affect the
    // sorting order!)
    bool operator<(const BeadData &op) const {
      return bead_type < op.bead_type ||
             (!(op.bead_type < bead_type) && (residue < op.residue));
    }
    bool operator==(const BeadData &op) const {
      return (residue == op.residue) && (bead_type == op.bead_type);
    }
  ...
  typedef boost::bimaps::bimap<boost::bimaps::unordered_set_of<size_t>,
                               boost::bimaps::unordered_set_of<BeadData>>
      BeadTypeMap;
