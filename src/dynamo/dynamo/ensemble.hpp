...
public:
  ...
  */
  virtual void swap(Ensemble &rhs) {
    std::swap(EnsembleVals, rhs.EnsembleVals);
  }
  ...
  */
  virtual double exchangeProbability(const Ensemble &) const {
    M_throw() << "Undefined in this Ensemble";
  }
  ...
  */
  virtual const std::array<double, 3> &getEnsembleVals() const {
    M_throw() << "Undefined Ensemble";
  }
...
public:
  EnsembleNVE(const dynamo::Simulation *SD) : Ensemble(SD, "EnsembleNVE") {}
...
public:
  EnsembleNVT(const dynamo::Simulation *SD) : Ensemble(SD, "EnsembleNVT") {}
