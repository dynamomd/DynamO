...
GParabolaSentinel::GParabolaSentinel(dynamo::Simulation *nSim,
...
Event GParabolaSentinel::getEvent(const Particle &part) const {
  Sim->dynamics->updateParticle(Sim->particles[part.getID()]);

  return Event(part, Sim->dynamics->getParabolaSentinelTime(part), GLOBAL,
               RECALCULATE_PARABOLA, ID);
}

void GParabolaSentinel::runEvent(Particle &part, const double) {
