...
GSOCells::GSOCells(dynamo::Simulation *nSim, const std::string &name)
    : Global(nSim, "SingleOccupancyCells"), _cellD(0) {
  globName = name;
...
void GSOCells::operator<<(const magnet::xml::Node &XML) {
  globName = XML.getAttribute("Name");

  if (XML.hasNode("CellOrigins")) {
    Vector pos;
    for (magnet::xml::Node node = XML.getNode("CellOrigins").findNode("Origin");
         node.valid(); ++node) {
      pos << node;
    ...
    if (cell_origins.size() != Sim->N())
      M_throw() << "Number of CellOrigins (" << cell_origins.size()
                << ") does not match number of particles (" << Sim->N() << ")\n"
                << XML.getPath();
  }
...
Event GSOCells::getEvent(const Particle &part) const {
#ifdef ISSS_DEBUG
  ...
  //   dout << "#Velocity " << part.getVelocity() / Sim->units.unitVelocity() <<
  //   std::endl; dout << "#Distance " << pos.nrm() / Sim->units.unitLength() <<
  //   std::endl; dout << "#Delay " << Sim->dynamics->getParticleDelay(part) /
  //   Sim->units.unitTime() << std::endl; double dt =
  //   Sim->dynamics->SphereSphereOutRoot(part, cellParticle, _cellD); dout <<
  //   "#event t = " << dt / Sim->units.unitTime() << std::endl; Vector finalP =
  //   (pos + dt * part.getVelocity()); dout << "#Final Position " << finalP /
  //   Sim->units.unitLength() << std::endl; dout << "#Relative final p" <<
  //   (finalP.nrm() - _cellD)/_cellD<< std::endl; dout << " cell origin " <<
  //   cell_origins[part.getID()] / Sim->units.unitLength() << std::endl;
  // }
  ...
  return Event(part,
               magnet::intersection::ray_sphere<true>(r12, v12, _cellD / 2) -
...
void GSOCells::runEvent(Particle &part, const double) {
  ...
  if (iEvent._dt == std::numeric_limits<float>::infinity())
    M_throw() << "An infinite Interaction (not marked as NONE) collision time "
                 "has been found\n";
#endif
  ...
  // Sim->BCs->applyBC(pos); //We don't apply the PBC, as the particles are
  // always tied to the primary image

  // dout << "!Particle " << part.getID() << " at " << part.getPosition() /
  // Sim->units.unitLength() << std::endl; dout << "!Cell origin " <<
  // cell_origins[part.getID()] / Sim->units.unitLength() << std::endl; dout <<
  // "!Normal " << pos.normal() / Sim->units.unitLength() << std::endl; dout <<
  // "!Distance " << pos.nrm() / Sim->units.unitLength() << std::endl; dout <<
  // "!CellD " << _cellD / Sim->units.unitLength() << std::endl; dout <<
  // "!Relative distance " << pos.nrm() / _cellD << std::endl; dout << "!Perp
  // velocity " << (pos.normal() | part.getVelocity()) << std::endl;

  // Now execute the event
  NEventData EDat(
      Sim->dynamics->runPlaneEvent(part, pos.normal(), 1.0, _cellD / 2));
  // dout << "!Perp velocity post " << (pos.normal() | part.getVelocity()) <<
  // std::endl; if (pos.nrm() > _cellD * 1.00000001)
  //   derr << "Particle " << part.getID() << " outside the cell by " <<
  //   (pos.nrm() - _cellD) / _cellD  << std::endl;
  // if (pos.nrm() < _cellD * 0.99999999)
  //   derr << "Particle " << part.getID() << " inside the cell by " <<
  //   (pos.nrm() - _cellD) / _cellD  << std::endl;
...
void GSOCells::initialise(size_t nID) {
  ...
  if (typeid(*Sim->dynamics) != typeid(DynNewtonian))
    M_throw() << "ERROR: SOCells requires Newtonian dynamics, its not been "
                 "generalised for other dynamics yet.";
  ...
  for (const Particle &p : Sim->particles) {
    ...
    if (pos.nrm2() > _cellD * _cellD)
      derr << "Particle " << p.getID() << " is at a distance of "
           << (pos).nrm() / Sim->units.unitLength() << " cell origin "
           << cell_origins[p.getID()] / Sim->units.unitLength()
           << " outside its SOCell where the diameter is "
           << _cellD / Sim->units.unitLength() << std::endl;
  }
}

void GSOCells::outputXML(magnet::xml::XmlStream &XML) const {
  XML << magnet::xml::tag("Global") << magnet::xml::attr("Type") << "SOCells"
      << magnet::xml::attr("Name") << globName << magnet::xml::attr("Diameter")
      << _cellD / Sim->units.unitLength();

  XML << magnet::xml::tag("CellOrigins");
  for (const Vector cellorigin : cell_origins)
    XML << magnet::xml::tag("Origin") << cellorigin / Sim->units.unitLength()
        << magnet::xml::endtag("Origin");
  XML << magnet::xml::endtag("CellOrigins") << magnet::xml::endtag("Global");
}

void GSOCells::load_cell_origins(const std::vector<Particle> particles) {
