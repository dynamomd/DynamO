...
namespace dynamo {
GWaker::GWaker(dynamo::Simulation *nSim, const std::string &name,
               IDRange *range, const double wt, const double wv,
               std::string nblist)
    : Global(nSim, "GWaker", range), _wakeTime(wt), _wakeVelocity(wv),
      _nblistName(nblist) {
  globName = name;
...
void GWaker::initialise(size_t nID) {
...
void GWaker::operator<<(const magnet::xml::Node &XML) {
  ...
  try {
    ...
    _wakeTime =
        XML.getAttribute("WakeTime").as<double>() * Sim->units.unitTime();
...
Event GWaker::getEvent(const Particle &part) const {
  if (part.testState(Particle::DYNAMIC))
    return Event(part, std::numeric_limits<float>::infinity(), GLOBAL, NONE,
                 ID);
  else
...
void GWaker::runEvent(Particle &part, const double dt) {
  ...
  if (std::isnan(iEvent._dt))
    M_throw() << "A NAN Interaction collision time has been found" << iEvent;
  ...
  Vector newVel{norm_dist(Sim->ranGenerator), norm_dist(Sim->ranGenerator),
                norm_dist(Sim->ranGenerator)};
...
void GWaker::outputXML(magnet::xml::XmlStream &XML) const {
  XML << magnet::xml::tag("Global") << magnet::xml::attr("Type") << "Waker"
      << magnet::xml::attr("Name") << globName
      << magnet::xml::attr("WakeVelocity")
      << _wakeVelocity / Sim->units.unitVelocity()
      << magnet::xml::attr("WakeTime") << _wakeTime / Sim->units.unitTime()
      << magnet::xml::attr("NBList") << _nblistName << range
      << magnet::xml::endtag("Global");
}

void GWaker::nblistCallback(const Particle &part, const size_t &oid) const {
