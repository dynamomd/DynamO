...
Global::Global(dynamo::Simulation *tmp, std::string name, IDRange *nR)
    : SimBase(tmp, name), range(nR ? nR : new IDRangeAll(tmp)) {}

bool Global::isInteraction(const Particle &p1) const {
...
shared_ptr<Global> Global::getClass(const magnet::xml::Node &XML,
  ...
  if (!XML.getAttribute("Type").getValue().compare("Cells")) {
    ...
    M_throw() << XML.getAttribute("Type").getValue() << ", Unknown type ("
