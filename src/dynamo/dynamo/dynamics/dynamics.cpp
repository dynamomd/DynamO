...
shared_ptr<Dynamics> Dynamics::getClass(const magnet::xml::Node &XML,
...
void Dynamics::initialise() {
  ...
  if (hasOrientationData()) {
    ...
    if (!hasInertia)
      M_throw() << "No species have inertia, yet the particles have "
                   "orientational degrees of freedom set!";

    sumEnergy *= 0.5 / Sim->units.unitEnergy();

    dout << "System Rotational Energy " << sumEnergy << "\nRotational kT "
         << sumEnergy / Sim->N() << std::endl;
  }
}

NEventData Dynamics::enforceParabola(Particle &) const {
  M_throw() << "This is not needed for this type of Dynamics";
}

PairEventData Dynamics::runLineLineCollision(Event &, const double &,
                                             const double &) const {
  M_throw() << "Not implemented for this Dynamics.";
}

std::pair<bool, double> Dynamics::getLineLineCollision(const double,
                                                       const Particle &,
                                                       const Particle &,
                                                       double) const {
  M_throw() << "Not implemented for this Dynamics.";
...
std::pair<bool, double> Dynamics::getOffcentreSpheresCollision(
    const double offset1, const double diameter1, const double offset2,
    const double diameter2, const Particle &p1, const Particle &p2,
    double t_max, double maxdist) const {
  M_throw() << "Not implemented for this Dynamics.";
}

double Dynamics::getPBCSentinelTime(const Particle &, const double &) const {
  M_throw() << "Not implemented for this Dynamics.";
}

void Dynamics::loadParticleXMLData(const magnet::xml::Node &XML) {
  ...
  if (outofsequence)
    dout << "Particle ID's out of sequence!\n"
         ...
         << "Erase any capture maps in the configuration file so they are "
            "regenerated."
         << std::endl;
  ...
  if (XML.getNode("ParticleData").hasAttribute("OrientationData")) {
    ...
    for (magnet::xml::Node node = XML.getNode("ParticleData").findNode("Pt");
      ...
      if (orientationData[i].orientation.nrm() == 0)
        M_throw() << "Particle " << i
                  << " has an invalid zero orientation quaternion";
    }
  }
}

void Dynamics::outputParticleXMLData(magnet::xml::XmlStream &XML,
                                     bool applyBC) const {
  ...
  for (size_t i = 0; i < Sim->N(); ++i) {
    ...
    if (hasOrientationData())
      XML << magnet::xml::tag("O") << orientationData[i].angularVelocity
          << magnet::xml::endtag("O") << magnet::xml::tag("U")
          << orientationData[i].orientation << magnet::xml::endtag("U");
...
size_t Dynamics::getParticleDOF() const {
  size_t DOFsum(0);
...
double Dynamics::getSystemKineticEnergy() const {
  double sumEnergy(0);
...
void Dynamics::rescaleSystemKineticEnergy(const double &scale) {
...
PairEventData Dynamics::parallelCubeColl(Event &event, const double &e,
                                         const double &d,
...
ParticleEventData Dynamics::runOscilatingPlate(
...
double Dynamics::getCylinderWallCollision(const Particle &part,
                                          const Vector &origin,
                                          const Vector &norm,
                                          const double &) const {
  M_throw() << "Not Implemented";
...
ParticleEventData Dynamics::runCylinderWallCollision(Particle &, const Vector &,
                                                     const Vector &,
                                                     const double &) const {
  M_throw() << "Not Implemented";
}

PairEventData Dynamics::RoughSpheresColl(Event &event, const double &e,
...
ParticleEventData Dynamics::runRoughWallCollision(Particle &part,
                                                  const Vector &vNorm,
                                                  const double &e,
                                                  const double &et,
...
void Dynamics::initOrientations(double kbT) {
  ...
  for (size_t i = 0; i < Sim->particles.size(); ++i) {
    ...
    if (std::isinf(I))
      ...
      orientationData[i].angularVelocity =
          Quaternion::initialDirector() ^ angVelCrossing;
      orientationData[i].angularVelocity *=
          0.5 * std::sqrt(kbT / I) * norm_dist(Sim->ranGenerator) /
...
Dynamics::getSphereTriangleEvent(const Particle &part, const Vector &A,
...
Dynamics::getCOMPosVel(const IDRange &particles) const {
  if (particles.empty())
    M_throw() << "Cannot calculate the COM position and velocity from an empty "
                 "IDRange";

  Vector pos = Vector{0, 0, 0}, vel = Vector{0, 0, 0};
...
void Dynamics::stream(const double &dt) {
  partPecTime += dt;
...
void Dynamics::advanceUpdateParticle(Particle &part, double &dt) const {
  streamParticle(part, dt + partPecTime + part.getPecTime());
  part.getPecTime() = -dt - partPecTime;
}

void Dynamics::updateParticle(Particle &part) const {
  streamParticle(part, part.getPecTime() + partPecTime);
  part.getPecTime() = -partPecTime;
}

bool Dynamics::isUpToDate(const Particle &part) const {
  return part.getPecTime() == -partPecTime;
}

void Dynamics::updateAllParticles() const {
...
void Dynamics::updateParticlePair(Particle &p1, Particle &p2) const {
...
double Dynamics::getParticleDelay(const Particle &part) const {
  return partPecTime + part.getPecTime();
}

Dynamics::rotData &Dynamics::getRotData(const Particle &part) {
  return getRotData(part.getID());
}

const Dynamics::rotData &Dynamics::getRotData(const Particle &part) const {
  return getRotData(part.getID());
}
} // namespace dynamo
