...
namespace dynamo {
DynNewtonianMCCMap::DynNewtonianMCCMap(dynamo::Simulation *tmp,
  ...
  if (XML.hasNode("Potential")) {
    for (magnet::xml::Node map_node = XML.getNode("Potential").findNode("Map");
      ...
      for (magnet::xml::Node entry_node = map_node.findNode("Contact");
           entry_node.valid(); ++entry_node)
        map[detail::CaptureMap::key_type(
            entry_node.getAttribute("ID1").as<size_t>(),
...
void DynNewtonianMCCMap::outputXML(magnet::xml::XmlStream &XML) const {
  XML << magnet::xml::attr("Type") << "NewtonianMCCMap"
      << magnet::xml::attr("Interaction") << _interaction_name
      << magnet::xml::tag("Potential");

  for (const auto &entry : _single_W) {
    XML << magnet::xml::tag("Map") << magnet::xml::attr("W")
        << entry.second._wval << magnet::xml::attr("Distance")
        << entry.second._distance;

    for (const auto &val : entry.first)
      XML << magnet::xml::tag("Contact") << magnet::xml::attr("ID1")
          << val.first.first << magnet::xml::attr("ID2") << val.first.second
...
NEventData DynNewtonianMCCMap::multibdyWellEvent(const IDRange &range1,
...
PairEventData DynNewtonianMCCMap::SphereWellEvent(Event &event,
                                                  const double &deltaKE,
                                                  const double &,
  ...
  // Calculate the deformed energy change of the system (the one used in the
  // dynamics)
  double MCDeltaKE = deltaKE;

  // If there are entries for the current and possible future energy, then take
  // them into account
  ...
  if ((MCDeltaKE < 0) && (sqrtArg < 0)) {
    ...
    if (retVal.rvdot < 0)
      retVal.impulse =
          retVal.rij * (2.0 * MCDeltaKE / (std::sqrt(sqrtArg) - retVal.rvdot));
    else
      retVal.impulse =
          retVal.rij * (-2.0 * MCDeltaKE / (retVal.rvdot + std::sqrt(sqrtArg)));
  }
...
void DynNewtonianMCCMap::replicaExchange(Dynamics &oDynamics) {
#ifdef DYNAMO_DEBUG
...
double DynNewtonianMCCMap::W(const detail::CaptureMap &map) const {
