...
namespace dynamo {
DynNewtonianMC::DynNewtonianMC(dynamo::Simulation *tmp,
                               const magnet::xml::Node &XML)
    : DynNewtonian(tmp), EnergyPotentialStep(1) {
  if (XML.hasNode("PotentialDeformation")) {
    EnergyPotentialStep = XML.getNode("PotentialDeformation")
                              .getAttribute("EnergyStep")
                              .as<double>() /
                          Sim->units.unitEnergy();

    for (magnet::xml::Node node =
             XML.getNode("PotentialDeformation").findNode("W");
         node.valid(); ++node) {
      double energy =
          node.getAttribute("Energy").as<double>() / Sim->units.unitEnergy();
...
void DynNewtonianMC::outputXML(magnet::xml::XmlStream &XML) const {
  std::unordered_map<int, double> wout = _W;

  XML << magnet::xml::attr("Type") << "NewtonianMC"
  ...
  for (const locpair &val : wout)
    XML << magnet::xml::tag("W") << magnet::xml::attr("Energy")
        ...
        << magnet::xml::attr("Value") << val.second << magnet::xml::endtag("W");
...
NEventData DynNewtonianMC::multibdyWellEvent(const IDRange &range1,
...
PairEventData DynNewtonianMC::SphereWellEvent(Event &event,
                                              const double &deltaKE,
