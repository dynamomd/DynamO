...
namespace dynamo {
  DynBaloon::DynBaloon(dynamo::Simulation* tmp, const magnet::xml::Node& XML):
    DynNewtonian(tmp) {
  }

  void 
  DynBaloon::initialise()
  {
    Dynamics::initialise();
  }

  void 
  DynBaloon::loadParticleXMLData(const magnet::xml::Node& xml)
  {
    Dynamics::loadParticleXMLData(xml);
  }

  double 
  DynBaloon::SphereSphereInRoot(const Particle& p1, const Particle& p2, double d) const
  {
  ...
  double
  DynBaloon::SphereSphereOutRoot(const Particle& p1, const Particle& p2, double d) const
  {
    Vector r12 = p1.getPosition() - p2.getPosition();
    Vector v12 = p1.getVelocity() - p2.getVelocity();
    Sim->BCs->applyBC(r12, v12);
  ...
  double 
  DynBaloon::getPlaneEvent(const Particle& part, const Vector & origin, const Vector & norm, double diameter) const
  {
  ...
  ParticleEventData 
  DynBaloon::runPlaneEvent(Particle& part, const Vector& vNorm, const double e, const double diameter) const
  {
    updateParticle(part);
  ...
  double
  DynBaloon::sphereOverlap(const Particle& p1, const Particle& p2,
  ...
  PairEventData 
  DynBaloon::SmoothSpheresColl(Event& event, const double& e, const double& d2, const EEventType& eType) const
  {
  ...
  PairEventData 
  DynBaloon::SphereWellEvent(Event& event, const double& deltaKE, const double& d2, size_t) const
  {
    Particle& particle1 = Sim->particles[event._particle1ID];
    Particle& particle2 = Sim->particles[event._particle2ID];
    updateParticlePair(particle1, particle2);  
  ...
  void 
  DynBaloon::outputXML(magnet::xml::XmlStream& XML) const
  {
  ...
  double 
  DynBaloon::getPBCSentinelTime(const Particle& part,
  ...
  PairEventData 
  DynBaloon::parallelCubeColl(Event&, const double&, const double&, const EEventType&) const
  { M_throw() << "Not Implemented"; }


  ParticleEventData 
  DynBaloon::runAndersenWallCollision(Particle& part, const Vector & vNorm, const double& sqrtT, const double d, const double slip) const
  {  
    updateParticle(part);
