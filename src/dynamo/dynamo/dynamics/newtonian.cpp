...
namespace dynamo
{
  ...
  DynNewtonian::DynNewtonian(dynamo::Simulation *tmp) : Dynamics(tmp), lastAbsoluteClock(-1), lastCollParticle1(0), lastCollParticle2(0)
  {
  }
  ...
  DynNewtonian::streamParticle(Particle &particle, const double &dt) const
  {
    ...
    if (hasOrientationData())
    {
      orientationData[particle.getID()].orientation = Quaternion::fromRotationAxis(orientationData[particle.getID()].angularVelocity * dt) * orientationData[particle.getID()].orientation;
  ...
  int DynNewtonian::getSquareCellCollision3(const Particle &part, const Vector &origin, const Vector &width) const
  {
    Vector rpos(part.getPosition() - origin);
    Vector vel(part.getVelocity());
    Sim->BCs->applyBC(rpos, vel);
  ...
  DynNewtonian::getPointPlateCollision(const Particle &part, const Vector &nrw0, const Vector &nhat, const double &Delta, const double &Omega, const double &Sigma, const double &t, bool lastpart) const
  {
    ...
    std::pair<bool, double> root1 = magnet::math::frenkelRootSearch(fL, t_low1, t_high, 1e-12 * Sigma);
    ...
    std::pair<bool, double> root2 = magnet::math::frenkelRootSearch(fL, t_low2, t_high, 1e-12 * Sigma);

    // Check if the particle is penetrating a wall
    // Or if no roots are found at all
    if ((fabs(surfaceOffset - (nhat | fL.wallPosition())) > Sigma) || ((root1.second == std::numeric_limits<float>::infinity()) && (root2.second == std::numeric_limits<float>::infinity())) || ((t_low1 > t_high) && (t_low2 > t_high)))
    {
      ...
      derr << "Particle " << part.getID()
           ...
           << "\nerror = "
           << (fabs(surfaceOffset - (nhat | fL.wallPosition())) - Sigma) / Sim->units.unitLength()
           << "\n Root1 = "
  ...
  std::pair<bool, double>
  DynNewtonian::getLineLineCollision(const double length, const Particle &p1, const Particle &p2, double t_max) const
  {
#ifdef DYNAMO_DEBUG
    if (!hasOrientationData())
  ...
  DynNewtonian::runRoughWallCollision(Particle &part, const Vector &vNorm, const double &e, const double &et, const double &r) const
  {
    ...
    double Jbar = Sim->species[retVal.getSpeciesID()]->getScalarMomentOfInertia(part.getID()) / (p1Mass * r * r);

    Vector gij = part.getVelocity() - r * (orientationData[part.getID()].angularVelocity ^ vNorm);

    Vector gijt = (vNorm ^ gij) ^ vNorm;

    part.getVelocity() -= (1 + e) * (vNorm | part.getVelocity()) * vNorm + (Jbar * (1 - et) / (Jbar + 1)) * gijt;
