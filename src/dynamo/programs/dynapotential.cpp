...
int main(int argc, char *argv[]) {
  try {
    ...
    options.add_options()("help", "Produces this message")(
        "cutoff", po::value<double>()->default_value(3),
        "The cutoff radius of the potential")(
        "attractive-steps", po::value<double>()->default_value(1),
        "The number of steps in the attractive part of the potential")(
        "steps", po::value<size_t>()->default_value(10),
        "The number of steps to output data for")(
        ...
        "kT", po::value<double>()->default_value(1),
        "Set the temperature for the B2 algorithm");
    ...
    if (vm.count("help")) {
      std::cout
          << "dynapotential  Copyright (C) 2011  Marcus N Campbell Bannerman\n"
    ...
    if (vm.count("deltar"))
      R_mode = PotentialLennardJones::DELTAR;
    else if (vm.count("deltau"))
      R_mode = PotentialLennardJones::DELTAU;
    else if (vm.count("deltav"))
      R_mode = PotentialLennardJones::DELTAV;
    else
      M_throw() << "Please specify which step placement algorithm to use";

    PotentialLennardJones::UMode U_mode;
    if (vm.count("mid"))
      U_mode = PotentialLennardJones::MIDPOINT;
    else if (vm.count("left"))
      U_mode = PotentialLennardJones::LEFT;
    else if (vm.count("right"))
      U_mode = PotentialLennardJones::RIGHT;
    else if (vm.count("volume"))
      U_mode = PotentialLennardJones::VOLUME;
    else if (vm.count("virial"))
      U_mode = PotentialLennardJones::VIRIAL;
    else if (vm.count("midvolume"))
      U_mode = PotentialLennardJones::MIDVOLUME;
    else
      M_throw() << "Please specify which step energy algorithm to use";

    PotentialLennardJones LJ(1.0, 1.0, vm["cutoff"].as<double>(), U_mode,
                             R_mode, vm["attractive-steps"].as<double>(),
                             vm["kT"].as<double>());
...
#ifndef DYNAMO_DEBUG
    std::cout << "Try using the debugging executable for more information on "
                 "the error."
              << std::endl;
#endif
    return 1;
