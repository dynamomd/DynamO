...
**** TODO Coil: Make the shader/sphere,shader/detail/filter, and object/cairo shader use the new defines interface.
...
**** TODO Coil: Add a ambient light shader, perhaps have SSAO on in this?
...
**** DONE Bugfix DynamO: Multi-core parallel tempering is now actually in parallel. Since Oct 2009 the code actually has been executing in serial.
...
**** TODO Coil: Fix the rotation of the camera objects freaking out at the peak, head tracking and standard rotate camera not being about the center point.
**** TODO Magnet: Fix the camera lookAt code which is messing with the camera's actual position.
**** TODO DynamO: Allow replica exchanges in collisions.
**** TODO Coil: Subtract the background color before saving to png (or renormalise the color).
*** DynamO 1.2							    :ARCHIVE:
