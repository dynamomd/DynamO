...
**** DONE Coil: Cleaned up the stereo (3D) vision mode, so it's possible for standard 3D TV's to use it as well as red-cyan glasses.
**** DONE Coil: Moved to a deferred shading and forward rendering setup. Improves performance for complex systems.
**** DONE Coil: Now requires and complies with the OpenGL 3.3 Core profile.
**** DONE Coil: Fixed segfaults occuring when adding a window crashes the main render thread.
**** DONE Coil: Fixed a rare segfault caused by a misinterpretation of the glade file data stored in memory.
**** DONE Coil: The snapshot button now captures using the same file counter as the record button.
**** DONE Coil: Disabled the console text feature in coil. Need to rethink its implementation.
...
**** TODO Coil: Fix the position look-up in the shadow mapping, perhaps using texel lookups.
**** TODO Coil: Correct the shadow mapping calculation and remove the noise.
**** TODO Coil: Check the perspective correction on the raytraced spheres, they're not fisheyed like the triangular ones are
**** TODO Coil: Need buttons to align the camera with the XYZ directions.
...
**** TODO Fix the coil library volume renderer.
