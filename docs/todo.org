...
**** TODO Coil: Fix bloom with a black background.
**** TODO Coil: Fix the volume renderer in the new pipeline.
     Fix the calculation of the forward rendering depth buffers and
     correct the normal calculation, as it appears to be terrible.

     Check that the light position is read in eye-space coordinates,
     not object coordinates!
**** TODO Coil: Fix the DOF filter and the SSAO filter, as multisampling has changed them.
**** TODO Coil: Fix the incorrect calculation of the camera position, due to the head position not being added on. This is breaking lots of calculations (e.g. SSAO).
