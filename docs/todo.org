...
**** DONE Coil: HDR Bloom effects can now be enabled.
**** DONE Coil: Remove the requirement to pass the frequency to the visualiser, it guesses at one MFT for each particle.
...
**** TODO DynamO: Fix compression, it seems to jam on binary systems.
