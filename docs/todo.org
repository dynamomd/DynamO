...
**** TODO Benchmark binary XML mode and text mode to see if binary is still required
**** TODO Volume Renderer
     - [ ] Move to OpenCL generation of the gradients from live data
     - [X] Implement volume renderer
     - [X] Make a transfer function interface
**** TODO Optimize the MinMax Heap memory usage to remove the wasted element
