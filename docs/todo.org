...
**** TODO Move to anti-aliased VSM
**** TODO Fix the light bleeding on VSM
...
**** TODO Add information on the data sets to the data set screen.
...
**** DONE *BUGFIX* DynamO: Fixed a load error on the MFT, triggered by an input configuration from a simulation with no events.
...
**** DONE Lots more Doxygen comments to help people understand the source code.
...
**** DONE DynamO: Implemented multicanonical simulations!
...
**** DONE COIL: Moved to a new console output formatting class.
**** DONE COIL: Moved to OpenGL 3.3, remove all the old OpenGL calls.
**** DONE COIL: Improve the way data is made available to Coil, allowing a paraview-like interface to the data sources.
**** DONE COIL: Cleanup the wii-mote head-tracking class.
**** TODO Work on Sebastians project.
**** TODO Compression with walls.
*** DynamO 1.1 							    :ARCHIVE:
