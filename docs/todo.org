...
**** TODO DynamO: Move the test state code into the liouvillean.
**** TODO DynamO: SquareBond assumes that if a core event occurs a well event cannot occur beforehand.
