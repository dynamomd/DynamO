...
for FCC in $(seq 4 10) $(seq 12 2 20) $(seq 24 4 40) $(seq 45 5 70) $(seq 80 10 100);do
	...
	bzcat config.out.xml.bz2  | xmlstarlet ed -u '//Global/@Type' -v 'CellsMorton' | bzip2 > config.tmp.xml.bz2
	...
	echo "$N $celldens $CellCount $t1 $t2 $t3" >> profile.morton.Rho$DENS.results
...
for FCC in  30 ;do #20 25
	...
	echo "$N $celldens $CellCount $t1 $t2 $t3" >> profile.morton.F$FCC.results
	echo "$N $celldens $CellCount $t1 $t2 $t3"
    done
done
