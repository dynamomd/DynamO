...
function HS_replex_test {
    for i in $(seq 0 2); do
	../bin/dynamod -m 0 -C 3 -T $(echo "0.5*$i + 0.5" | bc -l) \
    ...
    #Equilibration
    ../bin/dynarun --engine 2 $2 -c 10000000000 -i 10 -f 600 \
...
function HS_compressiontest { 
    ...
    ../bin/dynamod -m0 -d0.5 -C 4 &> run.log
    ...
    #Run the simulation
    ../bin/dynarun -c 100000 --engine 3 --growth-rate 100.0 $2 tmp.xml.bz2 &> run2.log
    cat run2.log >> run.log
    rm run2.log
    ../bin/dynarun -c 1 config.out.xml.bz2 &> run2.log
    cat run2.log >> run.log
    rm run2.log
...
#Just tests the square shoulder interaction between two walls
    ...
    ../bin/dynarun -c 9995 $2 tmp.xml.bz2 &> run.log
